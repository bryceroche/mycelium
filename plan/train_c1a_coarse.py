"""
C1-A Coarse Boundary Window Detection

Predicts boundary windows (W=16, S=8) instead of per-token boundaries.
~40-60% positive window rate vs 7.93% token-level → much more tractable.

Returns both window predictions AND per-token hidden states for downstream caching.
"""
...
class WindowBoundaryHead(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor, attention_mask: torch.Tensor):
        ...
        Returns:
            window_logits: (batch, n_windows)
            window_mask: (batch, n_windows) - 1 for valid windows, 0 for padding
        """
        ...
        if n_windows <= 0:
            n_windows = 1  # At least one window for short sequences
        ...
        for start in range(0, seq_len, self.stride):
            ...
            # Extract window hidden states
            window_hidden = hidden_states[:, start:end, :]  # (batch, window_len, hidden_dim)
            window_attn = attention_mask[:, start:end]  # (batch, window_len)

            # Mean pool over valid tokens in window
            mask = window_attn.unsqueeze(-1).float()  # (batch, window_len, 1)
            pooled = (window_hidden * mask).sum(dim=1) / mask.sum(dim=1).clamp(min=1)  # (batch, hidden_dim)

            # MLP to get logit
            logit = self.mlp(pooled).squeeze(-1)  # (batch,)
            window_logits_list.append(logit)

            # Window is valid if it has any valid tokens
            valid = (window_attn.sum(dim=1) > 0).float()  # (batch,)
        ...
        window_logits = torch.stack(window_logits_list, dim=1)  # (batch, n_windows)
        window_mask = torch.stack(window_mask_list, dim=1)  # (batch, n_windows)
...
class C1ACoarseModel(nn.Module):
    ...
    def forward(self, input_ids: torch.Tensor, attention_mask: torch.Tensor):
        """
        Returns:
            window_logits: (batch, n_windows) - boundary predictions
            hidden_states: (batch, seq_len, hidden_dim) - for downstream caching
            window_mask: (batch, n_windows) - valid window mask
        """
...
class C1ACoarseDataset(Dataset):
    """Dataset for coarse boundary window detection."""
    ...
    def __init__(self, data_file: str, tokenizer, max_length: int = 512,
        ...
        for idx in self.indices[:500]:  # Sample for stats
            r = all_records[idx]
    ...
    def __getitem__(self, idx):
        ...
        # Scale boundary positions to tokenized length
        ...
        # Generate window labels for the PADDED max_length (consistent across batch)
        # Use max_length for window calculation to ensure consistent tensor sizes
        ...
        # Create window mask: 1 for windows within actual sequence, 0 for padding
        window_mask = []
        for i, start in enumerate(range(0, self.max_length, self.stride)):
            end = min(start + self.window_size, self.max_length)
            if end - start < self.window_size // 2:
                break
            # Window is valid if its start is within actual sequence
...
def compute_metrics(window_logits, window_labels, window_mask, n_boundaries, has_boundaries,
    ...
    for i in range(batch_size):
        ...
        if has_boundaries[i]:
            # Positive problem - check if we found all boundary windows
            target_positive_idx = targets_i.nonzero(as_tuple=True)[0]
            pred_positive_idx = preds_i.nonzero(as_tuple=True)[0]

            if len(target_positive_idx) > 0:
                # All-correct: all true positive windows predicted
                found_all = all(preds_i[idx] for idx in target_positive_idx)
                if found_all:
                    all_correct += 1

                # Any-correct: at least one true positive window predicted
                found_any = any(preds_i[idx] for idx in target_positive_idx)
                if found_any:
                    any_correct += 1
        else:
            # Negative problem - check if predicted all zeros
            true_neg_total += 1
            if not preds_i.any():
                true_neg_correct += 1
...
def main():
    ...
    # Datasets
    train_dataset = C1ACoarseDataset(
        args.data_file, tokenizer, args.max_length,
        args.window_size, args.stride, split='train'
    )
    val_dataset = C1ACoarseDataset(
        args.data_file, tokenizer, args.max_length,
    ...
    print(f"\nStarting C1-A Coarse training: {len(train_dataset)} train, {len(val_dataset)} val")
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for batch_idx, batch in enumerate(tqdm(train_loader, desc=f"Epoch {epoch}")):
            ...
            window_logits, hidden_states, window_mask_pred = model(input_ids, attention_mask)
    ...
    with open(output_path / 'window_analysis.json', 'w') as f:
        json.dump(window_analysis, f, indent=2)
