"""
C1-A Coarse Boundary Window Detection v6 with Auxiliary Telegraph Prediction

Same as baseline v6 EXCEPT:
- Adds auxiliary per-token telegraph prediction head
- Combined loss: 1.0 * boundary_loss + 0.3 * telegraph_loss
- Early stopping on boundary F1 only (auxiliary is just for representation learning)
"""
...
class TelegraphHead(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor):
        ...
        Returns:
            telegraph_preds: [batch, seq_len] - predicted telegraph value per token
        """
        return self.mlp(hidden_states).squeeze(-1)


class C1AAuxModel(nn.Module):
    """C1-A with coarse boundary window detection + auxiliary telegraph prediction."""
    ...
    def forward(self, input_ids: torch.Tensor, attention_mask: torch.Tensor):
        ...
        # Primary task: window boundary prediction
        window_logits, window_mask = self.window_head(hidden_states, attention_mask)

        # Auxiliary task: per-token telegraph prediction
        telegraph_preds = self.telegraph_head(hidden_states)

        return window_logits, telegraph_preds, hidden_states, window_mask
...
class C1AAuxDataset(Dataset):
    """Dataset for coarse boundary window detection with telegraph targets."""
    ...
    def __getitem__(self, idx):
        ...
        # Load telegraph targets from hints cache
        # Telegraph is index 2 in the 4-feature hint vectors
        ...
        if problem_key in self.hints_cache:
            ...
            # Extract telegraph (index 2) and scale to tokenized length
            if hint_n_tokens > 0 and actual_len > 0:
                ...
                for pos, hint_vec in enumerate(hint_vectors):
                    if pos < hint_n_tokens and len(hint_vec) > 2:
                        telegraph_val = hint_vec[2]  # Telegraph is index 2
                        scaled_pos = min(int(pos * scale), actual_len - 1)
...
def compute_telegraph_metrics(telegraph_preds, telegraph_targets, attention_mask):
    """Compute auxiliary telegraph prediction metrics."""
    valid_mask = attention_mask.bool()

    preds_flat = telegraph_preds[valid_mask]
    targets_flat = telegraph_targets[valid_mask]

    # MAE
    mae = (preds_flat - targets_flat).abs().mean().item()
    ...
    denominator = (preds_centered.pow(2).sum().sqrt() * targets_centered.pow(2).sum().sqrt()).clamp(min=1e-8)
    correlation = (numerator / denominator).item()

    return {
        'telegraph_mae': mae,
        'telegraph_corr': correlation,
    }
...
def main():
    ...
    parser.add_argument('--hints-file', type=str, required=True,
                        help='Path to cached hints JSON with telegraph targets')
    ...
    parser.add_argument('--aux-weight', type=float, default=0.3,
                        help='Weight for auxiliary telegraph loss')
    ...
    # Datasets
    train_dataset = C1AAuxDataset(
        args.data_file, tokenizer, hints_cache, args.max_length,
        args.window_size, args.stride, split='train',
        apply_latex_preprocessing=apply_preprocessing
    )
    val_dataset = C1AAuxDataset(
        args.data_file, tokenizer, hints_cache, args.max_length,
    ...
    best_f1_07 = 0.0
    patience_counter = 0
    baseline_f1 = 0.714  # Baseline to compare against

    print(f"\nStarting C1-A v6 + Auxiliary Telegraph training")
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for batch_idx, batch in enumerate(tqdm(train_loader, desc=f"Epoch {epoch}")):
            ...
            window_logits, telegraph_preds, hidden_states, window_mask_pred = model(input_ids, attention_mask)

            # Align window dimensions
            ...
            # Auxiliary loss: telegraph MSE (per-token)
            ...
            # Combined loss
            loss = boundary_loss + args.aux_weight * telegraph_loss
        ...
        with torch.no_grad():
            for batch in tqdm(val_loader, desc="Validating"):
                ...
                telegraph_metrics = compute_telegraph_metrics(
                    telegraph_preds, telegraph_targets, attention_mask
                )
                all_telegraph_metrics.append(telegraph_metrics)
        ...
        avg_telegraph_metrics = {}
        for key in all_telegraph_metrics[0].keys():
            avg_telegraph_metrics[key] = np.mean([m[key] for m in all_telegraph_metrics])

        # Log
        print(f"\n{'='*70}")
        print(f"Epoch {epoch}")
        print(f"{'='*70}")
        print(f"Train Loss: {total_loss/n_batches:.4f} (total) | Boundary: {total_boundary_loss/n_batches:.4f} | Telegraph: {total_telegraph_loss/n_batches:.4f}")
        print(f"Val Loss: {val_loss:.4f} (boundary only)")
        ...
        print()
        print(f"Auxiliary telegraph MAE: {avg_telegraph_metrics['telegraph_mae']:.4f} | Pearson correlation: {avg_telegraph_metrics['telegraph_corr']:.3f}")
        print()
        delta = avg_metrics['f1'] - baseline_f1
        print(f"[vs Baseline: F1@0.5 delta = {'+' if delta >= 0 else ''}{delta:.4f}]")
        ...
        epoch_log = {
            ...
            'train_telegraph_loss': total_telegraph_loss / n_batches,
            'val_loss': val_loss,
            **avg_metrics,
            **avg_telegraph_metrics,
        }
        ...
        # Early stopping on F1@0.5 (boundary task only)
    ...
    print(f"\nC1-A v6 + Auxiliary Telegraph Complete!")
    ...
    print(f"Best telegraph correlation: {best_epoch['telegraph_corr']:.3f}")
    print()
    delta = best_f1 - baseline_f1
    print(f"[vs Baseline 0.714: F1@0.5 delta = {'+' if delta >= 0 else ''}{delta:.4f}]")
    ...
    comparison = {
        ...
        'best_telegraph_corr': best_epoch['telegraph_corr'],
        'hint_experiments': {
            'c0_hints_4_features': 0.725,
            'gt_hints_4_features': 0.727,
            'telegraph_only': 0.712,
        },
        'conclusion': 'meaningful_improvement' if best_f1 > 0.74 else 'marginal_or_none',
    }
