...
Raw spans carry lexical noise that prevents clustering. We reduce vocabulary while preserving operational integrity for the downstream DSL.

**The constraint:** every transformation must preserve the information the SubGraphDSL needs — the operation structure, the number of parameters, and the relationship between entities.

**Approach:**
1. **Entity replacement** — Names → `[PERSON]`, objects → `[ITEM]`, numbers → `[N]`, locations/times stripped
2. **Operational summarization** — Qwen summarizes the span into its operational core while preserving verb semantics
3. **Vocabulary collapse** — After summarization, spans that perform the same operation cluster naturally at high cosine similarity
