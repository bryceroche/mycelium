...
- **v106 neural-guided digit search — CODE READY, RUN DEFERRED (May 31):** BP and search solve different uncertainties — BP (breathing) narrows soft marginals continuously; search commits to specific digits when BP is uncertain. BP tells search WHERE to look (highest-entropy positions), search tells BP WHAT to assume (clamped digit value). Calibration-gated trigger (calib > 0.7 → skip search for easy problems). The digit codebook IS the search tree: each path from root = a sequence of clamped (var, position) → digit_value commitments, branching factor 10, BP scoring at every leaf. **Architectural insight:** one BP forward pass produces BOTH a policy (digit_logits.softmax → child priors) AND a value-like signal (calibration → state score), so PUCT [Silver+ 2017, the AlphaZero search algorithm] is the right search method — but we are NOT AlphaZero (no self-play, no adversary, no game outcomes). This is neural-guided combinatorial search [NeuroSAT-line] using PUCT as the search algorithm. Evaluation order: A (BP only) → B (greedy refinement) → D (PUCT). Files: `mycelium/factor_graph_v106.py` (563 lines, currently UCT-uniform; 5-line PUCT upgrade pending). **DO NOT RUN** until v105 BP produces useful per-position digit distributions — search amplification is multiplicative on BP quality. Full design + paper-careful framing: `memory/project_v106_mcts_design.md`.
- **v105.1.2 v2 prod on log-uniform data — IN FLIGHT (May 31):** isolated test of "is signal starvation at positions 0-2 the cause of slow upper-position learning?" Stage 1 (single-digit pretrain) failed acceptance (16.3% vs 70% target — K=8 breaths overkill for single-op problems). Stage 2 succeeded: regenerated 50k+5k factor graphs with log-uniform [1, 9999] values, all positions now have ≥18% coverage (vs ~0% on old [0,99] data). Stage 3 running from v104 warm-start, 5000 steps, ETA ~70 min remaining at the time of writing. Once converged, per-position diagnostic + v106 MCTS validation.
...
- **ODE integrator framing (May 30 — deepest mathematical identity):** the breathing transformer IS a learned ODE integrator solving dx/dt = -∇E(x) for energy descent on factor graphs. 4 transformer layers per breath = 4 RK4 stages (each layer computes one gradient estimate, residual stream accumulates them); K breaths = K integration steps; delta_gate = adaptive step size; calibration head = Dopri5-style error estimator for adaptive K. Foundation: attention IS one Hopfield energy descent step (Ramsauer 2020) — training aligns the implicit Hopfield energy with the explicit factor graph energy. Three vocabularies (CS: ODE integrator / physics: energy descent / ML: approximate BP) for the same mathematical object. See `memory/project_ode_integrator_framing.md`.
- **Two-phase architecture (May 30):** comprehension and inference are different computational regimes that need different model sizes. **Phase 1 (NL → factor graph)** = language task, one-shot, large model (Haiku/Sonnet/1B+ classifier). **Phase 2 (factor graph → answer)** = computational task, iterative K-breath ODE integration, small model (87M breathing transformer). The 410M GSM8K failure was Phase 1 being undersized; v98 Sudoku worked because Phase 1 is trivial (grid IS factor graph) and the architecture is sized correctly for Phase 2. The breathing transformer IS Phase 2. Building Phase 1 (NL parser) is the path to GSM8K, not redesigning the inference engine.
