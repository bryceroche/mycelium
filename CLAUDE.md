...
- **v59 K=4 on L4.7 + PERFECT decomposition (May 20):** v58 wider controller (CONTROLLER_N_LAYERS=2) + 1500 more steps from v58 step 750. v59 step 1500 segmented eval = **45% on L4.7** (per-step accuracy ~78%, **breaking the 4-layer arithmetic 65% ceiling**). Sample generations show PERFECT step decomposition — every step uses the previous step's output correctly, structure is right, remaining errors are arithmetic execution. Bottleneck shifted from orchestration → arithmetic. Ckpt: `.cache/l4_7_ckpts/v59_continue_l4_7_step1500.safetensors`. Bridge to GSM8K.
- **Eval perf wins (May 20-21):** Three-phase rollout, total 7.8× speedup on v59 segmented eval (752s → 96.5s):
  - **Phase 1 (May 20):** controller cross-attn at T_q=1 instead of T_q=T → **4×** (752s → 188s). Byte-exact parity. Default ON.
  - **Phase 2 parity fix (May 20):** `alpha` + `temp_mult` plumbed through `forward_cached_step_batched`. Fixes silent bugs shared with `cached_generate_batch` (per-head pitch + SINE_TEMP were being dropped from the cached step). v45's standard eval has been subtly off because of this; v55+ models would diverge more.
  - **Phase 3 closure-based fused JIT (May 21, Sonnet):** `cached_generate_segmented` rewritten with two closure JITs (Stage 1 prefill + Stage 2 fused step). 1.95× over Phase 1 (188s → 96.5s). 7.8× from original. Activated via `USE_KV_CACHE=1`. Root bug found: at `cnt=0` (first JIT call = eager), `.assign()` on closure buffers is silently dropped unless the assigned tensor is in the return tuple. See `memory/project_kv_cache_segmented_wip.md` for the full deep-dive.
