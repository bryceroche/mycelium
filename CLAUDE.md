...
- **v99 factor graph paradigm — IN FLIGHT (May 29):** synthetic arithmetic factor graph generator built (`scripts/build_factor_graph_data.py`, 50K train + 5K test at `.cache/factor_graph_*.jsonl`). Architecture build delegated to Sonnet — bipartite factor graph encoding (variable nodes + factor nodes), per-problem dynamic attention masks (factors connect variable triples), 100-way domain codebook, moment-matching constraint energy (mean + variance for add/sub; mean-only for mul; rearranged exact for div), soft head specialization (factor type embedding added to K/V on edges, no hardwired head-to-op assignment). Same Pythia-410M L0-L3 backbone, same K=20 breaths. **Prediction to test:** convergence rate ∝ DAG depth (one breath = one message-passing hop). If energy decay appears with depth-proportional convergence, BP framing is empirically validated as a general substrate. Then v100 = Haiku-parsed GSM8K factor graphs as the bridge to NL math.
