...
## 3. Empirical Status (as of 2026-05-27)

**Two paradigms in production, plus v81 — the architecturally aligned coarse-to-fine paradigm.**
...
- **v81 paradigm (current, May 27): same task per breath at different granularity.** Architectural insight: the 4 transformer layers are SHARED across K=7 breaths. If each breath has a different task, the 7-fold gradient pulls weights in different directions. v81 makes every breath do the SAME job — coarse-to-fine refinement of a multi-list DAG representation. (1) IB clustering on Pythia embeddings of GSM8K L2 step descriptions yields a 32-leaf cluster tree (4 ops × 8 sub-clusters avg). (2) Each breath emits 4 parallel lists — ops, types_path, args1, args2 — separated by " | "; each breath fills/refines ONE list more than the previous (B0 skeleton → B6 fully executable). (3) Multi-head WaistController (4 heads, one per list) decoded at the final breath; earlier breaths use legacy single-head. (4) FOUR masks required for clean training (kv_mask + notebook_pool_mask + main_attn_mask + embed_mask — see `memory/project_v81_four_mask_discovery.md`). v81 prod fires now. Data file: `.cache/gsm8k_steps_v81_train.jsonl` (3852 records, 99.4% B6 SymPy-parseable, 95.1% gold-match).
...
- **v81 firing (May 27):** prod training STEPS=2000 with full 4-mask + multi-head WaistController + IB-tree-aligned multi-list supervision. Step 10 pb_ce=[4.01, 3.90, 3.97, 3.85, 3.69, 4.27, 5.00] — monotonic ascent as designed. Perf wins applied: BATCH=4, FIXED_LEN=256 (was BATCH=2, FIXED_LEN=320; v81 sequences max 249 tokens so 256 fits). ETA ~5h.
