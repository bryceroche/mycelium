...
- **v106 MCTS-on-digit-codebook — CODE READY, RUN DEFERRED (May 31):** BP and MCTS solve different uncertainties — BP (breathing) narrows soft marginals continuously; MCTS commits to specific digits when BP is uncertain. BP tells MCTS WHERE to look (highest-entropy positions), MCTS tells BP WHAT to assume (clamped digit value). Entropy-driven expansion + calibration-gated trigger (calib > 0.7 → skip MCTS for easy problems). The digit codebook IS the search tree: each path from root = a sequence of clamped (var, position) → digit_value commitments, branching factor 10, BP scoring at every leaf. Architecturally the Bombe analog: BP propagates constraints, MCTS searches uncertain settings, contradictions surface as energy spikes. Files: `mycelium/factor_graph_v106.py` (563 lines), `scripts/v106_mcts_eval.py`, `scripts/v106_mcts_smoke.sh`. **DO NOT RUN** until v105 BP produces useful per-position digit distributions — MCTS amplification is multiplicative on BP quality. Full design: `memory/project_v106_mcts_design.md`.
