...
## System Independence
We have a periodic tree review that restructures the tree -- that is okay.  But we need to resist the urge to manually intervene in the tree.  We want the system to be independent.  This means only modifying python code and database schema but not the tree.  we want the mycelium system to be able to restructure the tree -- we just dont want to be tempted to make those adjustments ourselves.   but we shouldn't be going into the tree structure and cleaning up orphaned umbrealla routers ourselves for example  
