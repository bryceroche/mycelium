        ...
        if self._client is None:
            async with self._client_lock:
                if self._client is None:
                    ...
                    logger.debug(
                        f"[openai] Created connection pool: "
                        f"max_conn={self._max_connections} keepalive={self._max_keepalive}"
                    )
        return self._client
    ...
    async def generate(
        ...
        for attempt in range(MAX_RETRIES + 1):
            try:
                ...
                usage = data.get("usage", {})
                logger.debug(
                    f"[openai] tokens: prompt={usage.get('prompt_tokens', 0)} "
                    f"completion={usage.get('completion_tokens', 0)}"
                )
                choices = data.get("choices")
                if not choices or not isinstance(choices, list) or len(choices) == 0:
                    raise ValueError("Invalid API response: missing or empty 'choices' array")
                message = choices[0].get("message")
                if not message or not isinstance(message, dict):
                ...
                if attempt < MAX_RETRIES:
                    delay = self._calculate_backoff(attempt, e.response)
                    logger.warning(
                        f"[openai] Retry {attempt + 1}/{MAX_RETRIES} after {e.response.status_code}, "
                        f"waiting {delay:.1f}s"
                    )
                    await asyncio.sleep(delay)
                ...
                if attempt < MAX_RETRIES:
                    delay = self._calculate_backoff(attempt)
                    logger.warning(
                        f"[openai] Retry {attempt + 1}/{MAX_RETRIES} after {type(e).__name__}, "
                        f"waiting {delay:.1f}s"
                    )
                    await asyncio.sleep(delay)

        logger.error(f"[openai] All {MAX_RETRIES} retries exhausted")
        if last_exception:
            raise last_exception
        raise RuntimeError("Unexpected retry loop exit")
...
async def ask_llama(
    ...
    """Simple interface to ask the LLM a question.
