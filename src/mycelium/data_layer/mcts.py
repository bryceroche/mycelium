...
def get_tree_health_metrics() -> dict:
    """Get metrics about tree structure health.

    Per CLAUDE.md: "healthy tree would have ~5:1 ratio where each router
    has roughly five children."

    Returns:
        Dict with tree health metrics:
        - total_nodes: Total signatures
        - routers: Number of umbrella signatures
        - leaves: Number of leaf signatures
        - atomic_count: Number of atomic signatures
        - avg_children_per_router: Average children per router
        - router_to_children_ratio: Ratio metric (target ~5:1)
        - depth_distribution: Count of nodes at each depth
    """
    conn = get_db()
    metrics = {
        "total_nodes": 0,
        ...
        "leaves": 0,
        "atomic_count": 0,
        "avg_children_per_router": 0.0,
        "router_to_children_ratio": 0.0,
        "depth_distribution": {},
    }
    ...
    with conn.connection() as c:
        ...
        cursor = c.execute("""
            SELECT
                COUNT(*) as total,
                SUM(CASE WHEN is_semantic_umbrella = 1 THEN 1 ELSE 0 END) as routers,
                SUM(CASE WHEN is_semantic_umbrella = 0 THEN 1 ELSE 0 END) as leaves,
                SUM(CASE WHEN is_atomic = 1 THEN 1 ELSE 0 END) as atomic
            FROM step_signatures
            WHERE is_archived = 0
        """)
        row = cursor.fetchone()
        if row:
            metrics["total_nodes"] = row[0] or 0
            metrics["routers"] = row[1] or 0
            metrics["leaves"] = row[2] or 0
            metrics["atomic_count"] = row[3] or 0
        ...
        """)
        for row in cursor.fetchall():
            metrics["depth_distribution"][row[0]] = row[1]

        # Children per router
        cursor = c.execute("""
            SELECT sr.parent_id, COUNT(*) as child_count
            FROM signature_relationships sr
            JOIN step_signatures ss ON sr.parent_id = ss.id
            WHERE ss.is_archived = 0
            GROUP BY sr.parent_id
        """)
        child_counts = [row[1] for row in cursor.fetchall()]

        if child_counts:
            metrics["avg_children_per_router"] = sum(child_counts) / len(child_counts)
            # Ratio: avg children / 1 (we want ~5 children per router)
