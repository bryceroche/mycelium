...
def attach_fg_params_v200(
    ...
    if hasattr(model, "fg_v200_latents"):
        ...
        # #301: same backfill treatment for the estimator gate (zero-init,
        # deterministic; missing on pre-#301-attached models)
        if not hasattr(model, "fg_v200_cm_gate"):
            model.fg_v200_cm_gate = Tensor(np.zeros((1,), dtype=np.float32)).contiguous().realize()
            print("[v200] params already attached; cm_gate was missing — attached (gate=0)", flush=True)
        # v300-2w: backfill the odd-breath waist scale (zero-init; missing on
        # pre-v300-2w models). Only meaningful when the waist exists.
        if hasattr(model, "fg_v200_waist_gate") and not hasattr(model, "fg_v200_waist_odd_scale"):
            model.fg_v200_waist_odd_scale = Tensor(np.zeros((1,), dtype=np.float32)).contiguous().realize()
            print("[v200] params already attached; waist_odd_scale was missing — attached (0)", flush=True)
        # v300-2: backfill the expand/compress LoRA (zero-init B; missing on
        # pre-#v300-2-attached models)
        if not hasattr(model, "fg_v200_lora_names"):
            _bc = model.llama_cfg; _bH = _bc.hidden_size; _bf = _bc.intermediate_size; _br = 16
            _brng = np.random.RandomState(1300)
            model.fg_v200_lora_names = []
            for _li in range(len(model.llama_layers)):
                for _proj, _din, _dout in (("wq", _bH, _bH), ("wo", _bH, _bH), ("wdown", _bf, _bH)):
                    nA = f"fg_v200_lora_L{_li}_{_proj}_A"; nB = f"fg_v200_lora_L{_li}_{_proj}_B"
                    setattr(model, nA, Tensor((_brng.randn(_din, _br).astype(np.float32)/np.sqrt(_din)), dtype=dtypes.float).contiguous().realize())
                    setattr(model, nB, Tensor(np.zeros((_br, _dout), dtype=np.float32), dtype=dtypes.float).contiguous().realize())
                    model.fg_v200_lora_names += [nA, nB]
            print("[v200] params already attached; expand/compress LoRA was missing — attached (B=0)", flush=True)
        else:
    ...
    # ---- Explicit common-mode estimator α_cm (v300-1, kickoff §3) ----
    # Composition 6+7: estimate the null explicitly, remove it at read time.
    # z_j ← z_j − α_cm·mean_j(z) at the Seam-1 site, in-graph. ZERO-init per
    # §1A.E.8 reasoning: auxiliary REMOVAL on the workspace — the architecture
    # is complete without it (ran 4,000 steps without it), so zero-init is
    # correct where it was fatal on READ. α_cm=0 → subtraction contributes
    # exactly 0.0 → step-0 forward identical to flag-off (step-0 gate i).
    model.fg_v200_cm_gate        = Tensor(np.zeros((1,), dtype=np.float32)).contiguous().realize()

    # ---- Expand/compress phase-toggled LoRA (v300-2, Jun 13) ----
    # Rank-16 LoRA on each Llama L0-L3 layer's wq (routing) + wo (attn
    # write-back) + w_down (FFN write-back). Toggled ON only on COMPRESS
    # (odd) breaths. Adapted proj = h@W + (h@A)@B; A small-random, B ZERO-init
    # => zero delta at step 0 (compress breath byte-identical to expand =>
    # step-0 identity to the blur). The single variable of the v300-2 build.
    _lora_cfg = model.llama_cfg
    _Hd = _lora_cfg.hidden_size
    _ffn = _lora_cfg.intermediate_size
    _LORA_RANK = 16
    _rng_lora = np.random.RandomState(1300)
    def _lora_A(d_in):  # small-random A (kaiming-ish), B zero
        return (_rng_lora.randn(d_in, _LORA_RANK).astype(np.float32) / np.sqrt(d_in))
    def _lora_B(d_out):
        return np.zeros((_LORA_RANK, d_out), dtype=np.float32)
    model.fg_v200_lora_names = []
    for _li in range(len(model.llama_layers)):
        for _proj, _din, _dout in (("wq", _Hd, _Hd), ("wo", _Hd, _Hd), ("wdown", _ffn, _Hd)):
            nA = f"fg_v200_lora_L{_li}_{_proj}_A"; nB = f"fg_v200_lora_L{_li}_{_proj}_B"
            setattr(model, nA, Tensor(_lora_A(_din), dtype=dtypes.float).contiguous().realize())
            setattr(model, nB, Tensor(_lora_B(_dout), dtype=dtypes.float).contiguous().realize())
            model.fg_v200_lora_names += [nA, nB]
    ...
    if stage2a_waist:
        ...
        # v300-2w: ZERO-init scalar scaling the ODD-breath waist contribution
        # (the structural-cycle single variable; 0 => exact step-0 identity to
        # #238 waist-even-only; grows iff compress-every-breath is wanted).
        model.fg_v200_waist_odd_scale = Tensor(
            np.zeros((1,), dtype=np.float32)
        ).contiguous().realize()
...
def fg_v200_parameters(model: Any) -> list[Tensor]:
    ...
    v200_attrs = [
        ...
        "fg_v200_nb_gate",
        # Explicit common-mode estimator (#301, v300 brief §2)
        "fg_v200_cm_gate",
        # Stage 2a waist (only present when V200_STAGE2A_WAIST=1)
        "fg_v200_W_compress",
        "fg_v200_W_expand",
        "fg_v200_waist_gate",
        "fg_v200_waist_odd_scale",
    ...
    # v300-2 expand/compress LoRA params (filtered out by the driver when the
    # toggle is off — same None-grad discipline as cm_gate on the cont arm)
    for nm in getattr(model, "fg_v200_lora_names", []):
        params.append(getattr(model, nm))
...
def fg_v200_state_dict(model: Any) -> dict[str, Tensor]:
    ...
    # v300-2 expand/compress LoRA params
    for nm in getattr(model, "fg_v200_lora_names", []):
...
def fg_v200_lora_params(model: Any) -> list:
    """The v300-2 expand/compress LoRA tensors (for the optimizer-exclude
    pattern: the cont/blur arm filters these out to avoid None-grad)."""
    return [getattr(model, nm) for nm in getattr(model, "fg_v200_lora_names", [])]


def fg_v200_lora_dicts(model: Any) -> list:
    """Per-layer LoRA dicts {wq_A,wq_B,wo_A,wo_B,wdown_A,wdown_B} for the
    LlamaLayer lora= hook. Returns one dict per L0-L3 layer, or None if
    the LoRA params are absent."""
    if not getattr(model, "fg_v200_lora_names", None):
        return None
    dicts = []
    for li in range(len(model.llama_layers)):
        d = {}
        for proj in ("wq", "wo", "wdown"):
            d[f"{proj}_A"] = getattr(model, f"fg_v200_lora_L{li}_{proj}_A")
            d[f"{proj}_B"] = getattr(model, f"fg_v200_lora_L{li}_{proj}_B")
        dicts.append(d)
    return dicts
...
def fg_breathing_forward_v200(
    ...
    estimator_cm: bool = False,
    estimator_readback: bool = False,
    notebook_absent: bool = False,
    latent_noise_sigma: float = 0.0,
    expand_compress_lora: bool = False,
    expand_compress_cycle: bool = False,
) -> tuple[list[Tensor], list[Tensor]]:
    """v200 perceiver-CORE forward pass.

    expand_compress_cycle (v300-2w, Jun 13): the STRUCTURAL DIMENSIONALITY
    cycle — the waist (compress operator) fires on ODD breaths too (every
    breath = compress every cycle), via a ZERO-INIT scalar
    fg_v200_waist_odd_scale on the odd-breath waist contribution. Exact
    step-0 identity to #238 (waist-even-only) at scale=0; the scalar grows
    iff the gradient wants compress-every-breath (HILL-STANDS) vs stays ~0
    (COSMETIC). The single variable formalizing the natural L1->L2 seam the
    per-layer probe found; #238 (cycle off) is the blur control.

    expand_compress_lora (v300-2 Piece A, Jun 13): phase-toggled LoRA on the
    Llama L0-L3 layers (wq/wo/w_down, rank 16, zero-init B). ON only on
    COMPRESS breaths, OFF on EXPAND breaths — the single variable testing
    whether expand and compress want different transforms (HILL-STANDS) or
    the same one (COSMETIC). ADVISORY (sharpened on contact, invited by
    Bryce): the build doc labels even=expand/odd=compress, but the existing
    Stage 2a waist (the quantizer) fires on EVEN breaths — so COMPRESS is
    aligned to the WAIST (compress = even, k%2==0, LoRA ON + waist), EXPAND =
    odd (plain Llama). This unifies the collapse operator (waist+LoRA on one
    breath) and keeps the P2 silhouette read unconfounded; the doc's literal
    parity is inverted relative to the existing waist convention.

    estimator_readback (v300-1.1, brief §2 v300-1.1 form — the principled
    next run after v300-1's Seam-1 form deformed the GLOBAL family): the
    ORIGINAL pre-registration form, brought back as a distinct flag.
    μ_k = running mean over breaths 0..k-1 of pool(z_settled) (the same
    pooled settled state the notebook writes), DETACHED at accumulation
    (no gradient route — isolates the estimator's FUNCTION from the
    teacher's backward path), subtracted at the READ-BACK SITE
    (immediately after the notebook read-back, k≥1): z ← z − α_cm·μ_k.
    Shares fg_v200_cm_gate (α_cm) with estimator_cm. Expected sign:
    POSITIVE (at the read-back site, removal is removal — the Seam-1
    sign-reversal was a site pathology, v300-1 READ A/B). MUTUALLY
    EXCLUSIVE with estimator_cm (one variable per run).

    latent_noise_sigma (Z0-b MC-BP, v300 brief §5 — EVAL/EAGER ONLY, must
    be 0.0 in JIT paths): Gaussian noise added to the latent state at the
    START of each breath (before Seam-1), mirroring the v110-era MC-BP
    protocol's "noise on x at start of breath" injection point.

    estimator_cm (v300-1, kickoff §3): explicit common-mode estimator —
    per-breath spatial mean m_k = mean_j(z_k,j) over all latents, IN-GRAPH,
    subtracted immediately after the Seam-1 boundary norm (before the
    pre-breath capture, so the gate blend's anchor sees the centered
    state): z_j ← z_j − α_cm·m_k, every breath including k=0. α_cm
    (fg_v200_cm_gate) zero-init ⇒ step-0 forward exactly equals flag-off
    (step-0 gate i). The running-mean-over-breaths / detached / read-back-
    site variant is the pinned v300-1.1 ablation (v300 brief §2).

    notebook_absent (E.17, v300 brief §1 — EVAL ONLY): remove the WRITE
    operator from the graph entirely (no slot writes, no read-back, no
    pooling). Distinct from gate-0, which leaves the operator in-graph
    multiplied by 0.0; the difference between the two isolates
    graph-shape effects (tripwire 9).
    ...
    Stages per breath k:
      ...
      0.5. ESTIMATOR (v300-1, when estimator_cm): α_cm·mean_j(z) subtracted
         at the Seam-1 site, before the pre-breath capture.
      5. GATE: delta_gate convex blend of pre-breath and post-THINK state;
         then WRITE (#238): settled state pooled → notebook slot k
         (skipped under notebook_absent).
    ...
    the forward fills it in place:
      ...
      "wb_post_norm"    : K × Tensor (B, L, H) — post Seam-1 norm_breath   (§1A.E.9 ckpt 1;
                          under estimator_cm this is post-centering — the
                          estimator lives at the Seam-1 site, ADVISORY noted)
    ...
    cm_gate         = getattr(model, "fg_v200_cm_gate", None)   # (1,) ZERO-init (#301 estimator)
    assert not (estimator_cm and estimator_readback), \
        "estimator_cm (Seam-1) and estimator_readback (v300-1.1) are mutually exclusive — one variable per run"
    ...
    if stage2a_waist:
        ...
        waist_odd_scale = getattr(model, "fg_v200_waist_odd_scale", None)  # (1,) ZERO-init (v300-2w)
    ...
    cm_run_sum = None     # (v300-1.1) detached running sum of pooled settled state, (B, H)
    _lora_dicts_local = fg_v200_lora_dicts(model) if expand_compress_lora else None  # v300-2

    for k in range(K):
        # Z0-b MC-BP noise injection (eval/eager only): start of breath,
        # before Seam-1 — the v110 protocol's injection point.
        if latent_noise_sigma > 0.0:
            latents = latents + Tensor.randn(*latents.shape, dtype=latents.dtype) * latent_noise_sigma
        ...
        # Explicit common-mode estimator (v300-1, kickoff §3): subtract the
        # gated per-breath spatial mean at the Seam-1 site — the same site the
        # carrier-projection ablations tested (method identity with the
        # evidence that motivated this). In-graph; α_cm is the only new
        # parameter; applied BEFORE the pre-breath capture so the whole
        # breath, gate-blend anchor included, sees the centered state.
        if estimator_cm:
            assert cm_gate is not None, \
                "estimator_cm=True but fg_v200_cm_gate missing — re-run attach_fg_params_v200 (backfill attaches it)"
            m_k = latents.mean(axis=1, keepdim=True)   # (B, 1, H)
            latents = latents - cm_gate.cast(latents.dtype).reshape(1, 1, 1) * m_k
            if taps is not None:
                taps["cm_mu"].append(m_k.cast(dtypes.float).realize())
        ...
        if k >= 1 and not notebook_absent:
        ...
        # Explicit common-mode estimator, v300-1.1 READ-BACK form: subtract the
        # DETACHED running-mean reference at the read-back site (where the carrier
        # evidence + the v300-1 site-conflation post-mortem both pointed). μ_k is
        # detached ⇒ no gradient route; α_cm is the only learned quantity.
        # Expected-positive sign. k≥1 (μ needs ≥1 prior breath).
        if estimator_readback and k >= 1 and cm_run_sum is not None:
            assert cm_gate is not None, \
                "estimator_readback=True but fg_v200_cm_gate missing — re-run attach_fg_params_v200"
            mu_k = (cm_run_sum / float(k)).reshape(-1, 1, H).cast(latents.dtype)   # mean of breaths 0..k-1
            latents = latents - cm_gate.cast(latents.dtype).reshape(1, 1, 1) * mu_k
            if taps is not None:
                taps["cm_mu"].append(mu_k.cast(dtypes.float).realize())
        ...
        # v300-2: COMPRESS breaths (even, aligned with the waist) get the LoRA;
        # EXPAND breaths (odd) run plain Llama. lora_on => per-layer lora dicts.
        lora_on = expand_compress_lora and (k % 2 == 0)
        lora_dicts = _lora_dicts_local if lora_on else None
        ...
        elif stage2a_waist and expand_compress_cycle and (k % 2 == 1):
            # v300-2w STRUCTURAL CYCLE: compress on ODD breaths too (the waist
            # fires EVERY breath = compress every cycle). The single variable.
            # waist_odd_scale ZERO-INIT scalar => exact step-0 identity to #238
            # (waist-even-only); grows iff the gradient wants compress-every-breath
            # (HILL-STANDS) vs stays ~0 (COSMETIC, even-only was enough). Same
            # W_compress/W_expand/gate as even — the SAME compress transform,
            # just applied on odd breaths and scaled by the new gate.
            latents_w = _rms_norm(latents, commit_norm_w, rms_eps).cast(latents.dtype)
            w_full = _apply_waist_v200(latents_w, W_compress, W_expand, waist_gate)
            # interpolate ORIGINAL latents -> full even-style waist by the zero-init
            # scale: scale=0 leaves latents untouched (exact #238 identity), scale=1
            # = a full even-breath waist. (subtract the ORIGINAL latents, not the norm)
            latents = latents + waist_odd_scale.cast(latents.dtype).reshape(1, 1, 1) * (w_full - latents)
            if taps is not None:
                taps["waist_contrib"].append((w_full - latents_w).cast(dtypes.float).realize())
                taps["waist_breaths"].append(k)
        ...
        # Estimator running-mean accumulation (v300-1.1): the SAME pooled
        # settled state the notebook writes, DETACHED — μ adds no gradient
        # route (the read-back-site subtraction above reads breaths 0..k-1).
        if estimator_readback:
            cm_pooled = latents.mean(axis=1).cast(dtypes.float).detach()    # (B, H)
            cm_run_sum = cm_pooled if cm_run_sum is None else cm_run_sum + cm_pooled
...
def compute_drift_v200(
    ...
    estimator_cm (v300-1) / estimator_readback (v300-1.1): mirror of the
    canonical forward's explicit common-mode estimator — the drift metric
    must measure the same forward the training uses (method identity
    applies to the null).
    ...
    cm_gate         = getattr(model, "fg_v200_cm_gate", None)   # (#301) estimator gate

    if stage2a_waist:
        W_compress    = model.fg_v200_W_compress
        W_expand      = model.fg_v200_W_expand
        waist_gate    = model.fg_v200_waist_gate
        waist_odd_scale = getattr(model, "fg_v200_waist_odd_scale", None)  # (1,) v300-2w
    ...
    cm_run_sum = None     # (v300-1.1) — mirror of the canonical forward
    _lora_dicts_drift = fg_v200_lora_dicts(model) if expand_compress_lora else None  # v300-2
    ...
    for k in range(K):
        ...
        # Estimator centering (v300-1) — mirror of the canonical forward
        if estimator_cm:
            assert cm_gate is not None, \
                "estimator_cm=True but fg_v200_cm_gate missing — re-run attach_fg_params_v200 (backfill attaches it)"
            m_k = latents.mean(axis=1, keepdim=True)
            latents = latents - cm_gate.cast(latents.dtype).reshape(1, 1, 1) * m_k
        ...
        # Estimator read-back subtraction (v300-1.1) — mirror of the canonical forward
        if estimator_readback and k >= 1 and cm_run_sum is not None:
            mu_k = (cm_run_sum / float(k)).reshape(-1, 1, H).cast(latents.dtype)
            latents = latents - cm_gate.cast(latents.dtype).reshape(1, 1, 1) * mu_k

        h = latents.cast(dtypes.float)
        _ldd = _lora_dicts_drift if (expand_compress_lora and k % 2 == 0) else None  # v300-2 compress=even
        for _li, layer in enumerate(llama_layers[:4]):
            _ld = _ldd[_li] if _ldd is not None else None
        ...
        if stage2a_waist and (k % 2 == 0):
            ...
            # v300-2w structural cycle — mirror of the canonical forward
            latents_w_drift = _rms_norm(latents, commit_norm_w, rms_eps).cast(latents.dtype)
            w_full = _apply_waist_v200(latents_w_drift, W_compress, W_expand, waist_gate)
            latents = latents + waist_odd_scale.cast(latents.dtype).reshape(1, 1, 1) * (w_full - latents)
        ...
        # Estimator running-mean accumulation (v300-1.1) — mirror of canonical
        if estimator_readback:
            cm_pooled = latents.mean(axis=1).cast(dtypes.float).detach()
            cm_run_sum = cm_pooled if cm_run_sum is None else cm_run_sum + cm_pooled
...
def fg_v200_empty_taps() -> dict:
    ...
    return {
        ...
        "cm_mu": [],       # estimator m_k per breath, all K (v300-1; empty when flag off)
    }
...
def _compile_jit_fg_step_v200(
    ...
    estimator_cm (v300-1): threads the explicit common-mode estimator into
    the compiled forward. α_cm=0 at compile ⇒ step-0 identity (gate i);
    the flag changes the graph, so est and cont arms compile separately.
