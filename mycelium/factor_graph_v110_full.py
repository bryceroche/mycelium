"""v110-full: v109pi + ACCUMULATE notebook + photon-shaped waist gate.

Stacks v110-acc and v110-photon on the same v109pi substrate:
  - ACCUMULATE notebook (v110-acc): write-once K-slot memory, causal read
  - PHOTON gate (v110-photon): smooth sin² waist gate, B-field/E-field
    phase-locked to Q rotation

Both mechanisms target different bottlenecks:
  - Notebook helps when chain length is long (hard problems)
  - Smooth gate helps when gradient flow matters (easy problems)

If lifts are additive: all three difficulties at-or-above v109pi baseline.
If they interfere: stack falls between v110-acc and v110-photon.
"""
...
# ---------------------------------------------------------------------------
# Module constants — inherit defaults from v110-acc and v110-photon
# ---------------------------------------------------------------------------

V110_FULL_TASK              = int(os.environ.get("V110_FULL_TASK", "0")) > 0
V110_FULL_K_MAX             = int(os.environ.get("V110_FULL_K_MAX", str(V109PI_K_MAX)))
V110_FULL_N_DIGITS          = int(os.environ.get("V110_FULL_N_DIGITS", str(V109PI_N_DIGITS)))
V110_FULL_WAIST_DIM         = int(os.environ.get("V110_FULL_WAIST_DIM", str(V109PI_WAIST_DIM)))
V110_FULL_ALTERNATION       = int(os.environ.get("V110_FULL_ALTERNATION", "1")) > 0
V110_FULL_HARD_BREATH_LEVEL = int(os.environ.get("V110_FULL_HARD_BREATH_LEVEL", "0")) > 0
V110_FULL_VAR_LOSS_WEIGHT   = float(os.environ.get("V110_FULL_VAR_LOSS_WEIGHT", "1.0"))
V110_FULL_N_MAX             = int(os.environ.get("V110_FULL_N_MAX", str(V109PI_N_MAX)))
V110_FULL_F_MAX             = int(os.environ.get("V110_FULL_F_MAX", str(V109PI_F_MAX)))
V110_FULL_T_MAX             = V110_FULL_N_MAX + V110_FULL_F_MAX
V110_FULL_CODEBOOK_N        = int(os.environ.get("V110_FULL_CODEBOOK_N", str(V109PI_CODEBOOK_N)))
V110_FULL_IB_CENTROIDS      = os.environ.get("V110_FULL_IB_CENTROIDS", V109PI_IB_CENTROIDS)
V110_FULL_CALIB_WEIGHT      = float(os.environ.get("V110_FULL_CALIB_WEIGHT", str(V108_CALIB_WEIGHT)))
V110_FULL_FACTOR_AUX_WEIGHT = float(os.environ.get("V110_FULL_FACTOR_AUX_WEIGHT", str(V108_FACTOR_AUX_WEIGHT)))
V110_FULL_N_HEADS           = V109PI_N_HEADS
V110_FULL_PHASE_SCALE       = float(os.environ.get("V110_FULL_PHASE_SCALE", str(V109PI_PHASE_SCALE)))

V110_FULL_GATE_PROFILE = os.environ.get("V110_FULL_GATE_PROFILE", "sin2_pi")
V110_FULL_PHOTON_ALPHA = float(os.environ.get("V110_FULL_PHOTON_ALPHA", "0.5"))


# ---------------------------------------------------------------------------
# Forward pass — v109pi + ACCUMULATE notebook + PHOTON gate
# ---------------------------------------------------------------------------

def fg_breathing_forward_v110_full(
    model: Any,
    domain_init: Tensor,
    node_kinds: Tensor,
    staging_mask: Tensor,
    head_op_mask: Tensor,
    K: int,
    n_max: int = V110_FULL_N_MAX,
    f_max: int = V110_FULL_F_MAX,
    n_digits: int = V110_FULL_N_DIGITS,
    alternation: bool = V110_FULL_ALTERNATION,
    phase_scale: float = V110_FULL_PHASE_SCALE,
    gate_profile: str = V110_FULL_GATE_PROFILE,
    photon_alpha: float = V110_FULL_PHOTON_ALPHA,
):
    """Stacked forward: v109pi base + accumulate notebook + photon-gated waist."""
    ...
    for k in range(K):
        # READ from notebook (v110-acc) — empty at k=0, causal otherwise
        ...
        stk      = staging_mask[:, k, :, :]
        stk_h    = stk.reshape(B, 1, T, T).expand(B, V110_FULL_N_HEADS, T, T)
        ...
        # PHOTON-gated waist (v110-photon): blend binary + smooth profile
        if alternation:
        ...
        # WRITE to notebook (v110-acc): pool post-waist residual into slot k
...
# ---------------------------------------------------------------------------
# Parameter attachment — same as v110-acc (photon adds no params)
# ---------------------------------------------------------------------------

def attach_fg_params_v110_full(
    model: Any,
    hidden: int,
    n_max: int = V110_FULL_N_MAX,
    f_max: int = V110_FULL_F_MAX,
    k_max: int | None = None,
    n_digits: int = V110_FULL_N_DIGITS,
    n_code: int = V110_FULL_CODEBOOK_N,
    ib_centroids_path: str = V110_FULL_IB_CENTROIDS,
    waist_dim: int = V110_FULL_WAIST_DIM,
) -> None:
    """Attach v109pi + waist + accumulate notebook params. Photon adds none."""
    if k_max is None:
        k_max = V110_FULL_K_MAX
    attach_fg_params_v110_acc(
        model, hidden=hidden,
    ...
    profile = V110_FULL_GATE_PROFILE
    alpha   = V110_FULL_PHOTON_ALPHA
    ...
    print(f"[v110-full] photon gate_profile={profile} alpha={alpha:.3f}", flush=True)
    print(f"            pure_profile_gates: [{gates_s}]", flush=True)
    print(f"            blended (warm-start mix): [{blended_s}]", flush=True)


fg_v110_full_parameters = fg_v110_acc_parameters
fg_v110_full_state_dict = fg_v110_acc_state_dict
...
_JIT_V110_FULL_CACHE: dict = {}


def _compile_jit_fg_step_v110_full(
    model: Any,
    opt: Any,
    K: int,
    B: int,
    factor_aux_weight: float = V110_FULL_FACTOR_AUX_WEIGHT,
    calib_weight: float = V110_FULL_CALIB_WEIGHT,
    var_loss_weight: float = V110_FULL_VAR_LOSS_WEIGHT,
    hard_breath_level: bool = V110_FULL_HARD_BREATH_LEVEL,
    alternation: bool = V110_FULL_ALTERNATION,
    phase_scale: float = V110_FULL_PHASE_SCALE,
    n_max: int = V110_FULL_N_MAX,
    f_max: int = V110_FULL_F_MAX,
    ...
    key = ("v110_full", id(model), id(opt), int(K), int(B),
    ...
    if key in _JIT_V110_FULL_CACHE:
        return _JIT_V110_FULL_CACHE[key]

    fw, aw, vw, gc = float(factor_aux_weight), float(calib_weight), \
                     float(var_loss_weight), float(grad_clip)
    params = opt.params
    print(
        f"[JIT] compile v110-full fg step: K={K} B={B} "
    ...
    def _step(
        ...
        tree_lh, var_lh, fac_lh, calib_h = fg_breathing_forward_v110_full(
    ...
    _JIT_V110_FULL_CACHE[key] = _step
    print(f"[JIT] v110-full step ready (cache={len(_JIT_V110_FULL_CACHE)})", flush=True)
    return _step


def _compile_jit_fg_eval_v110_full(
    model: Any,
    K: int,
    B: int,
    n_max: int = V110_FULL_N_MAX,
    ...
    key = ("eval_v110_full", id(model), int(K), int(B), int(n_max), int(f_max),
    ...
    print(
        f"[JIT] compile v110-full fg eval: K={K} B={B} "
        f"profile={gate_profile} alpha={photon_alpha}...",
        flush=True,
    )
    ...
    def _eval(
        ...
        tree_lh, _, _, _ = fg_breathing_forward_v110_full(
            model, domain_init, node_kinds, staging_mask, head_op_mask,
            K=K, n_max=n_max, f_max=f_max, n_digits=n_digits,
            alternation=alternation, phase_scale=phase_scale,
            gate_profile=gate_profile, photon_alpha=photon_alpha,
    ...
    _JIT_V110_FULL_CACHE[key] = _eval
    print(f"[JIT] v110-full eval ready (cache={len(_JIT_V110_FULL_CACHE)})", flush=True)
    return _eval
