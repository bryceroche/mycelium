"""v105 factor graph data loader — digit-decomposed variables.

Extends v100 data loader with the following changes:

  1. VALUE RANGE: No filtering by [0,99]. All GSM8K values up to 10^N_DIGITS-1
     are handled (N_DIGITS=5 → values up to 99999).

  2. DIGIT DECOMPOSITION:
     - All variable values (gold and observed) are decomposed into N_DIGITS digit
       positions (MSD-first: index 0 = 10^(N-1) place).
     - digit_init: (B, N_MAX, N_DIGITS, 10)
         - Observed: one-hot at the correct digit position for each place.
         - Unobserved: uniform 1/10 across all 10 digit choices for each place.
     - gold_digits: (B, N_MAX, N_DIGITS) int — MSD-first digit targets.

  3. SEQUENCE LAYOUT:
     T_MAX = N_MAX * N_DIGITS + F_MAX
     node_kinds has shape (B, T_MAX) where:
       positions [v*N_DIGITS : v*N_DIGITS+N_DIGITS]  ← variable v's digit slots
       positions [N_MAX*N_DIGITS : N_MAX*N_DIGITS+F_MAX] ← factor nodes

  4. MASKS (staging and head-op):
     Same bipartite adjacency logic as v100, but all N_DIGITS positions of
     variable v are treated as a block:
       - Within-variable: all digit positions of v attend to each other (identity block).
       - Variable-to-factor: all digit positions of v can attend to factor f if v ∈ f.args.
       - Factor-to-variable: factor f can attend to all digit positions of v if v ∈ f.args.
     staging_mask now has shape (B, K_MAX, T_MAX, T_MAX).
     head_op_mask has shape (B, N_HEADS, T_MAX, T_MAX).

  5. GSM8K RECORDS:
     execute_fg_v105() does NOT filter by [0,99] — handles values up to 99999.
     Records with non-integer or negative intermediate values are skipped.
     Records with values > 10^N_DIGITS-1 are clamped with a warning counter.
"""
...
# ---------------------------------------------------------------------------
# Digit-level mask construction
# ---------------------------------------------------------------------------

def build_staging_and_head_masks_v105_np(
    ...
    """
    t_max     = n_max * n_digits + f_max
    n_var_tok = n_max * n_digits    # number of variable-digit tokens

    # 1. Build variable-level bipartite adjacency (T_v100, T_v100) where T_v100 = n_max + f_max
    t_max_v100 = n_max + f_max
    ...
    bipartite_v100 = bipartite_batch[0]  # (n_max+f_max, n_max+f_max) 0 or -1e4

    # 2. Expand to digit-level adjacency (t_max, t_max)
    # Layout: positions 0..(n_max*n_digits-1) are variable digit positions;
    #         positions n_max*n_digits..(n_max*n_digits+f_max-1) are factor positions.
    bipartite_v105 = np.full((t_max, t_max), -1e4, dtype=np.float32)

    # a) Within-variable: all digit positions of same variable attend to each other
    for vi in range(n_max):
        start = vi * n_digits
        end   = start + n_digits
        bipartite_v105[start:end, start:end] = 0.0   # full within-variable block

    # b) Factor self-attention
    for fi in range(f_max):
        fpos = n_var_tok + fi
        bipartite_v105[fpos, fpos] = 0.0

    # c) Variable-to-factor and factor-to-variable cross-attention
    #    If bipartite_v100[vi, n_max+fi] == 0 (allowed), then allow all digit positions of vi
    #    to attend to factor fi, and factor fi to attend to all digit positions of vi.
    for vi in range(n_max):
        for fi in range(n_factors):
            fpos_v100 = n_max + fi
            if bipartite_v100[vi, fpos_v100] >= -1e3:  # 0.0 means allowed
                vstart = vi * n_digits
                vend   = vstart + n_digits
                fpos_v105 = n_var_tok + fi
                bipartite_v105[vstart:vend, fpos_v105] = 0.0
                bipartite_v105[fpos_v105, vstart:vend] = 0.0

    # 3. Staging masks: same depth-based logic, but over expanded positions.
    # Factor result depths (same as v100)
    ...
    k_range = np.arange(k_max, dtype=np.int32)

    # Variable token visibility (per digit position → inherit from variable depth)
    # Build (t_max, k_max) visibility matrix
    ...
    for k in range(k_max):
        ...
        staging[k] = np.where(both_visible, bipartite_v105, -1e4)

    # 4. Head-op masks: same per-op specialization, expanded to digit level.
    ...
    # Self-attention always allowed for all ops
    diag_idx = np.arange(t_max)
    op_adj[:, diag_idx, diag_idx] = 0.0

    # Fill variable↔factor edges per op (respecting digit expansion)
    for fi in range(n_factors):
        ft = int(factor_types_np[fi])
        if ft < 0 or ft >= 4:
            continue
        ...
        # Also within-variable block for op-specific heads
        for vi_raw in factor_args_np[fi]:
            vi = int(vi_raw)
            if 0 <= vi < n_vars:
                vstart = vi * n_digits
...
def _execute_fg_v105(rec: dict) -> list[float] | None:
    """Execute a factor graph to completion, returning values for all variables.

    Same as v104's _execute_fg but no range restriction (handles large values).
    Returns None if execution cannot complete (missing values, div-by-zero).
    """
...
def convert_gsm8k_record_v105(
    rec: dict,
    n_digits: int = V105_N_DIGITS,
    n_max: int = V105_N_MAX,
    f_max: int = V105_F_MAX,
) -> dict | None:
    """Convert a GSM8K factor graph record to v105 format.

    Unlike v104, there is NO value range filter beyond 10^n_digits.
    Returns None if:
      - Factor graph cannot be executed.
      - Any value is non-integer.
      - Any value is negative (can't be represented as unsigned digits).

    Values above 10^n_digits-1 are CLAMPED (not filtered), so data is preserved.
    """
    vals = _execute_fg_v105(rec)
    if vals is None:
        return None
    ...
    # Require non-negative (digit representation is unsigned)
...
def _records_to_batch_v105(
    ...
    """Convert a list of records to numpy arrays for one v105 batch.

    New fields vs v100:
      digit_init      : (B, N_MAX, N_DIGITS, 10) float32
      gold_digits     : (B, N_MAX, N_DIGITS) int32
      factor_gold_dg  : (B, F_MAX, N_DIGITS) int32 — digit decomp of factor result gold
      factor_valid    : (B, F_MAX) float32 — 1 if real factor with valid result
    """
    B     = len(picks)
    T     = n_max * n_digits + f_max
    ...
    for b, rec in enumerate(picks):
        ...
        for vi in range(min(n_total, n_max)):
            ...
            digs = value_to_digits(gv, n_digits)
            gold_digits_np[b, vi] = digs

            # Set node_kinds for all digit positions of this variable
            tok_base = vi * n_digits
            if obs_mask_rec[vi] == 1:
                obs_val = max(0, int(round(obs_vals[vi])) if obs_vals[vi] is not None else gv)
                obs_digs = value_to_digits(obs_val, n_digits)
                for p in range(n_digits):
                    digit_init[b, vi, p, :] = 0.0
                    digit_init[b, vi, p, obs_digs[p]] = 1.0  # one-hot
                    node_kinds[b, tok_base + p] = 0  # observed
                obs_mask[b, vi] = 1
            else:
                # uniform 1/10 across all digits (already initialized above)
                for p in range(n_digits):
                    node_kinds[b, tok_base + p] = 1  # unobserved
                obs_mask[b, vi] = 0

        # Fill factor type/args
        for fi in range(min(nf, f_max)):
            ...
            # Factor gold digits (result index)
            r_idx = int(fa[2])
            if 0 <= r_idx < n_total:
                r_val = max(0, int(round(gold_vals[r_idx])))
                factor_gold_dg[b, fi] = value_to_digits(r_val, n_digits)
                factor_valid[b, fi] = 1.0

        # DAG depths
        depth_dict = compute_var_depth(obs_mask_rec,
                                       [fa_list[fi] for fi in range(min(nf, f_max))],
                                       n_total)
        for vi in range(min(n_total, n_max)):
            var_depth_np[b, vi] = depth_dict.get(vi, -1)

        # Build staging and head-op masks (expanded to T = N_MAX*N_DIGITS + F_MAX)
        staging_b, head_op_b = build_staging_and_head_masks_v105_np(
            factor_types_np[b],
            factor_args_np[b],
            var_depth_np[b],
...
def batch_to_tensors_v105(batch_np: dict) -> dict:
    """Convert v105 numpy batch dict to tinygrad Tensors (realized)."""
    return {
...
def load_gsm8k_records_v105(
    path: str,
    n_digits: int = V105_N_DIGITS,
    n_max: int = V105_N_MAX,
    f_max: int = V105_F_MAX,
) -> list[dict]:
    """Load and convert GSM8K factor graph records to v105 format.

    NO value range filter — handles values up to 10^n_digits.
    """
    ...
    with open(path) as f:
        for line in f:
            ...
            n_loaded += 1
            converted = convert_gsm8k_record_v105(rec, n_digits=n_digits, n_max=n_max, f_max=f_max)
            if converted is not None:
                records.append(converted)
    pct = 100.0 * len(records) / max(n_loaded, 1)
    print(
        f"[fg_data_v105] GSM8K: loaded {n_loaded}, kept {len(records)} ({pct:.1f}%) "
        f"(no range filter; n_digits={n_digits})",
        flush=True,
    )
    return records


class FactorGraphLoaderV105:
    """Iterates factor-graph records in v105 format (digit-expanded)."""
    ...
    def __init__(
        ...
        print(
            f"[fg_data_v105] loaded {len(records)} from {os.path.basename(path)}; "
    ...
    def sample_batch(self, step: int = 0) -> dict:
        ...
        batch_np = _records_to_batch_v105(
...
class DualDataLoaderV105:
    """Samples from synthetic + GSM8K records at configurable ratio (v105 format)."""
    ...
    def __init__(
        ...
        self.rng      = random.Random(seed)
        print(
            f"[fg_data_v105] DualLoader: synth={len(synth_loader)} gsm8k={len(gsm8k_records)} "
    ...
    def sample_batch(self, step: int = 0) -> dict:
        ...
        batch_np = _records_to_batch_v105(mixed, self.n_max, self.f_max, self.k_max,
