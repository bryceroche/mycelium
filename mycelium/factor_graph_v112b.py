"""v112b: Phase 1 — Shared learnable factor graph topology.

Hypothesis: a learned pairwise attention bias from a shared topology
tensor adds signal over the existing hard binary masks.

ONE shared learnable tensor `node_topology: (T, latent_dim)` (T = n_max
+ f_max = 24, latent_dim = 64). It feeds two adaptations into the
existing v110-step3 forward:

1. Attention bias: `attn_bias = attn_bias_scale * (topology @ topology.T)`
   added to the existing combined mask. Zero-init scale → byte-identical
   warm-start.

2. Per-position residual gate: at start of each breath,
   `x = x * (1 + tanh(topology @ W_res_gate))`. Zero-init W_res_gate →
   tanh(0)=0 → multiplier=1 → byte-identical.

Total new params: ~66K
  - node_topology       : (24, 64)    = 1,536
  - W_res_gate          : (64, 1024)  = 65,536
  - attn_bias_scale     : (1,)        = 1

Warm-start safety:
  attn_bias_scale init = 0 and W_res_gate init = 0 → both adaptations
  are no-ops at step 0 → forward is byte-identical to v110-step3.
"""
...
V112B_TOPOLOGY_DIM    = int(os.environ.get("V112B_TOPOLOGY_DIM",    "64"))
V112B_BIAS_SCALE_INIT = float(os.environ.get("V112B_BIAS_SCALE_INIT", "0.0"))


def attach_fg_params_v112b(
    model: Any,
    hidden: int,
    ...
    """Attach v110-step3 base + 3 new v112b topology params."""
    attach_fg_params_v110_step3(
        model, hidden=hidden,
    ...
    if hasattr(model, "fg_v115_node_topology"):
        return

    T = n_max + f_max
    latent_dim = V112B_TOPOLOGY_DIM

    # Warm-init topology from small-norm Gaussian. The model learns
    # node differentiation from training.
    node_topology_np = np.random.randn(T, latent_dim).astype(np.float32) * 0.02
    model.fg_v115_node_topology = Tensor(
        node_topology_np
    ).contiguous().realize()

    # W_res_gate: (latent_dim, hidden) zero-init
    W_res_gate_np = np.zeros((latent_dim, hidden), dtype=np.float32)
    model.fg_v115_W_res_gate = Tensor(
        W_res_gate_np
    ).contiguous().realize()

    # attn_bias_scale: scalar zero-init (shape (1,) for Tensor compat)
    attn_bias_scale_np = np.array([V112B_BIAS_SCALE_INIT], dtype=np.float32)
    model.fg_v115_attn_bias_scale = Tensor(
        attn_bias_scale_np
    ).contiguous().realize()

    n_new = T * latent_dim + latent_dim * hidden + 1
    print(f"[v112b] topology params attached: T={T} latent_dim={latent_dim} "
          f"bias_scale_init={V112B_BIAS_SCALE_INIT} "
          f"(zero scale + zero W_res_gate → byte-identical warm-start)  "
          f"+{n_new:,} params",
          flush=True)


def fg_v112b_parameters(model: Any) -> list[Tensor]:
    """v110-step3 (acc) params + 3 new topology params."""
    from scripts.v110_acc_factor_graph_train import collect_fg_params_v110_acc
    params = list(collect_fg_params_v110_acc(model))
    params.extend([
        model.fg_v115_node_topology,
        model.fg_v115_W_res_gate,
        model.fg_v115_attn_bias_scale,
    ])
    return params


def fg_v112b_state_dict(model: Any) -> dict[str, Tensor]:
    """v110-step3 (acc) state_dict + 3 new topology entries."""
    from scripts.v110_acc_factor_graph_train import model_state_dict_v110_acc
    sd = dict(model_state_dict_v110_acc(model))
    sd["fg_v115_node_topology"]    = model.fg_v115_node_topology
    sd["fg_v115_W_res_gate"]       = model.fg_v115_W_res_gate
    sd["fg_v115_attn_bias_scale"]  = model.fg_v115_attn_bias_scale
    return sd


def fg_breathing_forward_v112b(
    model: Any,
    domain_init: Tensor,
    node_kinds: Tensor,
    staging_mask: Tensor,
    ...
    """v110-step3 forward + shared learnable topology adaptations.
    ...
    With attn_bias_scale=0 and W_res_gate=0, output is byte-identical to
    v110-step3.
    """
    ...
    # Topology in float for stability of the pairwise op
    topology_f = node_topology.cast(dtypes.float)                       # (T, latent)
    # Attention bias: (T, T) = (topology @ topology.T) * scale
    attn_bias_full = (topology_f @ topology_f.T) * attn_bias_scale.reshape(1, 1).cast(dtypes.float)
    attn_bias_btht = attn_bias_full.reshape(1, 1, T, T)                  # broadcasts over (B, N_HEADS)

    # Per-position residual gate: (T, H), broadcast over batch as (1, T, H)
    ...
    for k in range(K):
        ...
        # *** v112b: per-position residual gate (zero-init → multiplier = 1) ***
        ...
        # *** v112b: add learnable attn bias (zero-init scale → no-op) ***
...
_JIT_V112B_CACHE: dict = {}


def _compile_jit_fg_step_v112b(
    model: Any,
    opt: Any,
    K: int,
    B: int,
    ...
    """Train step for v112b. Same loss as v110-step3, with topology forward."""
    key = ("v112b", id(model), id(opt), int(K), int(B),
    ...
    if key in _JIT_V112B_CACHE:
        return _JIT_V112B_CACHE[key]
    ...
    params = opt.params
    print(
        f"[JIT] compile v112b step: K={K} B={B} "
    ...
    _JIT_V112B_CACHE[key] = _step
    print(f"[JIT] v112b step ready (cache={len(_JIT_V112B_CACHE)})",
          flush=True)
    return _step


def compile_jit_eval_v112b(
    model: Any,
    ...
    """JIT'd eval forward — matches evaluate_v109 signature (6 inputs, 2 outputs).

    Noise is baked-in as zeros (deterministic eval). Topology adaptations
    still active via the learned params. Returns (pred_digits, cell_acc).
    """
    T = n_max + f_max
    H = 1024
    ...
    def _eval(
        ...
        tree_lh, _, _, _, _ = fg_breathing_forward_v112b(
            model, domain_init, node_kinds, staging_mask, head_op_mask,
...
def compile_jit_mcbp_v112b(
    ...
    """MC-BP-style JIT eval with v112b forward: returns final-breath tree_logits."""
    @TinyJit
    def _eval(
        domain_init: Tensor,
        node_kinds: Tensor,
