"""v110-step3 + Tree-logits feedback with Gumbel-softmax (discrete commitment).

Variant of factor_graph_v110_step3_fb that replaces the soft
`softmax(tree_logits) @ codebook` write with a Gumbel-softmax + straight-
through estimator: forward pass sees a near-one-hot DISCRETE digit
commitment, backward pass routes gradient through the soft distribution.

Hypothesis (other-Claude Option 3): forcing the model to COMMIT to a
discrete digit at each breath (instead of a smooth soft posterior)
gives the next breath an unambiguous "this is what I think" signal that
must be either confirmed or explicitly overridden. Smooth softmax
feedback lets the model maintain ambiguity across breaths; discrete
commitment forces resolution.

Mechanism (per breath k, write step):
  logits_perturbed = (tree_logits_k + gumbel_noise) / temperature
  soft = softmax(logits_perturbed)                      # (B, n_max, 5, 10)
  argmax = soft.argmax(axis=-1)                         # (B, n_max, 5) int
  hard = argmax.one_hot(10)                             # (B, n_max, 5, 10)
  st = (hard - soft).detach() + soft                    # straight-through
  fb = (st @ tree_codebook).sum_over_levels             # (B, n_max, H)
  fb_ln = layernorm(fb, ...)
  feedback_buffer = gate * fb_ln

Gumbel noise as JIT input (shape K_max × B × n_max × n_digits × 10),
sampled outside per step. At inference, can pass zeros to get
deterministic straight-through-argmax.

Temperature τ is a hyperparameter (env V110_STEP3_FB_GUMBEL_TAU,
default 1.0). At τ=1.0 with Gumbel noise, this approximates categorical
sampling. At τ→0 it becomes pure argmax. The straight-through gradient
flows through `soft` regardless.
"""
...
from mycelium.factor_graph_v110_step3_fb import (
    V110_STEP3_FB_GATE_INIT,
    attach_fg_params_v110_step3_fb,
    fg_v110_step3_fb_parameters, fg_v110_step3_fb_state_dict,
    _layernorm_fb,
)
...
V110_STEP3_FB_GUMBEL_TAU = float(os.environ.get(
    "V110_STEP3_FB_GUMBEL_TAU", "1.0"))


# Re-export attach (same param structure as fb soft variant)
attach_fg_params_v110_step3_fb_gumbel = attach_fg_params_v110_step3_fb
fg_v110_step3_fb_gumbel_parameters = fg_v110_step3_fb_parameters
fg_v110_step3_fb_gumbel_state_dict = fg_v110_step3_fb_state_dict


def fg_breathing_forward_v110_step3_fb_gumbel(
    model: Any,
    domain_init: Tensor,
    node_kinds: Tensor,
    staging_mask: Tensor,
    head_op_mask: Tensor,
    noise: Tensor,           # (K_max, B, T, H) — SBP noise
    noise_scale: Tensor,     # scalar — SBP scale
    gumbel: Tensor,          # (K_max, B, n_max, n_digits, 10) — Gumbel samples
    K: int,
    tau: float = V110_STEP3_FB_GUMBEL_TAU,
    ...
    """v110-step3 + Gumbel-ST feedback + SBP-compatible noise.

    Same returns as fb forward. With gate=0 and noise_scale=0 and
    gumbel=zeros, output is byte-identical to v110-step3 baseline at K=1
    (and very close at K>1 because the feedback gate is zero).

    Note: even at gate=0, the discrete argmax operation happens. At
    gate=0 the contribution is zero so this doesn't matter for output,
    but it slightly changes the JIT graph shape relative to the soft fb.
    """
    ...
    for k in range(K):
        ...
        # *** Gumbel-softmax + straight-through write for next breath ***
        #   logits_perturbed = (logits + gumbel) / tau
        #   clip to [-30, 30] for numerical stability (prevents softmax overflow)
        #   soft = softmax(logits_perturbed)
        #   hard = onehot(argmax(soft))
        #   st = soft + (hard - soft).detach()      # forward: hard; backward: soft
        logits_perturbed = ((tree_logits_k + gumbel[k].cast(tree_logits_k.dtype)) * inv_tau).clip(-30.0, 30.0)
        soft = logits_perturbed.softmax(axis=-1)                          # (B, n_max, n_digits, 10)
        argmax_k = soft.argmax(axis=-1)                                   # (B, n_max, n_digits)
        hard = argmax_k.one_hot(10).cast(soft.dtype)                      # (B, n_max, n_digits, 10)
        st = soft + (hard - soft).detach()

        st_flat = st.reshape(B, n_max, n_digits * 10).cast(dtypes.float)
        fb_raw  = st_flat @ tree_cb_flat.cast(dtypes.float)               # (B, n_max, H)
...
_JIT_V110_STEP3_FB_GUMBEL_CACHE: dict = {}


def _compile_jit_fg_step_v110_step3_fb_gumbel(
    model: Any,
    opt: Any,
    K: int,
    B: int,
    ...
    """Train step JIT with Gumbel-ST fb + SBP-style noise inputs."""
    key = ("v110_step3_fb_gumbel", id(model), id(opt), int(K), int(B),
           ...
           str(gate_profile), float(photon_alpha), float(tau), float(grad_clip))
    if key in _JIT_V110_STEP3_FB_GUMBEL_CACHE:
        return _JIT_V110_STEP3_FB_GUMBEL_CACHE[key]
    ...
    params = opt.params
    print(
        f"[JIT] compile v110-step3 FB-GUMBEL step: K={K} B={B} tau={tau} "
    ...
    def _step(
        ...
        noise, noise_scale, gumbel,
    ):
        opt.zero_grad()

        tree_lh, _, fac_lh, calib_h, step_mh = (
            fg_breathing_forward_v110_step3_fb_gumbel(
    ...
    _JIT_V110_STEP3_FB_GUMBEL_CACHE[key] = _step
    print(f"[JIT] v110-step3 FB-GUMBEL step ready "
          f"(cache={len(_JIT_V110_STEP3_FB_GUMBEL_CACHE)})", flush=True)
    return _step


def compile_jit_mcbp_fb_gumbel(
    model: Any,
    ...
    """MC-BP eval with Gumbel-ST fb + noise inputs. Returns final tree_logits."""
    @TinyJit
    def _eval(
        domain_init: Tensor,
        node_kinds: Tensor,
        ...
        tree_lh, _, _, _, _ = fg_breathing_forward_v110_step3_fb_gumbel(
            model, domain_init, node_kinds, staging_mask, head_op_mask,
            noise, noise_scale, gumbel,
            K=K, tau=tau,
            n_max=n_max, f_max=f_max, n_digits=n_digits,
...
def compile_jit_eval_v110_step3_fb_gumbel(
    model: Any,
    K: int = V110_STEP3_K_MAX,
    B: int = 8,
    tau: float = V110_STEP3_FB_GUMBEL_TAU,
    ...
    """Eval JIT matching evaluate_v109 signature (returns pred_digits, cell_acc).

    Gumbel noise + SBP noise both baked-in as zeros for deterministic eval.
    Straight-through-argmax still fires at gumbel=0 (no Gumbel perturbation,
    just deterministic argmax of (logits/τ)).
    """
    T = n_max + f_max
    H = 1024
    eval_noise_zeros = Tensor.zeros(K, B, T, H, dtype=dtypes.half).contiguous().realize()
    eval_gumbel_zeros = Tensor.zeros(K, B, n_max, n_digits, 10, dtype=dtypes.half).contiguous().realize()
    ...
    def _eval(
        ...
        tree_lh, _, _, _, _ = fg_breathing_forward_v110_step3_fb_gumbel(
            model, domain_init, node_kinds, staging_mask, head_op_mask,
            eval_noise_zeros, eval_noise_scale, eval_gumbel_zeros,
            K=K, tau=tau,
            n_max=n_max, f_max=f_max, n_digits=n_digits,
            alternation=alternation, phase_scale=phase_scale,
            gate_profile=gate_profile, photon_alpha=photon_alpha,
