...
os.environ.setdefault("ALG_NOTEBOOK","1"); os.environ.setdefault("ALG_SIXWAVE","1"); os.environ.setdefault("ALG_FTYPES","9")
...
for s0 in range(0,CAP,8):
    ...
    for bi,ri in enumerate(sl):
        for j in range(L_FAC):
            ...
            _ft=KINDS[int(gold["ftype"][ri,j])]
            if _ft=="rel":
                _ops=["add","sub","mul","div"]
                kind=_ops[int(gold["op"][ri,j])] if int(gold["op"][ri,j])<4 else "rel"
                if "arg_dup" in gold and gold["arg_dup"][ri,j]>0.5: kind="sq"
            elif _ft=="macro": kind="opa"
            elif _ft=="frac": kind="fr"
            else: kind=_ft
            prev=-1
