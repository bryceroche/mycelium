...
class TestSignatureStats:
    ...
    def test_is_reliable(self):
        """Test reliability determination."""
        # Not enough data
        stats = SignatureStats(signature_id=1, total_executions=3, success_rate=0.9)
        assert stats.is_reliable is False

        # Enough data but low success
        stats = SignatureStats(
            signature_id=1,
            total_executions=10,
            success_rate=0.5,
            param_extraction_rate=0.9,
        )
        assert stats.is_reliable is False
...
class TestStepStatsCollector:
    ...
    def test_get_routing_context(self, stats_db):
        """Test getting global routing context."""
        collector = StepStatsCollector(stats_db)

        # Record executions for multiple signatures
        for sig_id in range(1, 4):
            for i in range(10):
                collector.record_execution(
                    signature_id=sig_id,
                    step_text=f"sig {sig_id} step {i}",
                    execution_time_ms=50.0 * sig_id,  # Different speeds
                    step_completed=True,
                    was_injected=True,
                    dsl_type="math",
        ...
        ctx = collector.get_routing_context()

        assert ctx.total_signatures == 3
        assert ctx.total_executions == 30
        assert ctx.global_success_rate == 1.0
        assert len(ctx.fastest_signatures) > 0

    def test_get_dsl_type_breakdown(self, stats_db):
        """Test DSL type breakdown."""
        collector = StepStatsCollector(stats_db)

        # Record mixed types
        for dsl_type, count in [("math", 5), ("decompose", 3), ("router", 2)]:
            for i in range(count):
                collector.record_execution(
                    signature_id=1,
                    step_text=f"{dsl_type} step {i}",
                    execution_time_ms=10.0,
                    step_completed=True,
                    was_injected=False,
                    dsl_type=dsl_type,
        ...
        breakdown = collector.get_dsl_type_breakdown(1)

        assert breakdown["math"] == 5
        assert breakdown["decompose"] == 3
        assert breakdown["router"] == 2

    def test_get_recent_executions(self, stats_db):
        """Test getting recent execution records."""
        collector = StepStatsCollector(stats_db)
        ...
        for i in range(5):
            collector.record_execution(
                ...
                was_injected=False,
                dsl_type="math",
                params_extracted={"n": i},
                param_extraction_success=True,
            )

        recent = collector.get_recent_executions(1, limit=3)

        assert len(recent) == 3
        # Should be in reverse chronological order
        assert recent[0].step_text == "step 4"
        assert recent[2].step_text == "step 2"


class TestRoutingHelpers:
    """Tests for routing helper functions."""

    def test_compute_routing_bonus_insufficient_data(self):
        """Test routing bonus with insufficient data."""
        stats = SignatureStats(signature_id=1, total_executions=2)
        assert compute_routing_bonus(stats) == 0.0

    def test_compute_routing_bonus_fast_signature(self):
        """Test routing bonus for fast signature."""
        stats = SignatureStats(
            signature_id=1,
            total_executions=10,
            avg_execution_ms=50.0,  # Fast
            param_extraction_rate=1.0,
            dsl_execution_ratio=0.9,
        )
        bonus = compute_routing_bonus(stats)
        assert bonus > 0.15  # Should get time bonus + param bonus + dsl bonus

    def test_should_prefer_decomposition_insufficient_data(self):
        """Test decomposition preference with insufficient data."""
        stats = SignatureStats(signature_id=1, total_executions=3)
        assert should_prefer_decomposition(stats) is False

    def test_should_prefer_decomposition_dsl_heavy_low_success(self):
        """Test decomposition preference for DSL-heavy low-success signature."""
        stats = SignatureStats(
            signature_id=1,
            total_executions=20,
            dsl_execution_ratio=0.9,  # DSL heavy
            success_rate=0.3,  # Low success
            param_extraction_rate=0.5,
        )
        assert should_prefer_decomposition(stats) is True

    def test_should_prefer_decomposition_healthy_dsl(self):
        """Test decomposition preference for healthy DSL signature."""
        stats = SignatureStats(
            signature_id=1,
            total_executions=20,
            dsl_execution_ratio=0.9,
    ...
    def test_get_signature_health_unknown(self):
        """Test health assessment with insufficient data."""
        stats = SignatureStats(signature_id=1, total_executions=2)
        assert get_signature_health(stats) == "unknown"

    def test_get_signature_health_healthy(self):
        """Test health assessment for healthy signature."""
        stats = SignatureStats(
            signature_id=1,
            total_executions=100,
            success_rate=0.9,
        ...
        assert get_signature_health(stats) == "healthy"

    def test_get_signature_health_degraded(self):
        """Test health assessment for degraded signature."""
        stats = SignatureStats(
            signature_id=1,
            total_executions=100,
            success_rate=0.6,
            avg_execution_ms=500.0,
            param_extraction_rate=0.7,
            dsl_execution_ratio=0.5,
        )
        assert get_signature_health(stats) == "degraded"

    def test_get_signature_health_unhealthy(self):
        """Test health assessment for unhealthy signature."""
        stats = SignatureStats(
            signature_id=1,
            total_executions=100,
            success_rate=0.2,
        ...
        assert get_signature_health(stats) == "unhealthy"
