...
A defining commitment of the project is that the system's output is not an answer but a decision: answer, or abstain. A chain of independent gates — an out-of-distribution "mouth," a multi-view parsing vote, a cross-model panel, and finally an exact answer key — stands between the neural parse and any claim the system makes. The standard is not how often the system answers; it is that a certified answer is never wrong. The result is interpretability by construction: predicates, roles, bindings, and graph structure that can be read directly, rather than reconstructed after the fact.

One organizing idea Bryce borrows from compilers is the MLIR-style lowering ladder. A modern compiler doesn't translate source code to machine code in one leap; it descends through a stack of intermediate representations, each dialect preserving exactly the structure the next stage needs. Mycelium treats reasoning the same way: natural language is progressively lowered — prose to typed factor graph, recurring subgraphs to named macro-abstractions, macros expanded back to primitives before verification — with every rung machine-checkable. The ladder also climbs: as the system discovers recurring structural patterns, new annotation is written one level of abstraction higher, letting the representation grow richer while the verifier still grades everything in primitives.
