"""v103 factor graph training driver — VQ-VAE-style waist + codebook compression.

Architecture: encoder (1024→512) → quantize via codebook (32×512d) → decoder (512→1024).
The codebook is the ONLY information path from encoder to decoder — topology-invariant.
...
from mycelium.factor_graph_v103 import (
    V103_K_MAX, V103_FACTOR_AUX_WEIGHT, V103_CALIB_WEIGHT,
    V103_N_MAX, V103_F_MAX, V103_N_HEADS, V103_KL_DIAG, V103_CODEBOOK_N, V103_WAIST,
    attach_fg_params_v103, fg_v103_vqvae_parameters, fg_v103_state_dict,
    fg_breathing_forward_v103,
    _compile_jit_fg_step_v103, _compile_jit_fg_eval_v103,
)
...
def collect_fg_params_v103(model) -> list[Tensor]:
    """Trainable parameters: shared L0-L3 attn/FFN + v100 fg params + v103 VQ-VAE params."""
    params: list[Tensor] = []
    sw = model.block.shared
    ...
    params += fg_v103_vqvae_parameters(model)
    return params


def model_state_dict_v103(model) -> dict:
    """State dict for v103 factor-graph training (v100 keys + v103 VQ-VAE keys)."""
    sd = {
        "ln_f.g": model.ln_f_g,
        "ln_f.b": model.ln_f_b,
    }
...
def load_ckpt_v103(model, path: str):
    """Load a v100 or v103 checkpoint.

    v100 keys are loaded exactly.
    v103 VQ-VAE keys (fg_v103.*) are loaded if present; if missing (e.g.
    warm-starting from a v100 ckpt), the near-identity init from
    attach_fg_params_v103 is kept (W_expand=0, delta_gate_quant=0 → forward = v100).
    """
    sd = safe_load(path)
    ...
    v103_vq_keys  = [k for k in targets if k.startswith("fg_v103.")]
    loaded_vq     = [k for k in loaded  if k.startswith("fg_v103.")]
    if not loaded_vq:
        print(f"  no v103 VQ-VAE keys in ckpt — keeping near-identity init (warm-start from v100)")
    else:
        print(f"  loaded {len(loaded_vq)}/{len(v103_vq_keys)} v103 VQ-VAE keys")
    if missing:
        non_vq_missing = [k for k in missing if not k.startswith("fg_v103.")]
        if non_vq_missing:
            print(f"  ckpt missing {len(non_vq_missing)} non-VQ keys (kept init): "
                  f"{non_vq_missing[:5]}")


def evaluate_v103(model, loader: FactorGraphLoaderV100, K: int,
                  max_batches: int = 20, eval_fn=None,
                  n_max: int = V103_N_MAX, f_max: int = V103_F_MAX) -> dict:
...
def main():
    V103_TASK_LOCAL = int(getenv("V103_TASK", 0)) > 0
    assert V103_TASK_LOCAL, "V103_TASK=1 must be set"

    K      = int(getenv("V103_K_MAX",    str(V103_K_MAX)))
    ...
    CKPT_LABEL        = getenv("CKPT_LABEL",            "v103_smoke")
    RESUME_FROM       = getenv("RESUME_FROM",           "")
    PYTHIA_INIT       = int(getenv("PYTHIA_INIT",       "1")) > 0
    SEED              = int(getenv("SEED",              "42"))
    N_CODE            = int(getenv("V103_CODEBOOK_N",   str(V103_CODEBOOK_N)))
    WAIST             = int(getenv("V103_WAIST",        str(V103_WAIST)))

    N_MAX  = int(getenv("V103_N_MAX",  str(V103_N_MAX)))
    F_MAX  = int(getenv("V103_F_MAX",  str(V103_F_MAX)))
    T_MAX  = N_MAX + F_MAX

    DIFFICULTY_FILTER = (
        os.environ.get("V103_DIFFICULTY_FILTER", "").strip()
        or os.environ.get("V100_DIFFICULTY_FILTER", "").strip()
        or None
    )
    CURRICULUM        = int(getenv("V103_CURRICULUM",       "0")) > 0
    CURRICULUM_ANNEAL = int(getenv("V103_CURRICULUM_ANNEAL", "1000"))

    TRAIN_PATH   = getenv("V103_TRAIN", getenv("V100_TRAIN", ".cache/factor_graph_train.jsonl"))
    VAL_PATH     = getenv("V103_VAL",   getenv("V100_VAL",   ".cache/factor_graph_test.jsonl"))
    EVAL_BATCHES = int(getenv("EVAL_BATCHES", "20"))
    EVAL_BATCH   = int(getenv("EVAL_BATCH",   str(BATCH)))

    FACTOR_AUX_WEIGHT = float(getenv("V103_FACTOR_AUX_WEIGHT", str(V103_FACTOR_AUX_WEIGHT)))
    CALIB_WEIGHT      = float(getenv("V103_CALIB_WEIGHT",       str(V103_CALIB_WEIGHT)))

    KL_DIAG_EVERY   = int(getenv("KL_DIAG_EVERY", "100"))
    KL_DIAG_ENABLED = V103_KL_DIAG or (int(getenv("V103_KL_DIAG", "0")) > 0)

    print(f"=== v103 factor graph training (VQ-VAE waist + codebook compression) ===")
    print(f"device={Device.DEFAULT}  B={BATCH}  K={K}  steps={STEPS}  lr={LR}")
    print(f"N_MAX={N_MAX}  F_MAX={F_MAX}  T_MAX={T_MAX}  codebook_n={N_CODE}  waist={WAIST}")
    ...
    # Attach v100 params first, then v103 VQ-VAE params
    attach_fg_params_v100(model, hidden=cfg.hidden, n_heads=cfg.n_heads,
                          k_max=K, n_max=N_MAX, f_max=F_MAX)
    attach_fg_params_v103(model, hidden=cfg.hidden, n_code=N_CODE, waist=WAIST)
    Device[Device.DEFAULT].synchronize()

    params = collect_fg_params_v103(model)
    n_params = sum(int(np.prod(t.shape)) for t in params)
    print(f"  trainable params: {n_params/1e6:.1f}M")
    # Report VQ-VAE param count separately
    vq_params = fg_v103_vqvae_parameters(model)
    n_vq = sum(int(np.prod(t.shape)) for t in vq_params)
    K_max = int(model.fg_v100_breath_embed.shape[0])
    print(f"  VQ-VAE params: {n_vq/1e6:.4f}M  "
          f"(W_compress={cfg.hidden}×{WAIST}, codebook={N_CODE}×{WAIST}, "
          f"W_expand={WAIST}×{cfg.hidden}, delta_gate_quant={K_max})")

    if RESUME_FROM:
        print(f"resuming from ckpt: {RESUME_FROM}")
        load_ckpt_v103(model, RESUME_FROM)
        print("  loaded.")
    ...
    train_loader = FactorGraphLoaderV100(
        ...
        n_max=N_MAX, f_max=F_MAX, k_max=K, n_heads=V103_N_HEADS,
        seed=SEED,
    )
    ...
    ckpt_dir = ".cache/fg_v103_ckpts"
    os.makedirs(ckpt_dir, exist_ok=True)

    Tensor.training = True
    step_fn = _compile_jit_fg_step_v103(
    ...
    eval_fn = _compile_jit_fg_eval_v103(
    ...
    for step in range(1, STEPS + 1):
        ...
        if KL_DIAG_ENABLED and step % KL_DIAG_EVERY == 0:
            Tensor.training = False
            var_lh_diag, _, _ = fg_breathing_forward_v103(
                model, domain_init, node_kinds, staging_mask, head_op_mask,
                K=K, n_max=N_MAX, f_max=F_MAX,
            )
        ...
        if step % CKPT_EVERY == 0:
            ...
            safe_save(model_state_dict_v103(model), ckpt_path)
