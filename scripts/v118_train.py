"""v118 training driver — 4-phase piecewise photon + Perceiver + phase-gated LoRAs.

Three mechanisms, all toggleable via env flags:
  V118_GATE_PROFILE     — "piecewise_4phase" (new) or any existing profile
  V118_PERCEIVER_ENABLED — 0/1 (Perceiver bottleneck at phase transitions)
  V118_PER_PHASE_LORAS   — 0/1 (4 phase-gated LoRA adapter sets)

Ablation matrix (single build, four smoke variants):
  v118-a: GATE_PROFILE=piecewise_4phase, PERCEIVER=0, LORAS=0
  v118-b: GATE_PROFILE=piecewise_4phase, PERCEIVER=1, LORAS=0
  v118-c: GATE_PROFILE=piecewise_4phase, PERCEIVER=1, LORAS=1  (FULL)
  v118-d: GATE_PROFILE=sin2_pi,          PERCEIVER=0, LORAS=0  (control = v112b)

All variants warm-start from .cache/fg_v112b_ckpts/v112b_cont1_final.safetensors.
"""
import gc
import os
import sys
...
from mycelium.factor_graph_v118 import (
    V118_GATE_PROFILE, V118_PERCEIVER_ENABLED, V118_PERCEIVER_N_LATENTS,
    V118_PER_PHASE_LORAS, V118_LORA_RANK,
    attach_fg_params_v118,
    fg_v118_parameters, fg_v118_state_dict,
    _compile_jit_fg_step_v118,
    compile_jit_eval_v118,
)
...
def load_ckpt_v118(model, path: str) -> None:
    """Load v110-step3/v112b ckpt + optionally v118-specific params if present."""
    ...
    # v118-specific params (present only in v118 ckpts, not v112b ckpts)
    v118_keys = [
        "fg_v118_perceiver_latents",
        "fg_v118_perceiver_gate",
    ]
    for phase_name in ["explore", "compress", "commit", "refine"]:
        for layer_idx in range(4):
            for proj_name in ["q", "k", "v", "o", "ffn_w1", "ffn_w2"]:
                for suffix in ("down", "up"):
                    v118_keys.append(
                        f"fg_v118_lora_{phase_name}_{layer_idx}_{proj_name}_{suffix}"
                    )
    for key in v118_keys:
        if key in sd and hasattr(model, key):
            dst = getattr(model, key)
            src = sd[key].to(dst.device).realize()
            dst.assign(src).realize()
            print(f"  loaded v118 param: {key}", flush=True)
...
def main():
    ...
    CKPT_LABEL = getenv("CKPT_LABEL",              "v118_run")
    ...
    GATE_PROFILE = getenv("V118_GATE_PROFILE",     V118_GATE_PROFILE)
    ...
    # v118-specific flags
    PERCEIVER_ENABLED  = int(getenv("V118_PERCEIVER_ENABLED", "0")) > 0
    PERCEIVER_N_LATENTS = int(getenv("V118_PERCEIVER_N_LATENTS", str(V118_PERCEIVER_N_LATENTS)))
    PER_PHASE_LORAS    = int(getenv("V118_PER_PHASE_LORAS", "0")) > 0
    LORA_RANK          = int(getenv("V118_LORA_RANK", str(V118_LORA_RANK)))
    ...
    print(f"=== v118 training ===")
    print(f"  GATE_PROFILE={GATE_PROFILE}  PERCEIVER={PERCEIVER_ENABLED}({PERCEIVER_N_LATENTS} latents)  "
          f"LORAS={PER_PHASE_LORAS}(rank={LORA_RANK})")
    ...
    print(f"  photon_alpha={PHOTON_ALPHA}  balance_weight={BALANCE_WEIGHT}  "
          f"uncertainty_min={UNCERTAINTY_MIN}")
    ...
    attach_fg_params_v118(
        ...
        waist_dim=WAIST_DIM,
        perceiver_n_latents=PERCEIVER_N_LATENTS,
        lora_rank=LORA_RANK,
    ...
    params   = fg_v118_parameters(model)
    ...
    if ckpt_to_load:
        ...
        load_ckpt_v118(model, ckpt_to_load)
    ...
    step_fn = _compile_jit_fg_step_v118(
    ...
    print(f"\ntraining (gate={GATE_PROFILE}, perceiver={PERCEIVER_ENABLED}, "
          f"loras={PER_PHASE_LORAS}, SBP={sbp_mode})...\n")
    t0 = time.time()
    ...
    for step in range(1, STEPS + 1):
        ...
        if step % LOG_EVERY == 0:
            ...
            extras = ""
            if PERCEIVER_ENABLED and hasattr(model, "fg_v118_perceiver_gate"):
                pg = float(model.fg_v118_perceiver_gate.numpy()[0])
                extras += f"  perc_gate={pg:+.4f}"
                if hasattr(model, "fg_v118_perceiver_W_out"):
                    pwo_norm = float(np.linalg.norm(model.fg_v118_perceiver_W_out.numpy()))
                    extras += f"  perc_W_out={pwo_norm:.3f}"
            ...
            print(
                ...
                f"topo_sim01={sim01:+.3f}  bias_scale={scale:+.4f}  "
                f"Wres_norm={wres_norm:.3f}  "
                f"noisy={noisy_frac:.0%}{extras}  "
        ...
        if step % CKPT_EVERY == 0:
            ...
            safe_save(fg_v118_state_dict(model), ckpt_path)
