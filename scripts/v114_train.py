"""v114 training driver — mirror staging mask (bidirectional BP).

Identical to v112b_train.py except:
  - default CKPT_LABEL = "v114_mirror"
  - default ckpt_dir = ".cache/fg_v114_ckpts/"
  - warm-start from v112b_cont1_final via WARM_FROM=v112b_cont1_final env
  - V114_MIRROR_AT_K env var gates the mirror staging in factor_graph_data_v107

The model is unchanged (fg_breathing_forward_v112b); only the staging mask
construction differs (via _V114_MIRROR_AT_K in factor_graph_data_v107).
"""
import gc
import os
import sys
...
def main():
    ...
    CKPT_LABEL = getenv("CKPT_LABEL",              "v114_mirror")
    ...
    MIRROR_AT_K = int(getenv("V114_MIRROR_AT_K", "0"))
    ...
    print(f"=== v114 training (mirror staging mask) ===")
    print(f"  MIRROR_AT_K={MIRROR_AT_K}  (0=disabled, uses forward-only like v112b)")
    ...
    if ckpt_to_load:
        ...
        load_ckpt_v112b(model, ckpt_to_load)
    ...
    print(f"\ntraining (MIRROR_AT_K={MIRROR_AT_K}, SBP={sbp_mode})...\n")
    t0 = time.time()
