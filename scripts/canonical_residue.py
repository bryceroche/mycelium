...
def len_asc(text):
    ...
    if len(p) <= 3:
        return text
    return " ".join([p[0]] + sorted(p[1:-1], key=len) + [p[-1]])
...
def main():
    ...
    fixture = [{"idx": i,
               ...
               for i in residue]
    json.dump({"ckpt": os.path.basename(ALG_CKPT), "view": "len_asc",
               "n_residue": len(residue), "rows": fixture},
              open(".cache/residue_census.json", "w"), indent=1)
    print(f"[canon] RESIDUE BANKED: {len(residue)} rows -> "
          f".cache/residue_census.json (text-sha keyed)", flush=True)
