"""v112b Monte Carlo BP diagnostic.
...
Usage:
  V110_STEP3_TASK=1 .venv/bin/python scripts/diag_v112b_mcbp.py
...
from scripts.v112b_train import load_ckpt_v112b

from mycelium.factor_graph_v112b import (
    attach_fg_params_v112b,
    compile_jit_mcbp_v112b,
)
...
def main():
    ...
    print("=== v112b Monte Carlo BP diagnostic ===")
    print(f"  ckpt: {CKPT}")
    ...
    load_ckpt_v112b(model, CKPT)
    print(f"  attn_bias_scale value: {float(model.fg_v115_attn_bias_scale.numpy()[0]):+.4f}")
    print(f"  W_res_gate norm:       {float(np.linalg.norm(model.fg_v115_W_res_gate.numpy())):.4f}")
    print()
    ...
    mcbp_eval = compile_jit_mcbp_v112b(
        model, K=K_MAX,
