...
T = json.load(open('.cache/aug_table_v1.json'))["licensed"]
BY = {}
for e in T:
    ...
    for f in facs:
        ...
        try:
            if ft == "given":
                ...
                if a == b:
                    key = "dup"
                    cands = [m for m in pool.get("dup",[]) if ("times" in m or "*" in m) == (f["op"]=="mul")]
                ...
                cands = pool.get(ft,[])
                cands = [m for m in cands if "Half" not in m and "third" not in m] or cands
                fmt = cands[rng.randint(len(cands))]
                ...
                cands = [m for m in pool["sel"] if ("smaller" in m) == (f.get("sel")=="smaller")]
                if not cands: return None
                fmt = cands[rng.randint(len(cands))]
...
for arm, half in (("vlow", True), ("vfull", False)):
    pool = pools(half)
    rng = np.random.RandomState(85000 if half else 86000)
    n_ok = 0
