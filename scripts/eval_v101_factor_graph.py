"""v101 factor graph evaluation: per-difficulty accuracy + K-sweep + depth diagnostic.

Usage:
  python scripts/eval_v101_factor_graph.py CKPT_PATH [--test PATH] [--K N] [--batch N]
...
from mycelium.factor_graph_v101 import (
    V101_K_MAX, V101_N_MAX, V101_F_MAX, V101_WAIST,
    attach_fg_params_v101, fg_v101_state_dict,
...
def load_ckpt_v101(model, path: str):
    ...
    # Report waist key loading status
    v101_keys = [k for k in targets if k.startswith("fg_v101.")]
    loaded    = [k for k in v101_keys if k in sd]
    if not loaded:
        print(f"  INFO: no v101 waist keys in ckpt — using near-identity init")
    else:
        print(f"  loaded {len(loaded)}/{len(v101_keys)} v101 waist keys from ckpt")
...
def run_eval_at_K(model, loader: FactorGraphLoaderV100, K: int,
                  ...
                  f_max: int = V101_F_MAX) -> tuple[dict, dict]:
    """Run eval at a specific K value.

    Returns:
      agg_by_diff  : {difficulty: {cell_acc, query_acc, n_puzzles}}
      agg_by_depth : {depth: {cell_acc, query_acc, n_puzzles, energy}}
    """
    agg_diff  = {}
    agg_depth = {}
...
def main():
    ...
    ap.add_argument("ckpt", help="Path to a .safetensors v101 factor-graph ckpt.")
    ap.add_argument("--test",    default=".cache/factor_graph_test.jsonl")
    ap.add_argument("--K",       type=int, default=None,
                    help="Single K to eval at (default = V101_K_MAX env var or ckpt K).")
    ...
    ap.add_argument("--n_max",   type=int, default=V101_N_MAX)
    ap.add_argument("--f_max",   type=int, default=V101_F_MAX)
    ap.add_argument("--waist",   type=int, default=V101_WAIST)
    args = ap.parse_args()
    ...
    print(f"=== v101 factor graph eval (v100 + waist compression) ===")
    print(f"ckpt={args.ckpt}  test={args.test}")
    print(f"K_sweep={sweep_Ks}  k_alloc={k_alloc}  N_MAX={N_MAX}  F_MAX={F_MAX}  waist={args.waist}")
    ...
    attach_fg_params_v101(model, hidden=cfg.hidden, waist=args.waist)
    Device[Device.DEFAULT].synchronize()

    print(f"loading ckpt {args.ckpt}...")
    load_ckpt_v101(model, args.ckpt)
