...
Output: .cache/book9_drawing_order.json
"""
import json, glob, re
import numpy as np
...
draft_files = sorted(glob.glob(".cache/book8*prose_pairs_draft.jsonl"))
...
json.dump({"note": "GRAPH-SHAPED drawing order: signatures learned from drafted graphs' own "
           ...
           "drawing_order": order},
          open(".cache/book9_drawing_order.json", "w"), indent=1)
print("[cure] wrote .cache/book9_drawing_order.json — t9's gate is open")
