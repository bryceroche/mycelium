...
prior=max(_C(TY[ht:].tolist()).values())/len(TY[ht:])
rng2=np.random.RandomState(7)
ysh=rng2.permutation(ytr)
Ysh=np.eye(24)[ysh]*2-1
Wsh=np.linalg.lstsq(Xs.T@Xs+10.0*np.eye(Xs.shape[1]), Xs.T@Ysh, rcond=None)[0]
prsh=(((TX[ht:]-mu)/sg)@Wsh).argmax(1)
accsh=(prsh==TY[ht:]).mean()
print(f"[control] tail label PRIOR {prior:.3f} | shuffled-label probe on tail {accsh:.3f}")
prh=(((HX-mu)/sg)@W).argmax(1)
print(f"[probe] held-out TAIL first-arg acc {acc:.3f} (n={len(TY)-ht}) | healthy train-fit {(prh==HY).mean():.3f}")
json.dump({"tail_acc":float(acc),"n":int(len(TY)-ht)},open('.cache/pointer_probe_ctl.json','w'))
