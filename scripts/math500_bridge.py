"""math500_bridge.py — THE REGISTER BRIDGE v0 (translation lane + true chain).
From: math500_mouth.py — THE TRUE CHAIN (mouth first).
...
def mouth_pass(texts):
    ...
    return out
def translate(text):
    """v0 prose->register compiler: the linear slice. Returns canonical text
    or None (untranslatable — the original passes through untouched)."""
    t=text
    t=re.sub(r"\$\$?","",t)                      # strip latex fences
    t=re.sub(r"\\(?:cdot|times)"," * ",t)
    t=re.sub(r"\s+"," ",t)
    # collect candidate simple statements: X = N, X + Y = Z, X * Y = Z, N * X = Z
    stmts=re.findall(r"([A-Za-z])\s*=\s*(\d+)(?!\d*\s*[+*])",t)
    rels=re.findall(r"([A-Za-z])\s*([+*])\s*([A-Za-z])\s*=\s*([A-Za-z])",t)
    q=re.search(r"(?:what is|find|value of)\s*\$?([A-Za-z])\b",t,re.I)
    if not q or (not stmts and not rels): return None
    names=[]
    def nm(v):
        if v not in names: names.append(v)
        return chr(ord('a')+names.index(v))
    out=[]
    for v,val in stmts:
        if int(val)>300: return None
        out.append(f"{nm(v)} is {val}.")
    for a,op,b,c in rels:
        w="plus" if op=="+" else "times"
        out.append(f"{nm(a)} {w} {nm(b)} equals {nm(c)}.")
    if q.group(1) not in names: return None
    if len(names)>24: return None
    return " ".join(out)+f" What is {nm(q.group(1))}?"
_trans={i:translate(samples[i]["text"]) for i in range(n)}
_tn=sum(1 for v in _trans.values() if v)
print(f"[m500] THE BRIDGE: translated {_tn}/500 into the register (v0 linear slice)",flush=True)
for i in range(n):
    if _trans[i]: samples[i]["text"]=_trans[i]
