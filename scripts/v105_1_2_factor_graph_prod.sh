#!/usr/bin/env bash
# v105.1.2 factor graph — production run (3000 steps)
#
# Architecture: digit RoPE + projection waist + IB semantic codebook + per-NUMBER aux
#
# Target: 50%+ cell accuracy on full GSM8K val set (including values > 99).
#
# Training:
...
#   - Energy weight 0.01 (differentiable constraint energy)
...
#   v104 (+ IB 32-entry codebook):         K=10 = 47.7% cell acc  (tied v101)
#   v105.1 (digit RoPE, no waist/IB):      TBD
#   v105.1.2 (full stack):                 target >= 50% on full GSM8K val
#
# DO NOT launch without smoke-run passing first.
# Usage: bash scripts/v105_1_2_factor_graph_prod.sh
set -euo pipefail
...
CKPT_LABEL="${CKPT_LABEL:-v105_1_2_prod}" \
PYTHIA_INIT=1 \
RESUME_FROM="$CKPT" \
SUDOKU_TASK=0 \
V99_TASK=0 \
