"""alg4_autopsy.py — the binding constraint's autopsy (2026-07-23).
...
for name, ok in res.items():
    print(f"[autopsy] {name}: {int(ok.sum())}/{n}")
lost = np.where(res["v4"] & ~res["armR"])[0]
gained = np.where(~res["v4"] & res["armR"])[0]
print(f"[autopsy] LOST (v4-right, armR-wrong): {len(lost)} | GAINED: {len(gained)}")
...
json.dump(dict(lost=[shape(int(r)) for r in lost],
               gained=[shape(int(r)) for r in gained],
               totals={k: int(v.sum()) for k, v in res.items()}),
          open(".cache/alg4_autopsy.json", "w"), indent=1)
print("[autopsy] banked -> .cache/alg4_autopsy.json")
