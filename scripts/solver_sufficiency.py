"""trigger_license_v2.py — THE RELATIVE PROXY (2026-08-04; re-registered
pins in ledger): own-value vs RIVAL-number token mass; single-literal
rows reported separately; per-row banked."""
import os, sys, json, re
os.environ["ALG_BREATH"]="3"; os.environ["ALG_RINGS"]="1"
os.environ.setdefault("ALG2","1"); os.environ.setdefault("ALG_FTYPES","8")
os.environ.setdefault("ALG_HW","512"); os.environ.setdefault("ALG_DUP","1")
os.environ.setdefault("ALG_WIDE","1")
os.environ.setdefault("ALG_TEST",".cache/dag8_test.jsonl")
os.environ.setdefault("ALG_TEST_NAME","dag8test")
...
for s0 in range(0,n,8):
    ...
    for bi,i in enumerate(sl):
        ...
        _solver_obj = solve2(facs,q,{"n_vars":24,"m":300}) is None  # unsat/no-solution = OBJECTION
        for f in facs:
            ...
            for mm in re.finditer(r"\d+", smp["text"]):
                if mm.group()==vs: continue
                hs,he=mm.span()
                for ti,(cs,ce) in enumerate(offs[:T_ALG]):
                    if ce>cs and cs<he and ce>hs: rmask[ti]=True
            rmask &= ~mask
            rival = float(a[rmask].sum())
            has_r = bool(rmask.any())
            prox = own/(own+rival+1e-9)
            rows.append({"correct":bool(correct),"proxy":prox,"own":own,
                         "rival":rival,"has_rivals":has_r,"solver_obj":bool(_solver_obj),
...
import numpy as _np
pc=_np.array([x["proxy"] for x in r]); yc=_np.array([x["correct"] for x in r])
flag=pc<0.5
flagged_wrong=[x for x,f_,c_ in zip(r,flag,yc) if f_ and not c_]
n=len(flagged_wrong)
obj=sum(1 for x in flagged_wrong if x["solver_obj"])
print(f"[sufficiency] trigger-flagged WRONG slots n={n}: solver OBJECTS on {obj} ({obj/max(n,1):.3f})")
allw=[x for x,c_ in zip(r,yc) if not c_]
print(f"[context] all wrong literal slots {len(allw)}: solver-objection rate {sum(1 for x in allw if x['solver_obj'])/max(len(allw),1):.3f}; correct-slot objection rate {sum(1 for x,c_ in zip(r,yc) if c_ and x['solver_obj'])/max(int(yc.sum()),1):.3f}")
json.dump({"n_flagged_wrong":n,"solver_obj":obj},open('.cache/solver_sufficiency.json','w'))
