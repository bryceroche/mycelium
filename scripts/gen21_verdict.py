...
GEN = "21"
BARS = [("bigtest", 1227), ("alg4test", 402), ("alg2test", 560),
        ("vtest", 598), ("dagtest", 660), ("dag7btest", 500),
        ("dag8test", 500), ("h3held", 170), ("adupheld", 180)]
BAND_MIN = {"bigtest": 1217, "alg4test": 396}   # bar-noise law: gen-20 record - floor
KILL_FLOOR = 1217
...
for cand in ("H",):
    ...
    print(f"  ZONE DELTA vs gen-20 baseline (998/410/92): {zu-998:+d}/{zp-410:+d}/{zd-92:+d}"
          f"  -> {'ZONE-INVARIANT (the books deepen like spans)' if abs(zu-998)<=8 and abs(zd-92)<=8 else 'ZONE-SHIFTED — the books teach what the training lever did not'}")
...
m["gen_id"] = "21"
m["date"] = "2026-07-23"
m["parser_ckpt"] = ".cache/g21.safetensors"
m["corpora"]["train"] = ".cache/gen21_mix.jsonl"
...
m["notes"] = (f"2026-07-23 GEN-18 PROMOTED: g18b "
              f"({'hot-phase ration + teaching dose'}; "
