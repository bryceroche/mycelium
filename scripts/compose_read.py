...
W_THR=0.0
import glob as _g
_h=[json.loads(l) for l in open('.cache/math_harvest_v0.jsonl')]
_drafted=set()
for _f in _g.glob('.cache/book*_t*_batch*.jsonl'):
    for _l in open(_f):
        if _l.strip(): _drafted.add(json.loads(_l).get("src_idx",-1))
import numpy as _np
_fresh=[]
for _seed in (99,299):
    _r=_np.random.default_rng(_seed)
    _fresh+= [i for i in _r.permutation(len(_h)) if i not in _drafted][:10]
samples=[{"text":_h[i]["problem"],"n_vars":24,"m":300,
          "answer":int(str(_h[i]["answer"]).strip()) if str(_h[i]["answer"]).strip().isdigit() else None,
...
ga={i:samples[i]["answer"] for i in range(n)}
base={}; refusals=[]
for i in range(n):
    facs,q=lane_parse[i]
...
print(f"[compose] lane baseline: right {r0} wrong {w0} refusals {len(refusals)}",flush=True)
ar=0; aw=0; bk=0
for ct,ri in enumerate(refusals):
    ...
    if ct%50==0: print(f"[compose] rescue {ct}/{len(refusals)} (banked {bk})",flush=True)
print(f"[compose] RESCUE: banked {bk} +right {ar} +WRONG {aw}",flush=True)
print(f"[compose] NET: {r0+ar}/{n} (the 20 held-out)  wrongs {w0+aw} ",flush=True)
