...
while False and subprocess.run(["systemctl","--user","is-active","aug-fire.service"],
...
L = "abcdefghij"
TABLE = json.load(open('.cache/aug_table_v2.json'))["licensed"]
for probe in ("plus another", "lots of", "chopped into"):
...
def fdiv_novel(n, seed):
    ...
    while len(rows)<n:
        ...
        text=(f"Consider the numbers a, b, c, d. a is {A}. b is {B}. "
              f"a times b equals c. c chopped into {K} same-size chunks gives d. What is d?")
...
res={}
for name,ck in (("g22",".cache/g22.safetensors"),("vlow",".cache/g24_vlow.safetensors"),("vfull",".cache/g24_vfull.safetensors"),("v120",".cache/g24_v120.safetensors")):
    p=load(ck)
...
json.dump(res,open('.cache/aug_after_v2.json','w'),indent=1)
print("[saved] .cache/aug_after_v2.json — phase B (flip rate, 2b slot emission) rides next")
