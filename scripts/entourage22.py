"""entourage22.py — THE ESTATE SETTLEMENT FOR GEN-22 (2026-07-30):
an unattributable gate reads nothing. Adapted from entourage21 with its
fossils fixed: (a) centroids/mouth read the G22-ALIGNED state pair
(phase1_alg_states_g22.npz + _g22_states.npy, 82,400 rows, verified at
the fire — entourage21 mixed a g21 npz with g20 states and survived by
...
-> disjoint census -> dissent-overlap (g21 vs g22) -> collapse re-read
...
print("=== E22 1/10: fresh repair corpora ===", flush=True)
sh(".venv/bin/python3 scripts/algebra_nl_gen.py --n 800 --seed 171 --out .cache/g22r_v1.jsonl --teeth 0.8", tail=1)
sh(".venv/bin/python3 scripts/algebra2_nl_gen.py --n 800 --seed 172 --out .cache/g22r_v2.jsonl --teeth 0.8 --token-budget 250", tail=1)
sh(".venv/bin/python3 scripts/algebra3_nl_gen.py --n 800 --seed 173 --out .cache/g22r_v3.jsonl --teeth 0.8 --token-budget 250", tail=1)
sh(".venv/bin/python3 scripts/algebra_verbose_gen.py 600 174 .cache/g22r_vb", tail=1)
sh(".venv/bin/python3 scripts/algebra_dag7_gen.py 800 175 .cache/g22r_dag.jsonl", tail=1)
sh("cat .cache/g22r_v1.jsonl .cache/g22r_v2.jsonl .cache/g22r_v3.jsonl "
   ".cache/g22r_vb_verbose.jsonl .cache/g22r_dag.jsonl > .cache/gen22_repair.jsonl "
   "&& wc -l .cache/gen22_repair.jsonl", tail=1)

print("=== E22 2/10: precompute repair states ===", flush=True)
sh(".venv/bin/python3 scripts/phase1_algebra_head.py --precompute",
   {"ALG_TRAIN": ".cache/gen22_repair.jsonl", "ALG_TRAIN_NAME": "gen22repair",
    "PRECOMPUTE_ONLY": "gen22repair"}, tail=1)

print("=== E22 3/10: specialist remine vs g22 ===", flush=True)
e = {"ALG_TRAIN": ".cache/gen22_repair.jsonl", "ALG_TRAIN_NAME": "gen22repair",
...
print("=== E22 4/10: monitor centroids (g22 fst space, g22 family) ===", flush=True)
S4 = r'''
...
np.savez(".cache/monitor_centroids_gen22.npz", **lib)
print("centroids:", sorted(lib))
'''
open(".cache/_e22_s4.py", "w").write(S4)
sh(".venv/bin/python3 .cache/_e22_s4.py", tail=1)

print("=== E22 5/10: mouth rebuild (g22 family) + length refit ===", flush=True)
S5 = r'''
import sys, os, json; sys.path.insert(0,"."); sys.path.insert(0,"scripts")
...
fam, _ = pooled_npz(".cache/phase1_alg_states_g22.npz",
                    ".cache/phase1_alg_states_g22_states.npy", cap=12000)
...
np.savez(".cache/recognition_mouth_gen22.npz", bank=bank,
         thr_knn=np.float32(thr), coef=coef.astype(np.float32))
print(f"[mouth-22] bank rebuilt (g22 family); length-controlled thr {thr:.4f}")
'''
open(".cache/_e22_s5.py", "w").write(S5)
sh(".venv/bin/python3 .cache/_e22_s5.py", tail=1)

print("=== E22 6/10: disjoint census under the fresh mouth ===", flush=True)
sh(".venv/bin/python3 scripts/gen11_census.py",
   {"GATE_CKPT": PARSER, "CENSUS_DISJOINT": "1"}, tail=2)

print("=== E22 7/10: DISSENT-OVERLAP READ (manifest panel) ===", flush=True)
S7 = r'''
import json
...
def dissent_set(gate_votes):
    ...
    for i in range(1500):
        ...
        if gc == 5 and not (at == gt and ct == gt):
            out.add(i)
    return out
d21 = dissent_set(json.load(open(".cache/lattice_gen21_H.json"))["bigtest"])
d22 = dissent_set(json.load(open(".cache/lattice_gen22_H.json"))["bigtest"])
ov = d21 & d22
print(f"[dissent-overlap] gen-21: {len(d21)} | gen-22: {len(d22)} | "
      f"OVERLAP {len(ov)} — "
      f"{'STABLE dissent family (structural)' if len(ov) >= 0.5*max(len(d22),1) else 'dissent ROTATES (population-driven)'}")
json.dump({"d21": sorted(d21), "d22": sorted(d22), "overlap": sorted(ov)},
          open(".cache/dissent_overlap_22.json", "w"))
'''
open(".cache/_e22_s7.py", "w").write(S7)
sh(".venv/bin/python3 .cache/_e22_s7.py", tail=1)

print("=== E22 8/10: collapse re-read (dashboard accrual) ===", flush=True)
sh(".venv/bin/python3 scripts/collapse_probe.py",
   {"COLLAPSE_CKPT": PARSER}, tail=3)

print("=== E22 9/10: manifest member refresh + RE-HASH ===", flush=True)
m = json.load(open(".cache/GENERATION.json"))
m["specialist_ckpt"] = NEW_NACK
m["monitor_centroids"] = ".cache/monitor_centroids_gen22.npz"
m["mouth"] = ".cache/recognition_mouth_gen22.npz"
...
m["notes"] = (m.get("notes", "") +
              " | 2026-07-30 ENTOURAGE-22 PAID (entourage22.py): specialist "
...
print("=== E22 10/10: THE STANDING REHEARSAL (outside air, both tiers) ===", flush=True)
sh(".venv/bin/python3 scripts/wild_frontier_fixture.py",
   {"WFF_CKPT": PARSER, "WFF_OUT": ".cache/wild_frontier_fixture_gen22_entourage.json"},
   tail=4)
print("=== ENTOURAGE-22 SETTLED — the gate may read ===", flush=True)
