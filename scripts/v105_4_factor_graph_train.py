"""v105.4 factor graph training driver — v105.3 + hierarchical codebooks.
...
+ AR digit decoding) with four targeted additions:
  ...
  1. Magnitude head — per-cell 4-way classification of "how many digits is
     ...
     V105_4_MAGNITUDE_WEIGHT (default 0.3).
...
Warm-start from v104_prod_step3000.safetensors:
  ...
  - Copies fg_v104.codebook → fg_v105_4.ib_codebook
...
Env vars (subset; full list in factor_graph_v105_4.py module docstring):
  V105_4_TASK=1
  V105_4_K_MAX=8
  V105_4_N_DIGITS=5
  V105_4_N_MAX=16
...
from mycelium.factor_graph_v105_4 import (
    V105_4_K_MAX, V105_4_N_MAX, V105_4_F_MAX, V105_4_N_DIGITS, V105_4_N_HEADS,
    V105_4_ENERGY_WEIGHT, V105_4_FACTOR_AUX_WEIGHT, V105_4_CALIB_WEIGHT,
    V105_4_MAGNITUDE_WEIGHT, V105_4_N_MAGNITUDE,
    V105_4_WAIST, V105_4_CODEBOOK_N,
    attach_fg_params_v105_4, fg_v105_4_parameters, fg_v105_4_state_dict,
    fg_breathing_forward_v105_4, load_ckpt_v105_4,
    _compile_jit_fg_step_v105_4, _compile_jit_fg_eval_v105_4,
    value_to_digits_lsd, digits_to_value_lsd,
)
# v105.4 data loaders: LSD-first encoding + digit_valid_mask + magnitude_target
from mycelium.factor_graph_data_v105_4 import (
    FactorGraphLoaderV105_4, DualDataLoaderV105_4, load_gsm8k_records_v105_4,
)
...
def collect_fg_params_v105_4(model) -> list[Tensor]:
    """Trainable params: shared L0-L3 attn/FFN + v105.4 fg-specific params."""
    params: list[Tensor] = []
    sw = model.block.shared
    ...
    params += fg_v105_4_parameters(model)
    return params


def model_state_dict_v105_4(model) -> dict:
    sd = {
        "ln_f.g": model.ln_f_g,
        "ln_f.b": model.ln_f_b,
    }
    ...
    sd.update(fg_v105_4_state_dict(model))
    return sd


def evaluate_v105_4(
    model, loader: FactorGraphLoaderV105_4,
    ...
    for batch in loader.iter_eval(batch_size=loader.batch_size):
        ...
        if eval_fn is not None:
            ...
            dig_lh, _, _, _ = fg_breathing_forward_v105_4(
...
def main():
    V105_4_TASK_LOCAL = int(getenv("V105_4_TASK", 0)) > 0
    assert V105_4_TASK_LOCAL, "V105_4_TASK=1 must be set"

    K        = int(getenv("V105_4_K_MAX",    str(V105_4_K_MAX)))
    N_DIGITS = int(getenv("V105_4_N_DIGITS", str(V105_4_N_DIGITS)))
    N_MAX    = int(getenv("V105_4_N_MAX",    str(V105_4_N_MAX)))
    F_MAX    = int(getenv("V105_4_F_MAX",    str(V105_4_F_MAX)))
    WAIST    = int(getenv("V105_4_WAIST",    str(V105_4_WAIST)))
    N_CODE   = int(getenv("V105_4_CODEBOOK_N", str(V105_4_CODEBOOK_N)))
    ...
    CKPT_LABEL       = getenv("CKPT_LABEL",             "v105_4_prod")
    ...
    ENERGY_WEIGHT     = float(getenv("V105_4_ENERGY_WEIGHT",     str(V105_4_ENERGY_WEIGHT)))
    FACTOR_AUX_WEIGHT = float(getenv("V105_4_FACTOR_AUX_WEIGHT", str(V105_4_FACTOR_AUX_WEIGHT)))
    CALIB_WEIGHT      = float(getenv("V105_4_CALIB_WEIGHT",      str(V105_4_CALIB_WEIGHT)))
    MAGNITUDE_WEIGHT  = float(getenv("V105_4_MAGNITUDE_WEIGHT",  str(V105_4_MAGNITUDE_WEIGHT)))
    ...
    print("=== v105.4 factor graph training (v105.3 + hierarchical codebooks + magnitude head) ===")
    ...
    if RESUME_FROM:
        ...
        load_ckpt_v105_4(model, RESUME_FROM)

    params   = collect_fg_params_v105_4(model)
    ...
    synth_loader = FactorGraphLoaderV105_4(
        TRAIN_PATH, batch_size=BATCH,
        difficulty_filter=DIFFICULTY_FILTER,
        curriculum=CURRICULUM,
        curriculum_anneal_steps=CURRICULUM_ANNEAL,
        n_max=N_MAX, f_max=F_MAX, k_max=K, n_heads=V105_4_N_HEADS,
        n_digits=N_DIGITS, seed=SEED,
    )

    gsm8k_records = load_gsm8k_records_v105_4(
        GSM8K_PATH, n_digits=N_DIGITS, n_max=N_MAX, f_max=F_MAX,
    )

    dual_loader = DualDataLoaderV105_4(
    ...
    ckpt_dir = ".cache/fg_v105_4_ckpts"
    os.makedirs(ckpt_dir, exist_ok=True)

    Tensor.training = True
    step_fn = _compile_jit_fg_step_v105_4(
        ...
        n_magnitude=V105_4_N_MAGNITUDE,
        grad_clip=1.0,
    )
    eval_fn = _compile_jit_fg_eval_v105_4(
    ...
    print(f"\ntraining...\n")
    t0 = time.time()
    log_loss = log_ce = log_calib = log_aux = log_energy = log_mag = log_magacc = log_n = 0.0
    ...
    for step in range(1, STEPS + 1):
        ...
        if step % EVAL_EVERY == 0:
            ...
            try:
                dg_main  = model.fg_v105_4_delta_gate.numpy().tolist()
                dg_quant = model.fg_v105_4_delta_gate_quant.numpy().tolist()
                print(
                    "  delta_gate      : "
        ...
        if step % CKPT_EVERY == 0:
            ...
            safe_save(model_state_dict_v105_4(model), ckpt_path)
