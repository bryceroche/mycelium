...
Output artifacts:
  ...
  .cache/v200_smoke/train_237_5.log             — first line ADVISORY: ...
  .cache/v200_smoke/step2000_eval_237_5.json      — final eval + all reads
  .cache/v200_smoke/instrumentation_237_5.json    — per-checkpoint metric timeseries
  .cache/v200_smoke/grad_norms_237_5.npz
  .cache/v200_smoke/persistence/step{N}_z_237_5.npz       — per eval checkpoint
  .cache/v200_perceiver_ckpts/v200_perceiver_237_5_substrate_step{N}.safetensors

Usage:
  cd /home/bryce/mycelium
  bash scripts/v200_resmoke_237_5.sh
"""
...
def main() -> None:
    ...
    CKPT_LABEL = getenv("CKPT_LABEL",  "v200_perceiver_237_5_substrate")
    SMOKE_DIR  = getenv("SMOKE_DIR",   ".cache/v200_smoke")
    REF_DIR    = os.path.join(SMOKE_DIR, "reference_curves")

    SMOKE_LOG_PATH  = os.path.join(SMOKE_DIR, "train_237_5.log")
    EVAL_JSON_PATH  = os.path.join(SMOKE_DIR, f"step{STEPS}_eval_237_5.json")
    INSTR_JSON_PATH = os.path.join(SMOKE_DIR, "instrumentation_237_5.json")
    GRAD_NORMS_PATH = os.path.join(SMOKE_DIR, "grad_norms_237_5.npz")
    ...
    config_sig  = f"K{K}_L{N_LATENTS}_prenorm5_seamthree_gate-2_mask1a_dsnorm-fp32chain-whereguard"
    ...
    if advisory_prefix:
        log(advisory_prefix)
    log("=" * 72)
    log("v200 #237.5 — SUBSTRATE RESTORATION (§1A.E.14)")
    log("  Fixes vs #237 (all spec-restoration class, bundled): detached-scale seam norms")
    log("  (forward byte-identical, flat direction closed), fp32 inter-breath chain,")
    log("  where-gated NaN guard. Fresh Adam moments. Same seed/steps/config as #237.")
    log("  P1 (in-run): breath_embed+alpha_read grad norms hold >1e-5 through 2000.")
    log("  P2 (BINDING, post-run sweep): per-token rho sustains past step 300 — the causal test.")
    log("  P3: live organ rebuilds the carrier? recurrent form-and-dissolve => wants WRITE.")
    log("  #237 (frozen organ) is this run's control; the diff scores every cell.")
    ...
    def _persist_z(probe: dict, step_num: int) -> None:
        ...
        eff_gate = 1.0 / (1.0 + np.exp(-np.asarray(
            ...
            dtype=np.float32)[:K]))
        z_path = os.path.join(PERSIST_DIR, f"step{step_num}_z_237_5.npz")
        np.savez(
            z_path,
    ...
    def _run_checkpoint_instrumentation(step_num: int) -> None:
        ...
        log(f"  ladder_slope={slope:.5f} (target <= -0.05)  "
    ...
    # _persist_z (persistence/step{N}_z_237_5.npz + provenance), incl. step 0
    ...
    # C4: CE ladder slope — BINDS AT STEP 1000 per the E.10 pre-commit
    # ("slope <= -0.05 by step 1000"). Review fix: binding on the final step
    # would be result-flattering drift from the locked pre-registration; the
    # step-1000 checkpoint's slope is the binding value, final is confirmatory.
    log("\nCriterion 4 — Per-breath CE ladder slope <= -0.05 (binds at step 1000):")
    c4_bind_entry = next(
        (e for e in checkpoint_metrics
         if e["step"] >= 1000 and e["ladder_slope"] is not None),
        None,
    )
    if c4_bind_entry is not None:
        c4_bind_step  = c4_bind_entry["step"]
        c4_bind_slope = c4_bind_entry["ladder_slope"]
    else:
        # Horizon shorter than 1000 (env override) — fall back to final, ADVISORY-style.
        c4_bind_step  = STEPS
        c4_bind_slope = ladder_slope
        log("  WARNING: no checkpoint at step >= 1000 — binding on final step instead")
    log(f"  binding slope (step {c4_bind_step}): {float(c4_bind_slope):.5f}")
    log(f"  final slope   (step {STEPS}): {ladder_slope:.5f}  (confirmatory, not binding)")
    ...
    log(f"  slope trajectory: {[(s, f'{v:.5f}') for s, v in slope_traj]}")
    crit4 = bool(np.isfinite(c4_bind_slope) and (c4_bind_slope <= -0.05))
    ...
    if n_pass == 6:
        final_line = (
            f"{adv_line_prefix}#237.5 SUBSTRATE RUN PASSED  run=237.5  " + common_tail + "  all6=YES"
        )
    else:
        failed = [f"C{i+1}" for i, c in enumerate(criteria_passed) if not c]
        final_line = (
            f"{adv_line_prefix}#237.5 SUBSTRATE RUN FAILED  run=237.5  failed={','.join(failed)}  "
            + common_tail
        )
    ...
    eval_json = {
        ...
        "c4_binding": {
            ...
            "binding_slope": float(c4_bind_slope),
            "final_slope_confirmatory": ladder_slope,
            "pre_commit": "E.10: slope <= -0.05 by step 1000",
        },
    ...
    print(f"Latent z bundles: {PERSIST_DIR}/step{{N}}_z_237_5.npz")
