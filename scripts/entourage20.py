...
crown_v4 corpus, E20 seeds; NEW STAGE: the collapse-ratio re-read under
v4 (the pocket dashboard's first accrual — the quotient-deepening
question's first datapoint).
Stages: repair corpora -> states -> specialist remine vs gen-16 ->
centroids (gen-16 fst, g20fam family) -> mouth rebuild (g20fam family) ->
...
print("=== E20 1/8: fresh repair corpora ===", flush=True)
...
sh("cat .cache/g16r_v1.jsonl .cache/g16r_v2.jsonl .cache/g16r_v3.jsonl "
   ".cache/g16r_vb_verbose.jsonl .cache/g16r_dag.jsonl > .cache/gen20_repair.jsonl "
   "&& wc -l .cache/gen20_repair.jsonl", tail=1)

print("=== E20 2/8: precompute repair states ===", flush=True)
sh(".venv/bin/python3 scripts/phase1_algebra_head.py --precompute",
   {"ALG_TRAIN": ".cache/gen20_repair.jsonl", "ALG_TRAIN_NAME": "gen20repair",
    "PRECOMPUTE_ONLY": "gen20repair"}, tail=1)

print("=== E20 3/8: specialist remine vs gen-16 ===", flush=True)
e = {"ALG_TRAIN": ".cache/gen20_repair.jsonl", "ALG_TRAIN_NAME": "gen20repair",
...
print("=== E20 4/8: monitor centroids (gen-16 fst space, g20fam family) ===", flush=True)
S4 = r'''
...
np.savez(".cache/monitor_centroids_gen20.npz", **lib)
print("centroids:", sorted(lib))
'''
open(".cache/_e16_s4.py", "w").write(S4)
sh(".venv/bin/python3 .cache/_e16_s4.py", tail=1)

print("=== E20 5/8: mouth rebuild (g20fam family) + length refit ===", flush=True)
S5 = r'''
import sys, os, json; sys.path.insert(0,"."); sys.path.insert(0,"scripts")
...
np.savez(".cache/recognition_mouth_gen20.npz", bank=bank,
         thr_knn=np.float32(thr), coef=coef.astype(np.float32))
print(f"[mouth-16] bank rebuilt (g20fam family); length-controlled thr {thr:.4f}")
'''
open(".cache/_e16_s5.py", "w").write(S5)
sh(".venv/bin/python3 .cache/_e16_s5.py", tail=1)

print("=== E20 6/8: disjoint census under the fresh mouth ===", flush=True)
