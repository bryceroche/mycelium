#!/bin/bash
# v89 eval — eval_v85_dag.py on the v89 step N ckpt.
set -e
cd "$(dirname "$0")/.."

V89_CKPT="${V89_CKPT:-.cache/gsm8k_steps_ckpts/v89_smoke_step100.safetensors}"
NUM_EVAL="${NUM_EVAL:-60}"

# Same env needed for model construction (matches v89_smoke_train.sh).
...
# v89 — same flags so the model has the split args1/args2 cross-attn at eval.
...
/home/bryce/mycelium/.venv/bin/python -u scripts/eval_v85_dag.py 2>&1 | tee .cache/v89_smoke_eval.log
