...
rp = json.load(open(".cache/residue_portrait.json"))
residue = [i for i in rp["residue_items"] if ok[i]]
fixture = json.load(open(".cache/flux_continuity_audit.json"))["repair_intake"]
recovered = [i for i in fixture if i not in set(rp["residue_items"]) and ok[i]]
cold, corr = residue, recovered
print(f"[diff] populations: residue {len(cold)} | recovered-abstains {len(corr)}")
...
verdict = ("LIGHTS — deep deceptions fool the apertures DIFFERENTLY; the "
           ...
           "FLAT — the residue deception is aperture-invariant; guard+books routing takes its strong sentence")
