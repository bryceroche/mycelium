"""v100 factor graph evaluation: per-difficulty accuracy + K-sweep + depth diagnostic.

Usage:
  python scripts/eval_v100_factor_graph.py CKPT_PATH [--test PATH] [--K N] [--batch N]
...
from mycelium.factor_graph_v100 import (
    V100_K_MAX, V100_N_MAX, V100_F_MAX,
...
def compute_query_depth(rec) -> int:
    """Compute the DAG depth of the query variable in a single record."""
...
def run_eval_at_K(model, loader: FactorGraphLoaderV100, K: int,
                  ...
                  f_max: int = V100_F_MAX) -> tuple[dict, dict]:
...
def main():
    ...
    ap.add_argument("ckpt", help="Path to a .safetensors v100 factor-graph ckpt.")
    ap.add_argument("--test",    default=".cache/factor_graph_test.jsonl")
    ap.add_argument("--K",       type=int, default=None,
                    help="Single K to eval at (default = V100_K_MAX env var or ckpt K).")
    ...
    ap.add_argument("--k_alloc", type=int, default=None,
                    help="K_max for model allocation (must match ckpt). Defaults to max(K_sweep).")
    ap.add_argument("--n",       type=int, default=500, help="Max test records per difficulty.")
    ap.add_argument("--batch",   type=int, default=8)
    ap.add_argument("--n_max",   type=int, default=V100_N_MAX)
    ap.add_argument("--f_max",   type=int, default=V100_F_MAX)
    args = ap.parse_args()
    ...
    print(f"=== v100 factor graph eval ===")
    ...
    load_ckpt_v100(model, args.ckpt)
    ...
    # K-sweep summary: should show improving accuracy with more breaths (especially for deep problems)
    if len(sweep_Ks) > 1:
        print(f"\n=== K-sweep summary ===")
