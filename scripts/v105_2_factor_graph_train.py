"""v105.2 factor graph training driver — LSD-first + digit_valid_mask.

Changes from v105.1.2 trainer:
  - Uses factor_graph_v105_2 model + factor_graph_data_v105_2 loaders.
  - Threads `digit_valid_mask` and `factor_digit_valid_mask` through JIT step+eval.
  - All env vars renamed V105_1_2_* → V105_2_*.
  - Checkpoints saved under .cache/fg_v105_2_ckpts/.

The architecture differences are documented in mycelium/factor_graph_v105_2.py.
...
from mycelium.factor_graph_v105_2 import (
    V105_2_K_MAX, V105_2_N_MAX, V105_2_F_MAX, V105_2_N_DIGITS, V105_2_N_HEADS,
    V105_2_ENERGY_WEIGHT, V105_2_FACTOR_AUX_WEIGHT, V105_2_CALIB_WEIGHT,
    V105_2_WAIST, V105_2_CODEBOOK_N,
    attach_fg_params_v105_2, fg_v105_2_parameters, fg_v105_2_state_dict,
    fg_breathing_forward_v105_2, load_ckpt_v105_2,
    _compile_jit_fg_step_v105_2, _compile_jit_fg_eval_v105_2,
    value_to_digits_lsd, digits_to_value_lsd,
)
from mycelium.factor_graph_data_v105_2 import (
    FactorGraphLoaderV105_2, DualDataLoaderV105_2, load_gsm8k_records_v105_2,
)
...
def collect_fg_params_v105_2(model) -> list[Tensor]:
    """Trainable params: shared L0-L3 attn/FFN + v105.2 fg-specific params."""
    params: list[Tensor] = []
    sw = model.block.shared
    ...
    params += fg_v105_2_parameters(model)
    return params


def model_state_dict_v105_2(model) -> dict:
    sd = {
        "ln_f.g": model.ln_f_g,
        "ln_f.b": model.ln_f_b,
    }
    ...
    sd.update(fg_v105_2_state_dict(model))
    return sd


def evaluate_v105_2(
    model, loader: FactorGraphLoaderV105_2,
    ...
    those above the natural digit count, are treated as automatically correct).
    """
    ...
    for batch in loader.iter_eval(batch_size=loader.batch_size):
        ...
        for b in range(len(picks)):
            ...
            for vi in range(min(nv, n_max)):
                if obs_np[b, vi] == 0:
                    agg[diff]["n_unobs"] += 1
                    # Variable correct if every VALID digit position matches.
...
def main():
    V105_2_TASK_LOCAL = int(getenv("V105_2_TASK", 0)) > 0
    assert V105_2_TASK_LOCAL, "V105_2_TASK=1 must be set"

    K        = int(getenv("V105_2_K_MAX",    str(V105_2_K_MAX)))
    N_DIGITS = int(getenv("V105_2_N_DIGITS", str(V105_2_N_DIGITS)))
    N_MAX    = int(getenv("V105_2_N_MAX",    str(V105_2_N_MAX)))
    F_MAX    = int(getenv("V105_2_F_MAX",    str(V105_2_F_MAX)))
    WAIST    = int(getenv("V105_2_WAIST",    str(V105_2_WAIST)))
    N_CODE   = int(getenv("V105_2_CODEBOOK_N", str(V105_2_CODEBOOK_N)))
    ...
    CKPT_LABEL       = getenv("CKPT_LABEL",             "v105_2_prod")
    ...
    ENERGY_WEIGHT     = float(getenv("V105_2_ENERGY_WEIGHT",     str(V105_2_ENERGY_WEIGHT)))
    FACTOR_AUX_WEIGHT = float(getenv("V105_2_FACTOR_AUX_WEIGHT", str(V105_2_FACTOR_AUX_WEIGHT)))
    CALIB_WEIGHT      = float(getenv("V105_2_CALIB_WEIGHT",      str(V105_2_CALIB_WEIGHT)))
    ...
    print("=== v105.2 factor graph training (LSD-first + digit_valid_mask) ===")
    ...
    if RESUME_FROM:
        ...
        load_ckpt_v105_2(model, RESUME_FROM)

    params   = collect_fg_params_v105_2(model)
    ...
    synth_loader = FactorGraphLoaderV105_2(
        ...
        n_max=N_MAX, f_max=F_MAX, k_max=K, n_heads=V105_2_N_HEADS,
        n_digits=N_DIGITS, seed=SEED,
    )

    gsm8k_records = load_gsm8k_records_v105_2(
        GSM8K_PATH, n_digits=N_DIGITS, n_max=N_MAX, f_max=F_MAX,
    )

    dual_loader = DualDataLoaderV105_2(
    ...
    ckpt_dir = ".cache/fg_v105_2_ckpts"
    os.makedirs(ckpt_dir, exist_ok=True)

    Tensor.training = True
    step_fn = _compile_jit_fg_step_v105_2(
    ...
    eval_fn = _compile_jit_fg_eval_v105_2(
    ...
    for step in range(1, STEPS + 1):
        ...
        if step % EVAL_EVERY == 0:
            ...
            try:
                dg_main  = model.fg_v105_2_delta_gate.numpy().tolist()
                dg_quant = model.fg_v105_2_delta_gate_quant.numpy().tolist()
                print(
                    "  delta_gate      : "
        ...
        if step % CKPT_EVERY == 0:
            ...
            safe_save(model_state_dict_v105_2(model), ckpt_path)
