...
CKPT      = getenv("CKPT", ".cache/fg_v105_4_ckpts/v105_4_smoke_final.safetensors")
...
def _collect(loader, model, label):
    ...
    cells_all_correct = 0
    cells_total = 0
    # NEW v105.4: magnitude prediction tracking
    N_MAG = 4
    ...
    for batch in loader.iter_eval(batch_size=loader.batch_size):
        dig_lh, _, _, mag_lh = fg_breathing_forward_v105_4(
...
def main():
    print("=== v105.4 per-position + magnitude diagnostic (LSD-first) ===",
          flush=True)
    ...
    train_loader = FactorGraphLoaderV105_4(
    ...
    # NEW v105.4: magnitude accuracy + confusion matrix
    print("", flush=True)
    print("=== MAGNITUDE HEAD ACCURACY (NEW v105.4) ===", flush=True)
