...
def run_model(ck, want_states):
    ...
    for s0 in range(0,len(samples),8):
        ...
        for bi,ri in enumerate(sl):
            ...
            hits[int(ri)]=(a==smp["solution"][smp["query_var"]])
            if want_states and ORD.search(smp["text"]):
                for j in range(24):
                    if gold["presence"][ri,j]<=0 or gold["ftype"][ri,j]!=0: continue
                    gs=sorted(np.where(gold["args"][ri,j]>0.5)[0].tolist())
                    if len(gs)!=2: continue
                    SS[(int(ri),j)]=(onp["fst_s"][bi,j],gs[0])
            elif want_states and hits[int(ri)]:
                for j in range(24):
                    if gold["presence"][ri,j]<=0 or gold["ftype"][ri,j]!=0: continue
                    gs=sorted(np.where(gold["args"][ri,j]>0.5)[0].tolist())
                    if len(gs)!=2 or len([1 for k2 in SS if k2[0]==-1])>0: continue
                    if sum(1 for k2 in SS if k2[1]=="h")<1000: SS[(-int(ri)-1,j)]=(onp["fst_s"][bi,j],gs[0])
    return hits, SS
base_hits,_=run_model(".cache/g23v5.safetensors", False)
json.dump({str(k):bool(v) for k,v in base_hits.items()}, open('.cache/base_partition.json','w'))
print("[frozen] baseline partition banked")
_,SS=run_model(".cache/g35_size8x.safetensors", True)
HX=[];HY=[];PB={"pass":([],[]),"miss":([],[])}
for (ri,j),(x,y) in SS.items():
    if ri<0: HX.append(x); HY.append(y); continue
    g_="pass" if base_hits[ri] else "miss"
    PB[g_][0].append(x); PB[g_][1].append(y)
HX=np.array(HX);HY=np.array(HY)
if len(HX)<200:
    # healthy pool from non-ordinal passers gathered above may be small; fall back to pass-band train
    HX=np.array(PB["pass"][0][:800]); HY=np.array(PB["pass"][1][:800])
...
for g_ in ("pass","miss"):
    ...
    print(f"[FROZEN {g_}] slots {len(X)} acc {(pr==yv).mean():.3f}  (baseline-partition bands)")
