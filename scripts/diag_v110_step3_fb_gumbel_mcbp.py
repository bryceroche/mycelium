...
from mycelium.factor_graph_v110_step3_fb_gumbel import (
    attach_fg_params_v110_step3_fb_gumbel as attach_fg_params_v110_step3_fb,
    compile_jit_mcbp_fb_gumbel,
)
...
def main():
    ...
    GUMBEL_TAU = float(os.environ.get("V110_STEP3_FB_GUMBEL_TAU", "1.0"))
    mcbp_eval = compile_jit_mcbp_fb_gumbel(
        model, K=K_MAX, tau=GUMBEL_TAU,
    ...
    print(f"  gumbel_tau: {GUMBEL_TAU}")
    # Gumbel noise tensor — always zeros at inference (deterministic ST-argmax)
    gumbel_zeros_np = np.zeros((K_MAX, 1, V110_STEP3_N_MAX, V110_STEP3_N_DIGITS, 10), dtype=np.float16)
    gumbel_zeros_t  = Tensor(gumbel_zeros_np).cast(dtypes.half).contiguous().realize()
    ...
    for i, rec in enumerate(records):
        ...
        out = mcbp_eval(di, nk, sm, hm, noise_t, ns_zero_t, gumbel_zeros_t)
        tl = out.numpy()
    ...
    for ns_value in NOISE_SCALES:
        ...
        for i, rec in enumerate(records):
            ...
            for n in range(N_SAMPLES):
                ...
                out = mcbp_eval(di, nk, sm, hm, noise_t, ns_t, gumbel_zeros_t)
