...
def main():
    ...
    for s0 in range(0, len(ems), 8):
        ...
        for i, r in enumerate(sl):
            ...
            stats.setdefault(key, []).append(
                (concord, ans["A"] == r["top"], ans["B"] == r["top"]))
    print("tag   class  n   concord  laneA  laneB")
    kept = {"r": 0, "l": 0}
    for (tag, right), xs in sorted(stats.items()):
        ...
        print(f"{tag:5s} {cls:5s} {n:3d}   {c}/{n}     {a}/{n}   {b}/{n}", flush=True)
        if tag == "gold":
            kept["r" if right else "l"] = c
    net = kept["r"] - kept["l"]
    ok = net >= 3 and kept["r"] >= 6
    print(f"[verdict] veto-applied gold: rights kept {kept['r']}, lies kept "
          f"{kept['l']}, net {net:+d} (bar: net >= +3 AND rights >= 6) -> "
