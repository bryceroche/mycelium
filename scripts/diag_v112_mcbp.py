"""v112 Monte Carlo BP diagnostic.

Same MC-BP framework as diag_v110_step3_fb_mcbp.py: runs N independent
stochastic BP forwards with Gaussian noise injection, averages tree_logits
BEFORE argmax, compares to deterministic BP.

Usage:
  V110_STEP3_TASK=1 .venv/bin/python scripts/diag_v112_mcbp.py

Env overrides:
  CKPT                          (.cache/fg_v112_ckpts/v112_coldstart_final.safetensors)
...
from scripts.v112_train import load_ckpt_v112

from mycelium.factor_graph_v112 import (
    attach_fg_params_v112,
    compile_jit_mcbp_v112,
)
...
def _predict_values_from_tree_logits(tree_logits_np):
    ...
    digits = tree_logits_np.argmax(axis=-1).astype(np.int64)
    return digits_to_value_msd(digits)
...
def main():
    CKPT = os.environ.get(
        "CKPT",
        ".cache/fg_v112_ckpts/v112_coldstart_final.safetensors",
    )
    ...
    print("=== v112 dual-notebook Monte Carlo BP diagnostic ===")
    print(f"  ckpt: {CKPT}")
    ...
    mcbp_eval = compile_jit_mcbp_v112(
        model, K=K_MAX,
