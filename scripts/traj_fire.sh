...
env ALG_FTYPES=9 $(echo $2 | sed "s/_AND_SIXWAVE/ ALG_SIXWAVE=1/") ALG_BREATH=3 BREATH_NORM=1 RESUME=1 \
...
env ALG_FTYPES=9 $(echo $2 | sed "s/_AND_SIXWAVE/ ALG_SIXWAVE=1/") ALG_BREATH=3 BREATH_NORM=1 ALG_CKPT=.cache/$1.safetensors ALG_TEST=.cache/algebra_nl_test.jsonl ALG_TEST_NAME=test23 $PY scripts/phase1_algebra_head.py --eval | grep TOTAL
env ALG_FTYPES=9 $(echo $2 | sed "s/_AND_SIXWAVE/ ALG_SIXWAVE=1/") ALG_BREATH=3 BREATH_NORM=1 CK=.cache/$1.safetensors $PY scripts/dup_axis_scan2.py | grep "^\[scan\]"
done
