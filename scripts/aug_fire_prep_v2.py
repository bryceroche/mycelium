...
T = json.load(open('.cache/aug_table_v2.json'))["licensed"]
BY = {}
for e in T:
    ...
    for f in facs:
        ...
        try:
            if ft == "given":
                ...
                if a == b:
                    key = "dup"
                    cands = [m for m in pool.get("dup",[]) if any(w in m for w in ("times","*","square","multiplied","x ")) == (f["op"]=="mul")]
...
for arm, half in (("v120", False),):
    pool = pools(half)
    rng = np.random.RandomState(87000)
    n_ok = 0
