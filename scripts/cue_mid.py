"""cue_mid.py — THE MID-POINT DOSE-RESPONSE FIRE (2026-08-28: 158 wild spans, halfway to the guessed threshold — measure the slope instead of flying blind). Mixed diet: mint cue labels + the 100-row wild pack at straw dosage (2/8 per batch). TWIN: arm F (frozen trunk, head only) vs arm L (LoRA r8). BASELINES: zero-wild frozen 7/143 F1 0.330; zero-wild LoRA 10/143 F1 0.135; THE ROCK 17/143 F1 0.499. Any lift over baseline = spans work, slope calibrates the true threshold; flat = audit before more harvest. Original header: rung2b.py — THE LEAN-LoRA TAGGER (2026-08-27, word given; the
...
NB = 8
STEPS = int(os.environ.get("R2B_STEPS", "3000"))
ARM = os.environ.get("CUE_ARM", "L")
...
def main():
    ...
    W = np.load('.cache/cue_wild_pack.npz')
    WY = W['y'].astype(np.int32)
    WY2 = WY.copy(); WY2[WY == 4] = 5          # wild fr -> mint-space fr
    WIDS = W['ids']; WMSK = W['msk'].astype(np.float32)
    texts = []
    ...
    print(f"[2b] tokenized {len(rows_ok)} mint rows + {len(WY2)} wild rows", flush=True)
    host = _trunk_host()
    ...
    if ARM == "F":
        train_keys = ["W1", "b1", "W2", "b2"]
    else:
        train_keys = list(P.keys())
    for k, v in P.items(): v.requires_grad = (k in train_keys)
    opt = AdamW([P[k] for k in train_keys], lr=1e-4, weight_decay=0.0)
    ...
    for s in range(STEPS):
        mi = np.sort(rng.choice(rows_ok, 6, replace=False))
        wi = rng.choice(len(WY2), 2, replace=False)
        bi = np.concatenate([IDS[mi], WIDS[wi]])
        X = trunk(Tensor(bi, dtype=dtypes.int))
        y = np.concatenate([Y2[mi], WY2[wi]])
        m = np.concatenate([MSK[mi], WMSK[wi]])
    ...
    safe_save({k: v for k, v in P.items()}, f'.cache/cue_mid_{ARM}.safetensors')
    ...
    for d, g in zip(preds, golds):
        ...
        if d == g: exo += 1
        f1s.append(f1(d, g))
    print(f"[mid {ARM}] GOLD143 CANON exact {exo}/143 (rock {cex})  "
          f"F1 {np.mean(f1s):.3f} (rock {cf1:.3f})  distinct {len(distinct)}",
          flush=True)
    print(f"[mid {ARM}] VERDICT: " + ("ABOVE THE ROCK — the trunk learned to see "
