"""v110-step3 + tree-feedback training driver.

Two modes via env:
  V110_STEP3_SBP_NOISE_SCALE=0     pure feedback (Path B) — no SBP noise
  V110_STEP3_SBP_NOISE_SCALE=0.02  feedback + SBP (Path C) — 50/50 alternating noise

Warm-start safety:
  fg_v111_feedback_gate is initialized to 0.0 → step 0 forward is byte-identical
  to the underlying v110-step3 (or v110-step3 + SBP) baseline. Gradient drives
  the gate to learn the right feedback strength.
"""
import gc
import os
import sys
...
from mycelium.factor_graph_v110_step3_fb import (
    ...
    _compile_jit_fg_step_v110_step3_fb,
    compile_jit_eval_v110_step3_fb,
)
from mycelium.factor_graph_v110_step3_sbp import V110_STEP3_SBP_NOISE_SCALE
...
def load_ckpt_v110_step3_fb(model, path: str) -> None:
    """Load v110-step3 (acc) ckpt + optionally feedback params if present.

    Existing v110-step3 ckpts (without feedback params) load via v110-acc loader.
    Feedback params are added by attach_fg_params_v110_step3_fb at init (zeros)
    and stay at zeros if the ckpt doesn't include them.
    """
    from tinygrad.nn.state import safe_load
    load_ckpt_v110_acc(model, path)
    sd = safe_load(path)
    for key in ("fg_v111_feedback_gate", "fg_v111_feedback_ln_g", "fg_v111_feedback_ln_b"):
        if key in sd:
            dst = getattr(model, key)
            src = sd[key].to(dst.device).realize()
            dst.assign(src).realize()
            print(f"  loaded fb param: {key}", flush=True)
...
def main():
    ...
    CKPT_LABEL = getenv("CKPT_LABEL",              "v110_step3_fb_smoke")
    ...
    SBP_NOISE  = float(getenv("V110_STEP3_SBP_NOISE_SCALE", "0.0"))  # default OFF for fb-alone
    ...
    sbp_mode = "OFF (Path B: fb-only)" if SBP_NOISE == 0.0 else f"ON σ={SBP_NOISE} (Path C: fb+SBP)"
    print(f"=== v110-step3 FB training (tree-logits feedback) ===")
    print(f"  feedback_gate_init={FB_GATE_INIT}  SBP={sbp_mode}")
    ...
    params   = fg_v110_step3_fb_parameters(model)
    ...
    ckpt_dir = ".cache/fg_v110_step3_fb_ckpts"
    ...
    step_fn = _compile_jit_fg_step_v110_step3_fb(
    ...
    eval_fn = compile_jit_eval_v110_step3_fb(
    ...
    for step in range(1, STEPS + 1):
        ...
        # When SBP_NOISE > 0: alternate noisy/det. When SBP_NOISE = 0: always det.
        ...
        if step % CKPT_EVERY == 0:
            ...
            safe_save(fg_v110_step3_fb_state_dict(model), ckpt_path)
