"""gen16_verdict.py — THE ONLY PEN, gen-16 (2026-07-22). Reads the collected
...
verdicts = {}
for cand in ("V4",):
    log = open(f".cache/gen16_{cand}.log").read()
    ...
    # cert-v2 join
    cv = json.load(open(f".cache/lattice_gen16_{cand}.json"))["bigtest"]
    cert = cert_ok = 0
    dissent = flips = 0
    ents = {"cert": [], "ans": [], "abst": []}
    for i in range(1500):
...
m["date"] = "2026-07-22"
m["env"]["ALG_FTYPES"] = "8"
m["parser_ckpt"] = ".cache/crown_reader_v4.safetensors"
m["corpora"]["train"] = ".cache/crown_v4.jsonl"
m["macro"] = {"grammar": "mg2", "module": "mycelium/macros.py",
              "crown_acceptance_column": "68% pooled at saturation vintage (curve 35/55/64/68)"}
...
m["notes"] = (f"2026-07-22 GEN-16 PROMOTED: crown_reader_v4 (the record cashed; "
...
print(f"\n*** PROMOTED: GEN-16 = crown_reader_v4 "
