#!/bin/bash
set -eo pipefail
cd /home/bryce/mycelium
export DEV=AMD ALG2=1 ALG_FTYPES=8 ALG_DUP=1 ALG_HW=512 ALG_WIDE=1 ALG_ALLOW_PEN_TRAIN=1
export ALG_TEST=.cache/algebra_nl_test.jsonl ALG_TEST_NAME=test23
export ALG_RINGS=1 ALG_BREATH=3 ALG_BEXIT=1
PY=.venv/bin/python3
SEED=113
for ARM in c2ctl cure; do
  SEED=$((SEED+1))
  if [ "$ARM" = "cure" ]; then
    export ALG_CLOCK=1 ALG_CLOCK_FLOOR=0.3 ALG_OPATT=1 OPATT_GOLD=.cache/opatt_gold.npy OPATT_W=0.05
  else
    export ALG_CLOCK=0 ALG_OPATT=0
  fi
  echo "=== PHASE2 [$ARM]: 4k from g23 ==="
  env WARM_FROM=.cache/g23.safetensors ALG_TRAIN=.cache/gen23_mix.jsonl \
      ALG_TRAIN_NAME=gen23 ALG_CKPT=.cache/g28_p2_${ARM}.safetensors \
      STEPS=4000 LR=1e-4 BATCH=8 SEED=${SEED} SNAP_EVERY=0 \
      $PY scripts/phase1_algebra_head.py --train | tail -2
done
echo "=== PHASE2 BURNED ==="
