"""root_fingerpost.py — THE READ FOR THE DOOR (2026-08-29): 16 lineage-diverse reader ballots intersected with witness-reachable roots; the unrelated-witnesses clause swept over (W votes, A lineages); BAR: gold net > 0 — never achieved. Original: elig_read.py — THE ELIGIBILITY WIRING (2026-08-29): the standing witness (scanner + NL-atlas geometry, the 29/143 instrument) drives ENUMERATION: witness multiset -> reachable() with ADDF-WILDCARD (each addf application branches add|sub) -> unique-root EMISSION graded by the key. Fixtures: 143 golds + wv 20 + held 20. Baseline: the chain-based eligibility era (~9% eligible, lies-dominated unique). Original: nl_atlas_v0.py — THE NL-ATLAS (2026-08-28, the emergency brake honored):
...
def main():
    ...
    for r, st, off in zip(allrows, gS, gOFF):
        ...
        r['_roots'] = roots
    import glob as _g
    cfgs = {c['id']: c for c in json.load(open('docs/reader_pool.json'))}
    ballots = {}
    for f in _g.glob('.cache/rootvote_R*.json'):
        d2 = json.load(open(f))
        ballots[d2['id']] = d2['answers']
    print(f"[fp] ballots: {len(ballots)} readers", flush=True)
    for W in (4, 5, 6, 8):
        for A in (2, 3, 4):
            T2 = {t: [0, 0, 0] for t in ("gold", "wv", "held")}
            for ri, r in enumerate(allrows):
                roots = r.get('_roots')
                if not roots: continue
                vc = {}
                for rid, ans in ballots.items():
                    a = ans[ri]
                    if a is not None and a in roots:
                        vc.setdefault(a, set()).add(cfgs[rid]['lineage'])
                votecnt = {}
                for rid, ans in ballots.items():
                    a = ans[ri]
                    if a is not None and a in roots:
                        votecnt[a] = votecnt.get(a, 0) + 1
                best = None
                for a, cnt in sorted(votecnt.items(), key=lambda x: -x[1]):
                    if cnt >= W and len(vc[a]) >= A:
                        best = a; break
                if best is None: continue
                t2 = T2[r['tag']]
                if best == r['answer']: t2[0] += 1
                else: t2[1] += 1
                t2[2] += 1
            g = T2['gold']; w = T2['wv']; h = T2['held']
            print(f"[fp W>={W} A>={A}] gold EMIT {g[2]} r{g[0]} l{g[1]} net {g[0]-g[1]}"
                  f" | wv {w[2]}: {w[0]}r/{w[1]}l | held {h[2]}: {h[0]}r/{h[1]}l", flush=True)
