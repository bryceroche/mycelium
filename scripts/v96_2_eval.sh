#!/bin/bash
# v96.2 EVAL — eval_v77_dag.py wrapped with v96.2 env so the consolidation table
# + per-breath temperature decay fires at eval (same artifact-compute path as
# training; constraint heads are NOT used at eval — they only supervise during
# training).
#
...
# v96.2 ON — per-breath temperature decay (graph-level: constraint heads
# inactive at eval but temperature scaling on logits applies).
export V96_TEMPERATURE_DECAY=1
export V96_T_START="${V96_T_START:-2.0}"
export V96_T_END="${V96_T_END:-0.3}"
