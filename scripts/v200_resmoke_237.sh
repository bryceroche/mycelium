...
#
# Run:
#   cd /home/bryce/mycelium && bash scripts/v200_resmoke_237.sh
#
set -euo pipefail
...
echo "=== v200 #237 — structural READ masks (partition 1a) ==="
echo "  SMOKE_DIR: $SMOKE_DIR"
echo "  PYTHON: $PYTHON"
echo "  Single variable vs #236: §2 latent_topology_mask (24 per-token + 4 per-op + 4 global)"
echo "  STEP-0 GATE: per-group entropy < log(support) [1/6/24], per-group never mean"
echo "  ADVISORY: per-op family = mod-4 proxy (see line 1 of train_237.log)"
echo ""
...
CKPT_LABEL="${CKPT_LABEL:-v200_perceiver_237_mask1a}" \
SEED=42 \
B_REF="${B_REF:-4}" \
SMOKE_DIR="$SMOKE_DIR" \
"$PYTHON" -u scripts/v200_resmoke_237.py 2>&1 | tee "$SMOKE_DIR/train_237_console.log"

STEPS_DONE="${STEPS:-2000}"
EVAL_JSON="$SMOKE_DIR/step${STEPS_DONE}_eval_237.json"

echo ""
echo "=== #237 masks run complete ==="
echo "Final log line:"
tail -1 "$SMOKE_DIR/train_237.log"
echo ""
...
echo ""
echo "Instrumentation timeseries: $SMOKE_DIR/instrumentation_237.json"
