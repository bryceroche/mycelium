"""funnel_repair.py — THE INDUSTRIAL FUNNEL, STAGES 2-4 (2026-08-28):
...
def main():
    ...
    for ti, (li, gold, dia, ne, raw) in enumerate(fixed):
        ...
        print(f"  [{li:3d}] gold {gold:>4} edits {ne} | votes {votes} -> "
              f"{'GATE-OK' if ok else 'refuses'} | {dia[:90]}", flush=True)
        if ok:
    ...
    json.dump(banked, open('.cache/funnel_gateok.json', 'w'))
    print(f"[fr] GATE-OK awaiting review: {len(banked)} -> .cache/funnel_gateok.json",
          flush=True)
