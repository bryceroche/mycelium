"""v112b training driver — Phase 1 shared learnable topology.

Two modes via env:
  V110_STEP3_SBP_NOISE_SCALE=0     pure topology adaptation — no SBP noise
  V110_STEP3_SBP_NOISE_SCALE=0.02  topology + SBP — 50/50 alternating noise

Warm-start safety:
  attn_bias_scale = 0 and W_res_gate = 0 → step 0 forward is byte-identical
  to the underlying v110-step3 baseline. Gradient drives both to learn
  topology structure.
"""
import gc
import os
...
from mycelium.factor_graph_v112b import (
    ...
    _compile_jit_fg_step_v112b,
    compile_jit_eval_v112b,
)
...
def load_ckpt_v112b(model, path: str) -> None:
    """Load v110-step3 (acc) ckpt + optionally v112b topology params if present.

    Existing v110-step3 ckpts (without v112b params) load via v110-acc loader.
    v112b params are added by attach_fg_params_v112b at init (zeros for
    W_res_gate and attn_bias_scale, small Gaussian for node_topology) and
    stay there if the ckpt doesn't include them.
    """
...
def main():
    ...
    CKPT_LABEL = getenv("CKPT_LABEL",              "v112b_smoke")
    ...
    SBP_NOISE  = float(getenv("V110_STEP3_SBP_NOISE_SCALE", "0.0"))  # default OFF
    ...
    sbp_mode = "OFF" if SBP_NOISE == 0.0 else f"ON σ={SBP_NOISE} (Path C: topo+SBP)"
    print(f"=== v112b training (shared learnable topology — Phase 1) ===")
    ...
    params   = fg_v112b_parameters(model)
    ...
    if RESUME_FROM:
        ...
        load_ckpt_v112b(model, RESUME_FROM)
    ...
    step_fn = _compile_jit_fg_step_v112b(
    ...
    eval_fn = compile_jit_eval_v112b(
    ...
    for step in range(1, STEPS + 1):
        ...
        if step % CKPT_EVERY == 0:
            ...
            safe_save(fg_v112b_state_dict(model), ckpt_path)
