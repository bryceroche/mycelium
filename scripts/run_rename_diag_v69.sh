#!/bin/bash
# Entity grounding diagnostic for v69 ckpt.
# Replaces first proper noun in each GSM8K question with "Xyz123" and checks
# whether the model echoes the substitution (grounded) vs the original name
# (memorized template) vs neither.
#
# Expected outcome: similar to v66 (0/20 grounded), but worth checking whether
# the 128d codebook compression changed entity preservation.
#
# Usage:
...
export CKPT="$CKPT"
export DIAG_OUT=/tmp/v69_rename_diag.jsonl

echo "=== v69 rename diagnostic ==="
echo "  ckpt: $CKPT"
echo "  output: $DIAG_OUT"
echo ""

/home/bryce/mycelium/.venv/bin/python scripts/rename_diagnostic_v69.py 2>&1 | tee /tmp/v69_rename_diag.log

echo ""
echo "=== rename diag done — see $DIAG_OUT for per-example output ==="
