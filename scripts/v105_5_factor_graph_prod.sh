#!/usr/bin/env bash
# v105.5 factor graph — production run (5000 steps)
#
...
#   - PPFFN intermediate dim 2048 (~22M extra params; W_out zero-init)
#   - Warm-start from v104_prod_step3000.safetensors
#
# DO NOT launch without smoke-run passing first.
# Usage: bash scripts/v105_5_factor_graph_prod.sh
set -euo pipefail
...
V105_5_PERPOS_FFN="${V105_5_PERPOS_FFN:-1}" \
...
CKPT_LABEL="${CKPT_LABEL:-v105_5_prod}" \
PYTHIA_INIT=1 \
RESUME_FROM="$CKPT" \
SUDOKU_TASK=0 \
V99_TASK=0 \
