#!/bin/bash
# v92 PRODUCTION RESUME — continuation from v92_prod_step600 (1st prod run died at step 670).
#
# Original v92 prod was killed at step 670 (AM driver crash, GPU bus reset). We have
# valid ckpts at step 200, 400, 600. Resume from step 600 with STEPS=1400 → 2000 total effective.
# Slight LR cosine discontinuity at restart (3e-5 vs schedule's 2.38e-5) is small noise.
#
...
V92_INIT="${V92_INIT:-.cache/gsm8k_steps_ckpts/v92_prod_step600.safetensors}"
if [ ! -f "$V92_INIT" ]; then
...
export CKPT_LABEL="${CKPT_LABEL:-v92_prod_resume}"
export RESUME_FROM="$V92_INIT"

/home/bryce/mycelium/.venv/bin/python -u scripts/l3_train.py 2>&1 | tee .cache/v92_prod_resume_train.log
