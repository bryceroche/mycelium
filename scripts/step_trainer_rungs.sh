...
echo "== rung 1: forward equivalence on the champion — step-partitioned vs fused, np.array_equal per key, FAIL LOUDLY on any diff (ST_EQ_TOL=0 is the pinned bar; the printout carries the numbers if a re-pin must be argued) =="
