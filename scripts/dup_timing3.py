...
def fwd(ids,msk,snt):
    ...
    _o0=forward(p,_st,_tk,_sn)
    _mk=build_slot_masks({k:_o0[k].realize().numpy() for k in ("fat","args","res")},snt)
    out=forward(p,_st,_tk,_sn,slot_mask=Tensor(_mk,dtype=dtypes.float))
...
json.dump({"premature":prem,"lost":lost,"other":other},open('.cache/dup_timing3.json','w'))
