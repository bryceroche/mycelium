"""v105.1.2 factor graph training driver — full-stack architecture.

Combines:
  - Digit-by-digit codebook (10-way per digit position)
  - RoPE on digit positions (v105.1 correlation fix)
  - Projection waist 1024 → 512 → 1024 (LoRA-style, from v101)
  - IB semantic codebook (32 entries, from v104)
  - Per-NUMBER factor auxiliary loss (joint log-likelihood, not per-digit average)

Warm-start from v104_prod_step3000.safetensors:
  - Copies backbone (shared.*, phase*.*, ln_f.*)
  - Also copies fg_v104.codebook → fg_v105_1_2.ib_codebook (if shapes match)
  - Fresh-inits digit codebook, RoPE tables, waist, breath_embed
  - All zero-init gates ensure step-0 forward == v104 backbone behavior
...
Env vars:
  ...
  V105_1_2_FACTOR_AUX_WEIGHT=0.5
...
from mycelium.factor_graph_v105_1_2 import (
    V105_1_2_K_MAX, V105_1_2_N_MAX, V105_1_2_F_MAX, V105_1_2_N_DIGITS, V105_1_2_N_HEADS,
    V105_1_2_ENERGY_WEIGHT, V105_1_2_FACTOR_AUX_WEIGHT, V105_1_2_CALIB_WEIGHT,
    V105_1_2_WAIST, V105_1_2_CODEBOOK_N,
    attach_fg_params_v105_1_2, fg_v105_1_2_parameters, fg_v105_1_2_state_dict,
    fg_breathing_forward_v105_1_2, load_ckpt_v105_1_2,
    _compile_jit_fg_step_v105_1_2, _compile_jit_fg_eval_v105_1_2,
    value_to_digits, digits_to_value,
)
# v105.1.2 data loaders: MSD-first encoding + digit_valid_mask
from mycelium.factor_graph_data_v105_1_2 import (
    FactorGraphLoaderV105_1_2, DualDataLoaderV105_1_2, load_gsm8k_records_v105_1_2,
)
...
def collect_fg_params_v105_1_2(model) -> list[Tensor]:
    """Trainable params: shared L0-L3 attn/FFN + v105.1.2 fg-specific params."""
    params: list[Tensor] = []
    sw = model.block.shared
    ...
    params += fg_v105_1_2_parameters(model)
    return params


def model_state_dict_v105_1_2(model) -> dict:
    sd = {
        "ln_f.g": model.ln_f_g,
        "ln_f.b": model.ln_f_b,
    }
    ...
    sd.update(fg_v105_1_2_state_dict(model))
    return sd


def evaluate_v105_1_2(
    model, loader: FactorGraphLoaderV105_1_2,
            ...
            for vi in range(min(nv, n_max)):
                if obs_np[b, vi] == 0:
                    ...
                    if v_valid.any():
                        ...
                        # Edge case (shouldn't happen — every value uses at least pos N-1).
...
def main():
    V105_1_2_TASK_LOCAL = int(getenv("V105_1_2_TASK", 0)) > 0
    assert V105_1_2_TASK_LOCAL, "V105_1_2_TASK=1 must be set"

    K        = int(getenv("V105_1_2_K_MAX",    str(V105_1_2_K_MAX)))
    N_DIGITS = int(getenv("V105_1_2_N_DIGITS", str(V105_1_2_N_DIGITS)))
    N_MAX    = int(getenv("V105_1_2_N_MAX",    str(V105_1_2_N_MAX)))
    F_MAX    = int(getenv("V105_1_2_F_MAX",    str(V105_1_2_F_MAX)))
    WAIST    = int(getenv("V105_1_2_WAIST",    str(V105_1_2_WAIST)))
    N_CODE   = int(getenv("V105_1_2_CODEBOOK_N", str(V105_1_2_CODEBOOK_N)))
    ...
    CKPT_LABEL       = getenv("CKPT_LABEL",             "v105_1_2_prod")
    ...
    ENERGY_WEIGHT     = float(getenv("V105_1_2_ENERGY_WEIGHT",     str(V105_1_2_ENERGY_WEIGHT)))
    FACTOR_AUX_WEIGHT = float(getenv("V105_1_2_FACTOR_AUX_WEIGHT", str(V105_1_2_FACTOR_AUX_WEIGHT)))
    CALIB_WEIGHT      = float(getenv("V105_1_2_CALIB_WEIGHT",      str(V105_1_2_CALIB_WEIGHT)))
    ...
    print("=== v105.1.2 factor graph training (digit RoPE + waist + IB codebook) ===")
    ...
    if RESUME_FROM:
        ...
        load_ckpt_v105_1_2(model, RESUME_FROM)

    params   = collect_fg_params_v105_1_2(model)
    ...
    synth_loader = FactorGraphLoaderV105_1_2(
        ...
        n_max=N_MAX, f_max=F_MAX, k_max=K, n_heads=V105_1_2_N_HEADS,
        n_digits=N_DIGITS, seed=SEED,
    )

    gsm8k_records = load_gsm8k_records_v105_1_2(
        GSM8K_PATH, n_digits=N_DIGITS, n_max=N_MAX, f_max=F_MAX,
    )

    dual_loader = DualDataLoaderV105_1_2(
    ...
    ckpt_dir = ".cache/fg_v105_1_2_ckpts"
    os.makedirs(ckpt_dir, exist_ok=True)

    Tensor.training = True
    step_fn = _compile_jit_fg_step_v105_1_2(
    ...
    eval_fn = _compile_jit_fg_eval_v105_1_2(
    ...
    for step in range(1, STEPS + 1):
        ...
        if step % EVAL_EVERY == 0:
            ...
            # If gates taper across breaths, the model has learned adaptive
            # step sizes (ODE convergence). If flat, V-shape is structural.
            try:
                dg_main  = model.fg_v105_1_2_delta_gate.numpy().tolist()
                dg_quant = model.fg_v105_1_2_delta_gate_quant.numpy().tolist()
                print(
                    "  delta_gate      : "
        ...
        if step % CKPT_EVERY == 0:
            ...
            safe_save(model_state_dict_v105_1_2(model), ckpt_path)
