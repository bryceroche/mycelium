"""v109 factor graph training driver — v108 + waist + alternation."""
import gc
import os
import sys
import time
...
from mycelium.factor_graph_v109 import (
    ...
    V109_WAIST_DIM, V109_ALTERNATION, V109_HARD_BREATH_LEVEL,
    V109_VAR_LOSS_WEIGHT, V109_CALIB_WEIGHT, V109_FACTOR_AUX_WEIGHT,
    V109_CODEBOOK_N, V109_IB_CENTROIDS,
    attach_fg_params_v109, fg_v109_parameters, fg_v109_state_dict,
    fg_breathing_forward_v109,
    _compile_jit_fg_step_v109, _compile_jit_fg_eval_v109,
)
...
from scripts.v108_factor_graph_train import (
    cast_layers_fp32, evaluate_v108,
)

DIFFICULTIES = ["easy", "medium", "hard"]


def collect_fg_params_v109(model) -> list:
    params = []
    ...
    params += fg_v109_parameters(model)
    return params


def model_state_dict_v109(model) -> dict:
    ...
    sd.update(fg_v109_state_dict(model))
    return sd


def load_ckpt_v109(model, path: str):
    """Load v108 or v109 ckpt. v109 keys fall back to fresh init if absent."""
    sd = safe_load(path)
    targets = model_state_dict_v109(model)
    loaded, missing = [], []
    ...
    print(f"  loaded {len(bb)} backbone + {len(v7)} v107 + "
          f"{len(v8)} v108 + {len(v9)} v109 keys", flush=True)
    if any(k.startswith("fg_v109.") for k in missing):
        print("  v109 waist keys not in ckpt — using fresh LoRA init "
              "(W_expand=0 → byte-identical to v108 at step 0)", flush=True)


def evaluate_v109(model, loader, K, max_batches=20, eval_fn=None,
                  n_max=V109_N_MAX, f_max=V109_F_MAX, n_digits=V109_N_DIGITS):
    """Reuse v108's evaluate (same eval_fn signature, same per-position logic)."""
...
def main():
    V109_TASK_LOCAL = int(getenv("V109_TASK", 0)) > 0
    assert V109_TASK_LOCAL, "V109_TASK=1 must be set"

    K          = int(getenv("V109_K_MAX",      str(V109_K_MAX)))
    ...
    CKPT_LABEL = getenv("CKPT_LABEL",           "v109_smoke")
    RESUME_FROM = getenv("RESUME_FROM",         "")
    PYTHIA_INIT = int(getenv("PYTHIA_INIT",     "1")) > 0
    SEED       = int(getenv("SEED",             "42"))
    N_CODE     = int(getenv("V109_CODEBOOK_N",  str(V109_CODEBOOK_N)))
    IB_PATH    = getenv("V109_IB_CENTROIDS",    V109_IB_CENTROIDS)
    N_DIGITS   = int(getenv("V109_N_DIGITS",    str(V109_N_DIGITS)))
    WAIST_DIM  = int(getenv("V109_WAIST_DIM",   str(V109_WAIST_DIM)))
    ALTERNATION = int(getenv("V109_ALTERNATION", "1")) > 0
    HARD_LEVEL = int(getenv("V109_HARD_BREATH_LEVEL", "0")) > 0
    VW         = float(getenv("V109_VAR_LOSS_WEIGHT",  str(V109_VAR_LOSS_WEIGHT)))
    FW         = float(getenv("V109_FACTOR_AUX_WEIGHT", str(V109_FACTOR_AUX_WEIGHT)))
    AW         = float(getenv("V109_CALIB_WEIGHT",      str(V109_CALIB_WEIGHT)))
    N_MAX      = int(getenv("V109_N_MAX",      str(V109_N_MAX)))
    F_MAX      = int(getenv("V109_F_MAX",      str(V109_F_MAX)))

    DIFFICULTY_FILTER = os.environ.get("V109_DIFFICULTY_FILTER", "").strip() or None
    TRAIN_PATH   = getenv("V109_TRAIN",       ".cache/factor_graph_train.jsonl")
    VAL_PATH     = getenv("V109_VAL",         ".cache/factor_graph_test.jsonl")
    GSM8K_PATH   = getenv("V109_GSM8K_TRAIN", ".cache/gsm8k_factor_graphs_train.jsonl")
    GSM8K_RATIO  = float(getenv("V109_GSM8K_RATIO", "0.5"))
    EVAL_BATCHES = int(getenv("EVAL_BATCHES", "10"))
    EVAL_BATCH   = int(getenv("EVAL_BATCH",   str(BATCH)))

    mode = "ALTERNATING (waist on even breaths)" if ALTERNATION else "WAIST-EVERY-BREATH (v101 style)"
    print(f"=== v109 factor graph training (v108 + waist + alternation) ===")
    print(f"  mode: {mode}")
    ...
    params   = collect_fg_params_v109(model)
    ...
    if RESUME_FROM:
        ...
        load_ckpt_v109(model, RESUME_FROM)
    ...
    synth_loader = FactorGraphLoaderV107(
        ...
        n_max=N_MAX, f_max=F_MAX, k_max=K, n_heads=V109_N_HEADS, seed=SEED,
    )
    ...
    dual_loader = DualDataLoaderV107(
        ...
        n_max=N_MAX, f_max=F_MAX, k_max=K, n_heads=V109_N_HEADS, seed=SEED + 1,
    )
    val_loader = FactorGraphLoaderV107(
        VAL_PATH, batch_size=EVAL_BATCH,
        difficulty_filter=None, curriculum=False,
        n_max=N_MAX, f_max=F_MAX, k_max=K, n_heads=V109_N_HEADS, seed=SEED + 2,
    )

    ckpt_dir = ".cache/fg_v109_ckpts"
    os.makedirs(ckpt_dir, exist_ok=True)

    Tensor.training = True
    step_fn = _compile_jit_fg_step_v109(
    ...
    for step in range(1, STEPS + 1):
        ...
        if step % CKPT_EVERY == 0:
            ...
            safe_save(model_state_dict_v109(model), ckpt_path)
