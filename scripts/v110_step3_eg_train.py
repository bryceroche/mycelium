"""v110-step3 entropy-gated waist training driver.

Trains v110-step3 with adaptive waist gate modulated by tree_logits entropy.
Warm-start safe: fg_v112_entropy_scale init=0 → byte-identical to baseline.

Env:
  V110_STEP3_EG_ENTROPY_SCALE_INIT  default 0.0 (byte-identical warm-start)
  V110_STEP3_SBP_NOISE_SCALE        default 0.0 (no SBP; isolate eg test)
"""
import gc
import os
import sys
...
from mycelium.factor_graph_v110_step3_eg import (
    V110_STEP3_EG_ENTROPY_SCALE_INIT,
    attach_fg_params_v110_step3_eg,
    fg_v110_step3_eg_parameters, fg_v110_step3_eg_state_dict,
    _compile_jit_fg_step_v110_step3_eg,
    compile_jit_eval_v110_step3_eg,
)
...
def load_ckpt_v110_step3_eg(model, path: str) -> None:
    """Load v110-acc ckpt + entropy_scale if present."""
    from tinygrad.nn.state import safe_load
    load_ckpt_v110_acc(model, path)
    sd = safe_load(path)
    if "fg_v112_entropy_scale" in sd:
        dst = model.fg_v112_entropy_scale
        src = sd["fg_v112_entropy_scale"].to(dst.device).realize()
        dst.assign(src).realize()
        print(f"  loaded fb param: fg_v112_entropy_scale", flush=True)
...
def main():
    ...
    CKPT_LABEL = getenv("CKPT_LABEL",              "v110_step3_eg_smoke")
    ...
    ENT_INIT    = float(getenv("V110_STEP3_EG_ENTROPY_SCALE_INIT",
                                str(V110_STEP3_EG_ENTROPY_SCALE_INIT)))
    ...
    print(f"=== v110-step3 EG training (entropy-gated waist) ===")
    print(f"  entropy_scale_init={ENT_INIT}  SBP_noise={SBP_NOISE}")
    ...
    params   = fg_v110_step3_eg_parameters(model)
    ...
    if RESUME_FROM:
        ...
        load_ckpt_v110_step3_eg(model, RESUME_FROM)
    ...
    ckpt_dir = ".cache/fg_v110_step3_eg_ckpts"
    ...
    step_fn = _compile_jit_fg_step_v110_step3_eg(
    ...
    eval_fn = compile_jit_eval_v110_step3_eg(
    ...
    for step in range(1, STEPS + 1):
        ...
        if step % LOG_EVERY == 0:
            dt = time.time() - t0
            ent_scale_val = float(model.fg_v112_entropy_scale.numpy()[0])
            print(
        ...
        if step % CKPT_EVERY == 0:
            ...
            safe_save(fg_v110_step3_eg_state_dict(model), ckpt_path)
