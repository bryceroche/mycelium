"""form_assemble15.py — G51 THE MIXTURE (2026-08-21, word given): deeds
...
print(f"[assemble15] anchor-law corpus: {_before} -> {len(_byid)} "
...
print(f"[assemble15] G55 wild-val split: diet {len(_clean)} / wild-val {len(_wildval)} (seed 555, banked)", flush=True)
...
with open('.cache/form_mix15.jsonl','w') as f:
    for r in mix: f.write(json.dumps(r)+"\n")
n=len(mix)
print(f"[assemble15] base 25000 + deeds {ND} (x1) + diet {NU}x{REPS}={REPS*NU} = {n}"
      f" (diet mass {REPS*NU})", flush=True)
b8=np.load('.cache/phase1_alg_states_form8_states.npy', mmap_mode='r')
out=np.lib.format.open_memmap('.cache/phase1_alg_states_form15_states.npy', mode='w+',
                              dtype=np.float16, shape=(n,T_ALG,2048))
CH=2048
for s0 in range(0,25000,CH):
    sl=keep[s0:min(s0+CH,25000)]
    out[s0:s0+len(sl)]=b8[sl]
samples, ids2, mask, offsets = PH.tokenize('.cache/form_mix15.jsonl')
NUQ=ND+NU
...
np.savez('.cache/phase1_alg_states_form15.npz', tokmask=mask.astype(np.uint8),
         sent=sent.astype(np.int8), mix_sha=mix_sha16('.cache/form_mix15.jsonl'),
         **{f"g_{k}":v for k,v in gold.items()})
print(f"[assemble15] STITCHED base+deeds+diet + gold + sha ({n} rows)", flush=True)
