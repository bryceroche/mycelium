"""v110-full factor graph training driver — v109pi + ACCUMULATE notebook + photon gate."""
import gc
import os
import sys
import time
...
from mycelium.factor_graph_v110_full import (
    V110_FULL_K_MAX, V110_FULL_N_MAX, V110_FULL_F_MAX, V110_FULL_N_HEADS,
    V110_FULL_N_DIGITS,
    V110_FULL_WAIST_DIM, V110_FULL_ALTERNATION, V110_FULL_HARD_BREATH_LEVEL,
    V110_FULL_VAR_LOSS_WEIGHT, V110_FULL_CALIB_WEIGHT,
    V110_FULL_FACTOR_AUX_WEIGHT,
    V110_FULL_CODEBOOK_N, V110_FULL_IB_CENTROIDS, V110_FULL_PHASE_SCALE,
    V110_FULL_GATE_PROFILE, V110_FULL_PHOTON_ALPHA,
    attach_fg_params_v110_full, fg_v110_full_parameters, fg_v110_full_state_dict,
    _compile_jit_fg_step_v110_full, _compile_jit_fg_eval_v110_full,
)
...
# Inherit infrastructure from v110-acc (same params, same state_dict shape).
collect_fg_params_v110_full = collect_fg_params_v110_acc
model_state_dict_v110_full = model_state_dict_v110_acc
load_ckpt_v110_full = load_ckpt_v110_acc
evaluate_v110_full = evaluate_v110_acc


def main():
    V110_FULL_TASK_LOCAL = int(getenv("V110_FULL_TASK", 0)) > 0
    assert V110_FULL_TASK_LOCAL, "V110_FULL_TASK=1 must be set"

    K          = int(getenv("V110_FULL_K_MAX",    str(V110_FULL_K_MAX)))
    ...
    CKPT_LABEL = getenv("CKPT_LABEL",             "v110_full_smoke")
    RESUME_FROM = getenv("RESUME_FROM",           "")
    PYTHIA_INIT = int(getenv("PYTHIA_INIT",       "1")) > 0
    SEED       = int(getenv("SEED",               "42"))
    N_CODE     = int(getenv("V110_FULL_CODEBOOK_N", str(V110_FULL_CODEBOOK_N)))
    IB_PATH    = getenv("V110_FULL_IB_CENTROIDS",   V110_FULL_IB_CENTROIDS)
    N_DIGITS   = int(getenv("V110_FULL_N_DIGITS",   str(V110_FULL_N_DIGITS)))
    WAIST_DIM  = int(getenv("V110_FULL_WAIST_DIM",  str(V110_FULL_WAIST_DIM)))
    ALTERNATION = int(getenv("V110_FULL_ALTERNATION", "1")) > 0
    HARD_LEVEL = int(getenv("V110_FULL_HARD_BREATH_LEVEL", "0")) > 0
    PHASE_SCALE = float(getenv("V110_FULL_PHASE_SCALE", str(V110_FULL_PHASE_SCALE)))
    GATE_PROFILE = getenv("V110_FULL_GATE_PROFILE", V110_FULL_GATE_PROFILE)
    PHOTON_ALPHA = float(getenv("V110_FULL_PHOTON_ALPHA", str(V110_FULL_PHOTON_ALPHA)))
    VW         = float(getenv("V110_FULL_VAR_LOSS_WEIGHT",  str(V110_FULL_VAR_LOSS_WEIGHT)))
    FW         = float(getenv("V110_FULL_FACTOR_AUX_WEIGHT", str(V110_FULL_FACTOR_AUX_WEIGHT)))
    AW         = float(getenv("V110_FULL_CALIB_WEIGHT",      str(V110_FULL_CALIB_WEIGHT)))
    N_MAX      = int(getenv("V110_FULL_N_MAX",    str(V110_FULL_N_MAX)))
    F_MAX      = int(getenv("V110_FULL_F_MAX",    str(V110_FULL_F_MAX)))

    DIFFICULTY_FILTER = os.environ.get("V110_FULL_DIFFICULTY_FILTER", "").strip() or None
    TRAIN_PATH   = getenv("V110_FULL_TRAIN",       ".cache/factor_graph_train.jsonl")
    VAL_PATH     = getenv("V110_FULL_VAL",         ".cache/factor_graph_test.jsonl")
    GSM8K_PATH   = getenv("V110_FULL_GSM8K_TRAIN", ".cache/gsm8k_factor_graphs_train.jsonl")
    GSM8K_RATIO  = float(getenv("V110_FULL_GSM8K_RATIO", "0.5"))
    EVAL_BATCHES = int(getenv("EVAL_BATCHES", "10"))
    EVAL_BATCH   = int(getenv("EVAL_BATCH",   str(BATCH)))

    print(f"=== v110-full factor graph training (v109pi + accumulate + photon) ===")
    ...
    params   = collect_fg_params_v110_full(model)
    ...
    if RESUME_FROM:
        ...
        load_ckpt_v110_full(model, RESUME_FROM)
    ...
    synth_loader = FactorGraphLoaderV107(
        ...
        n_max=N_MAX, f_max=F_MAX, k_max=K, n_heads=V110_FULL_N_HEADS, seed=SEED,
    )
    ...
    dual_loader = DualDataLoaderV107(
        ...
        n_max=N_MAX, f_max=F_MAX, k_max=K, n_heads=V110_FULL_N_HEADS, seed=SEED + 1,
    )
    val_loader = FactorGraphLoaderV107(
        VAL_PATH, batch_size=EVAL_BATCH,
        difficulty_filter=None, curriculum=False,
        n_max=N_MAX, f_max=F_MAX, k_max=K, n_heads=V110_FULL_N_HEADS, seed=SEED + 2,
    )

    ckpt_dir = ".cache/fg_v110_full_ckpts"
    os.makedirs(ckpt_dir, exist_ok=True)

    Tensor.training = True
    step_fn = _compile_jit_fg_step_v110_full(
    ...
    eval_fn = _compile_jit_fg_eval_v110_full(
    ...
    for step in range(1, STEPS + 1):
        ...
        if step % CKPT_EVERY == 0:
            ...
            safe_save(model_state_dict_v110_full(model), ckpt_path)
