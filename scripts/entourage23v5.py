"""entourage23v5.py — THE ESTATE SETTLEMENT FOR GEN-23 (2026-08-03; adapted
...
(phase1_alg_states_gen23.npz + _g22_states.npy, 82,400 rows, verified at
...
PARSER = ".cache/g23v5.safetensors"
NEW_NACK = ".cache/phase1_gen23v5_nack.safetensors"
...
if _lines(".cache/gen23v5_repair.jsonl") == 3800:
    print("    [skip] repair corpora banked", flush=True)
else:
    sh(".venv/bin/python3 scripts/algebra_nl_gen.py --n 800 --seed 231 --out .cache/g23v5r_v1.jsonl --teeth 0.8", tail=1)
    sh(".venv/bin/python3 scripts/algebra2_nl_gen.py --n 800 --seed 232 --out .cache/g23v5r_v2.jsonl --teeth 0.8 --token-budget 250", tail=1)
    sh(".venv/bin/python3 scripts/algebra3_nl_gen.py --n 800 --seed 233 --out .cache/g23v5r_v3.jsonl --teeth 0.8 --token-budget 250", tail=1)
    sh(".venv/bin/python3 scripts/algebra_verbose_gen.py 600 234 .cache/g23v5r_vb", tail=1)
    sh(".venv/bin/python3 scripts/algebra_dag7_gen.py 800 235 .cache/g23v5r_dag.jsonl", tail=1)
    sh("cat .cache/g23v5r_v1.jsonl .cache/g23v5r_v2.jsonl .cache/g23v5r_v3.jsonl "
   ".cache/g23v5r_vb_verbose.jsonl .cache/g23v5r_dag.jsonl > .cache/gen23v5_repair.jsonl "
   "&& wc -l .cache/gen23v5_repair.jsonl", tail=1)
...
if _os.path.exists(".cache/phase1_alg_states_gen23repair.npz"):
    ...
    sh(".venv/bin/python3 scripts/phase1_algebra_head.py --precompute",
       {"ALG_TRAIN": ".cache/gen23v5_repair.jsonl", "ALG_TRAIN_NAME": "gen23repair",
...
if _os.path.exists(".cache/phase1_gen23v5_nack.safetensors"):
    ...
    e = {"ALG_TRAIN": ".cache/gen23v5_repair.jsonl", "ALG_TRAIN_NAME": "gen23repair",
...
np.savez(".cache/monitor_centroids_gen23v5.npz", **lib)
print("centroids:", sorted(lib))
'''
open(".cache/_e22_s4.py", "w").write(S4)
sh(".venv/bin/python3 .cache/_e22_s4.py", tail=1)

print("=== E23 5/10: mouth rebuild (g22 family) + length refit ===", flush=True)
S5 = r'''
import sys, os, json; sys.path.insert(0,"."); sys.path.insert(0,"scripts")
...
fam, _ = pooled_npz(".cache/phase1_alg_states_gen23.npz",
                    ".cache/phase1_alg_states_gen23_states.npy", cap=12000)
...
np.savez(".cache/recognition_mouth_gen23v5.npz", bank=bank,
...
def dissent_set(gate_votes):
    ...
    for i in range(1500):
        ...
        if gc == 5 and not (at == gt and ct == gt):
            out.add(i)
    return out
d21 = dissent_set(json.load(open(".cache/lattice_gen22_H.json"))["bigtest"])
d22 = dissent_set(json.load(open(".cache/lattice_gen23_H.json"))["bigtest"])
ov = d21 & d22
...
print("=== E23 8/10: collapse re-read (dashboard accrual) ===", flush=True)
sh(".venv/bin/python3 scripts/collapse_probe.py",
   {"COLLAPSE_CKPT": PARSER}, tail=3)

print("=== E23 9/10: manifest member refresh + RE-HASH ===", flush=True)
m = json.load(open(".cache/GENERATION.json"))
m["specialist_ckpt"] = NEW_NACK
m["monitor_centroids"] = ".cache/monitor_centroids_gen23v5.npz"
m["mouth"] = ".cache/recognition_mouth_gen23v5.npz"
...
print("=== E23 10/10: THE STANDING REHEARSAL (outside air, both tiers) ===", flush=True)
sh(".venv/bin/python3 scripts/wild_frontier_fixture.py",
   {"WFF_CKPT": PARSER, "WFF_OUT": ".cache/wild_frontier_fixture_gen23_entourage.json"},
   tail=4)
