"""book5_lanes.py — BOOK 5, stage 2 (2026-07-22): THE FIRST LANE PASS UNDER
A CROWN-READING GATE (gen-16). Fresh L4/L5 stock beyond every consumed
...
N_CAND = int(os.environ.get("B5_CAND", "200"))
...
cands = pool[:N_CAND]
print(f"[book5] harder-strata pool: {len(pool)} bankable "
...
c = Counter(l["lane"] for l in lanes)
print(f"\n[book5-lanes] {dict(c)} of {len(cands)}  "
      f"(re-pricing watch: book-4-era baseline 71% L3)")
json.dump(lanes, open(".cache/book5_lanes.json", "w"))
with open(".cache/book5_lane1.jsonl", "w") as f:
    for b in lane1:
        f.write(json.dumps(b) + "\n")
print(f"[book5] lane-1 free entries: {len(lane1)} -> .cache/book5_lane1.jsonl")
print("[book5] repair bench + surgery lists -> .cache/book5_lanes.json")
