"""v105.3 factor graph training driver — LSD-first array layout refactor.

Functionally equivalent to v105.1.2 v2 with AR LSD-first enabled, but uses
LSD-first array layout natively (no "right-aligned RoPE" reversal patch).

Combines:
  - Digit-by-digit codebook (10-way per digit position)
  - RoPE on digit positions (LSD natural alignment — no reversal)
  ...
  - LSD-first autoregressive digit decoding (when V105_3_AR_DIGITS=1)

Warm-start from v104_prod_step3000.safetensors:
  - Copies backbone (shared.*, phase*.*, ln_f.*)
  - Also copies fg_v104.codebook → fg_v105_3.ib_codebook (if shapes match)
...
Env vars (all identical structure to v105.1.2 but prefixed V105_3_):
  V105_3_TASK=1
  V105_3_K_MAX=8
  V105_3_N_DIGITS=5
  V105_3_N_MAX=16
  ...
  V105_3_AR_DIGITS=1            (active variant: AR on by default in prod)
  V105_3_AR_MSD_FIRST=0         (LSD-first AR: proven direction)
...
from mycelium.factor_graph_v105_3 import (
    V105_3_K_MAX, V105_3_N_MAX, V105_3_F_MAX, V105_3_N_DIGITS, V105_3_N_HEADS,
    V105_3_ENERGY_WEIGHT, V105_3_FACTOR_AUX_WEIGHT, V105_3_CALIB_WEIGHT,
    V105_3_WAIST, V105_3_CODEBOOK_N,
    attach_fg_params_v105_3, fg_v105_3_parameters, fg_v105_3_state_dict,
    fg_breathing_forward_v105_3, load_ckpt_v105_3,
    _compile_jit_fg_step_v105_3, _compile_jit_fg_eval_v105_3,
    value_to_digits_lsd, digits_to_value_lsd,
)
# v105.3 data loaders: LSD-first encoding + digit_valid_mask
from mycelium.factor_graph_data_v105_3 import (
    FactorGraphLoaderV105_3, DualDataLoaderV105_3, load_gsm8k_records_v105_3,
)
...
def collect_fg_params_v105_3(model) -> list[Tensor]:
    """Trainable params: shared L0-L3 attn/FFN + v105.3 fg-specific params."""
    params: list[Tensor] = []
    sw = model.block.shared
    ...
    params += fg_v105_3_parameters(model)
    return params


def model_state_dict_v105_3(model) -> dict:
    sd = {
        "ln_f.g": model.ln_f_g,
        "ln_f.b": model.ln_f_b,
    }
    ...
    sd.update(fg_v105_3_state_dict(model))
    return sd


def evaluate_v105_3(
    model, loader: FactorGraphLoaderV105_3,
    ...
    for batch in loader.iter_eval(batch_size=loader.batch_size):
        ...
        if eval_fn is not None:
            ...
            dig_lh, _, _ = fg_breathing_forward_v105_3(
...
def main():
    V105_3_TASK_LOCAL = int(getenv("V105_3_TASK", 0)) > 0
    assert V105_3_TASK_LOCAL, "V105_3_TASK=1 must be set"

    K        = int(getenv("V105_3_K_MAX",    str(V105_3_K_MAX)))
    N_DIGITS = int(getenv("V105_3_N_DIGITS", str(V105_3_N_DIGITS)))
    N_MAX    = int(getenv("V105_3_N_MAX",    str(V105_3_N_MAX)))
    F_MAX    = int(getenv("V105_3_F_MAX",    str(V105_3_F_MAX)))
    WAIST    = int(getenv("V105_3_WAIST",    str(V105_3_WAIST)))
    N_CODE   = int(getenv("V105_3_CODEBOOK_N", str(V105_3_CODEBOOK_N)))
    ...
    CKPT_LABEL       = getenv("CKPT_LABEL",             "v105_3_prod")
    ...
    ENERGY_WEIGHT     = float(getenv("V105_3_ENERGY_WEIGHT",     str(V105_3_ENERGY_WEIGHT)))
    FACTOR_AUX_WEIGHT = float(getenv("V105_3_FACTOR_AUX_WEIGHT", str(V105_3_FACTOR_AUX_WEIGHT)))
    CALIB_WEIGHT      = float(getenv("V105_3_CALIB_WEIGHT",      str(V105_3_CALIB_WEIGHT)))
    ...
    print("=== v105.3 factor graph training (LSD-first array + digit RoPE + waist + IB) ===")
    ...
    if RESUME_FROM:
        ...
        load_ckpt_v105_3(model, RESUME_FROM)

    params   = collect_fg_params_v105_3(model)
    ...
    synth_loader = FactorGraphLoaderV105_3(
        ...
        n_max=N_MAX, f_max=F_MAX, k_max=K, n_heads=V105_3_N_HEADS,
        n_digits=N_DIGITS, seed=SEED,
    )

    gsm8k_records = load_gsm8k_records_v105_3(
        GSM8K_PATH, n_digits=N_DIGITS, n_max=N_MAX, f_max=F_MAX,
    )

    dual_loader = DualDataLoaderV105_3(
    ...
    val_loader = FactorGraphLoaderV105_3(
    ...
    ckpt_dir = ".cache/fg_v105_3_ckpts"
    os.makedirs(ckpt_dir, exist_ok=True)

    Tensor.training = True
    step_fn = _compile_jit_fg_step_v105_3(
    ...
    eval_fn = _compile_jit_fg_eval_v105_3(
    ...
    for step in range(1, STEPS + 1):
        ...
        if step % EVAL_EVERY == 0:
            ...
            try:
                dg_main  = model.fg_v105_3_delta_gate.numpy().tolist()
                dg_quant = model.fg_v105_3_delta_gate_quant.numpy().tolist()
                print(
                    "  delta_gate      : "
        ...
        if step % CKPT_EVERY == 0:
            ...
            safe_save(model_state_dict_v105_3(model), ckpt_path)
