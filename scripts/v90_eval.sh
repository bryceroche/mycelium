#!/bin/bash
# v90 eval — eval_v85_dag.py on the v90 step N ckpt.
set -e
cd "$(dirname "$0")/.."

V90_CKPT="${V90_CKPT:-.cache/gsm8k_steps_ckpts/v90_smoke_step100.safetensors}"
NUM_EVAL="${NUM_EVAL:-60}"

# Same env needed for model construction (matches v90_smoke_train.sh).
...
export V89_SUPERVISED_ATTN=1
export V89_SUPERVISED_ATTN_WEIGHT=0.5
export V89_PROJ_INIT_SCALE=0.02
export V89_INHERIT_V86=0
# v90 has no eval-side flags — the bias is in the ckpt itself. Default OFF here
# so the eval load doesn't overwrite the trained bias.
...
/home/bryce/mycelium/.venv/bin/python -u scripts/eval_v85_dag.py 2>&1 | tee .cache/v90_smoke_eval.log
