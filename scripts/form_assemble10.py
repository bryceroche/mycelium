...
import glob as _g
b9=[json.loads(l) for f in sorted(_g.glob('.cache/book*_t*_batch*.jsonl')) for l in open(f) if l.strip()]
import copy as _cp
_b=[]
for r in b9:
...
samples, ids2, mask, offsets = PH.tokenize('.cache/form_mix10.jsonl')
...
np.savez('.cache/phase1_alg_states_form10.npz', tokmask=mask.astype(np.uint8),
         sent=sent.astype(np.int8), mix_sha=mix_sha16('.cache/form_mix10.jsonl'),
         **{f"g_{k}":v for k,v in gold.items()})
print(f"[assemble10] STITCHED 96100 + computed 25x10 + gold + sha ({n} rows)", flush=True)
