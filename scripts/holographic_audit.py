...
def main():
    ...
    stats = {t: {r2: [0, 0] for r2 in ('arg1', 'arg2', 'res', 'op')}
             for t in ('mint', 'gold')}
    hbuck = {}   # (tag, n_factors) -> [ok, n] across all four roles
        ...
        for i, r in enumerate(sl):
            ...
            for j in range(L_FAC):
                ...
                for role in truth:
                    ...
                    hb = hbuck.setdefault((r['tag'], _nf), [0, 0])
                    hb[1] += 1
                    if rec == truth[role]: hb[0] += 1
    ...
    print("[holographic curve] recovery vs superposition load "
          "(bucket = factors/row; load = 4x that in bindings):")
    for t in ('mint', 'gold'):
        ks = sorted(k[1] for k in hbuck if k[0] == t)
        for nf in ks:
            ok, n = hbuck[(t, nf)]
            print(f"  {t} nf={nf:2d} (load~{4*nf:3d}): "
                  f"recovery {ok/max(n,1):.3f} (n={n})")
