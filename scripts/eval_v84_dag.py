"""v84 DAG eval — SINGLE-HEAD AR decoding on the outline-to-essay format.

v84's final breath (B4 with K=5) emits the SAME 3-list format as v82's B6:

    "<ops_csv> | <types_csv> | <args_csv>"

So this script is structurally identical to scripts/eval_v82_dag.py with two
v84-specific knobs: K=5 by default (matches the training schedule) and the
v84 test set (.cache/gsm8k_steps_v84_test.jsonl). The eval uses the SAME
single-head WaistController path, the SAME 4-mask discipline (kv_mask,
notebook_pool_mask, main_attn_mask, embed_mask via WaistController copy
suppression), and the SAME DAG executor (v82_sympy_eval).

Env vars (mirror eval_v82_dag):
    CKPT=...           required. Path to the v84 ckpt.
    V77_TEST_PATH=...  test set JSONL (default v84 test).
    NUM_EVAL=60        max problems to eval.
    K=5                inner breaths (matches v84 training).
    FIXED_LEN=256      input padding.
    BATCH=4            eval batch size.
    MAX_NEW=80         max AR tokens.

The launcher should set MULTI_HEAD_WAIST=0 (matches training).
"""
import os
import sys
import time
...
# v84 sympy eval re-exports v82's parser (same final-breath format).
from v84_sympy_eval import b6_string_to_dag, dag_to_answer  # type: ignore
...
def _compile_jit_single_head_step(model, K: int, fixed_len: int, B: int):
    ...
    WaistController path. (Identical to eval_v82_dag's helper.)
    """
    key = ("v84_single", id(model), int(K), int(fixed_len), int(B))
...
def extract_final_from_text(text: str) -> str:
    """Truncate decoded text to the first 3 pipe-separated segments.

    The model may emit more pipes if it doesn't learn a stop signal. Split on
    ' |' (preferred) or '|' and take the first 3 segments.
    """
    text = text.strip()
    segments = text.split(" |")
    if len(segments) < 3:
...
def eval_batch(model, tok, examples, K, fixed_len, batch_size, max_new):
    ...
    for i, ex in enumerate(examples):
        ...
        final_text = extract_final_from_text(gen_text)
        dag = b6_string_to_dag(final_text)
        ...
        results.append({
            ...
            "gen": gen_text,
            "text": final_text,
            "segs": tuple(final_text.split(" | ")) if " | " in final_text else (final_text,),
            "dag": dag,
            "val": val,
            "gold": ex.gold_answer,
            "ok": ok,
...
def main():
    ...
    TEST_PATH = os.environ.get("V77_TEST_PATH", ".cache/gsm8k_steps_v84_test.jsonl")
    ...
    print(f"=== v84 DAG eval — SINGLE-HEAD AR (outline-to-essay paradigm) ===")
    print(f"  ckpt: {CKPT}")
    print(f"  test: {TEST_PATH}")
    print(f"  num_eval: {NUM_EVAL}  K: {K}  fixed_len: {FIXED_LEN}  batch: {BATCH}  max_new: {MAX_NEW}")
    if not os.path.exists(TEST_PATH):
        print(f"ERROR: test set not found at {TEST_PATH}", file=sys.stderr)
        sys.exit(1)

    print(f"\nloading v77/v84 test set...")
    ...
    print(f"\n=== v84 DAG eval results ({dt:.1f}s) ===")
    print(f"  accuracy:       {acc:.1f}% ({correct}/{total})")
    print(f"  parse rate:     {parse_pct:.1f}% ({parseable}/{total})")
