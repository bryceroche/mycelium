...
def one(t):
    ...
    keys=("pres","ftype","op","islit","dig","args","res","query")+(("sel",) if "sel" in out else ())+(("dup",) if "dup" in out else ())+(("dargs",) if "dargs" in out else ())
...
json.dump(res,open('.cache/dup_axis_scan5.json','w'))
