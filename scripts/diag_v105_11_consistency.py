"""Decoder consistency diagnostic for v105.11 AR digit decoder.
...
unobserved cells:
  ...
  2. Greedy-decode digits via the AR chain (supports v105.11 concat-cond
     mechanism via V105_11_CONCAT_COND=1).
...
Interpretation:
  * If d2 responds to clamp changes → decoder learned conditional rules; the
    AR conditioning is load-bearing as v105.11 intended.
  * If d2 stays constant → decoder ignores its conditioning; v105.11
    Mechanism 1 (stronger cond_scale) was insufficient. Fall back to
    Mechanism 2 (V105_11_CONCAT_COND=1) or Mechanism 3
    (V105_11_COND_DROPOUT=0.5).
...
Usage:
  CKPT=.cache/fg_v105_5_ckpts/v105_11_prod_step5000.safetensors \
    V105_9_AR_COND_SCALE=2.0 \
    V105_11_NUMBER_MSE=1 \
    .venv/bin/python scripts/diag_v105_11_consistency.py

To check the concat-cond variant (if the ckpt was trained with it):
  CKPT=... V105_11_CONCAT_COND=1 V105_11_NUMBER_MSE=1 .venv/bin/python ...
"""
...
# Default env vars for v105.11. Caller can override via shell env.
os.environ.setdefault("V105_5_TASK", "1")
os.environ.setdefault("V105_11_NUMBER_MSE", "1")
os.environ.setdefault("V105_11_NUMBER_MSE_BETA", "1.0")
os.environ.setdefault("V105_11_CONCAT_COND", "0")
os.environ.setdefault("V105_11_COND_DROPOUT", "0.0")
os.environ.setdefault("V105_9_AR_DIGIT_DECODER", "1")
os.environ.setdefault("V105_9_AR_COND_SCALE", "2.0")
# v105.11 module-load mutex will force V105_8/V105_10 off, but be explicit:
os.environ.setdefault("V105_8_PER_NUMBER_READOUT", "0")
os.environ.setdefault("V105_10_DUAL_READOUT", "0")
...
def layernorm_np(x: np.ndarray, gain: np.ndarray, bias: np.ndarray, eps: float) -> np.ndarray:
...
def ar_decode_one_clamp(
    ...
    concat_W_np: np.ndarray | None = None,  # (2H, H) when concat-cond enabled
    ln_g_np: np.ndarray | None = None,      # (H,) — when LN_COND enabled
    ln_b_np: np.ndarray | None = None,      # (H,) — when LN_COND enabled
    ln_eps: float = 1e-5,
) -> tuple[np.ndarray, np.ndarray]:
    """Run the LSD-first AR chain on a stack of cell_hiddens.

    If concat_W_np is None: additive conditioning (default v105.9 / Mechanism 1).
      pos_hidden = cell_hidden + cond_accum

    If concat_W_np is provided: concat-and-project conditioning (Mechanism 2).
      pos_hidden = concat(cell_hidden, cond_accum, axis=-1) @ concat_W_np

    If ln_g_np/ln_b_np provided (Mechanism 4): LayerNorm BOTH cell_hidden and
    cond_accum before combining (additive or concat). Equalizes magnitudes so
    cond_accum is not buried under cell_hidden (~32 mag) by being only ~2 mag.
    ...
    ln_enabled = ln_g_np is not None and ln_b_np is not None
    for p in range(n_digits):
        cb_p = digit_codebook_np[p]                              # (10, H)
        if ln_enabled:
            cell_for_combine = layernorm_np(cell_hidden_np, ln_g_np, ln_b_np, ln_eps)
            cond_for_combine = layernorm_np(cond_accum, ln_g_np, ln_b_np, ln_eps)
        else:
            cell_for_combine = cell_hidden_np
            cond_for_combine = cond_accum
        if concat_W_np is not None:
            cat_in     = np.concatenate([cell_for_combine, cond_for_combine], axis=-1)  # (M, 2H)
            pos_hidden = cat_in @ concat_W_np                                            # (M, H)
        else:
            pos_hidden = cell_for_combine + cond_for_combine      # (M, H)
...
def main():
    ...
    print(f"=== v105.11 decoder consistency diag ===")
    print(f"  ckpt:         {CKPT}")
    print(f"  val:          {VAL_PATH}")
    print(f"  n_cells:      {N_CELLS}  batch: {BATCH}")
    print(f"  cond_scale:   {V105_9_AR_COND_SCALE}")
    print(f"  concat_cond:  {V105_11_CONCAT_COND}")
    print()
    ...
    # Pull the concat-cond projection if enabled.
    concat_W_np: np.ndarray | None = None
    if V105_11_CONCAT_COND and hasattr(model, "fg_v105_11_concat_W"):
        concat_W_np = model.fg_v105_11_concat_W.numpy()  # (2H, H)
        print(f"  loaded concat_W: shape {concat_W_np.shape}")

    # Pull LN params if Mechanism 4 enabled.
    ln_g_np: np.ndarray | None = None
    ln_b_np: np.ndarray | None = None
    if V105_11_LN_COND:
        ln_g_np = model.ln_f_g.numpy()
        ln_b_np = model.ln_f_b.numpy()
        print(f"  loaded ln_f for LN_COND: g shape {ln_g_np.shape}, b shape {ln_b_np.shape}")
    ...
    _, probs_base = ar_decode_one_clamp(
        ...
        concat_W_np=concat_W_np,
        ln_g_np=ln_g_np, ln_b_np=ln_b_np,
    )
    ...
    if max_kl > 0.05:
        verdict = "RESPONSIVE"
        explanation = (
            "decoder learned conditional rules → AR conditioning is load-bearing "
            "as v105.11 intended. Continue to OOD test."
        )
    elif max_kl > 0.005:
        verdict = "WEAKLY-RESPONSIVE"
        explanation = (
            "decoder shows some response to conditioning but signal is faint. "
            "Consider escalating to Mechanism 2 (V105_11_CONCAT_COND=1) or "
            "Mechanism 3 (V105_11_COND_DROPOUT=0.5)."
        )
    else:
        verdict = "UNRESPONSIVE"
        explanation = (
            "decoder appears to ignore its d1 conditioning. Mechanism 1 "
            "(stronger cond_scale) was insufficient. Escalate to Mechanism 2 "
            "(V105_11_CONCAT_COND=1) or Mechanism 3 (V105_11_COND_DROPOUT=0.5)."
        )
