#!/usr/bin/env bash
# v118-b smoke: piecewise_4phase + Perceiver bottleneck (LoRAs=0).
#
# Tests the Perceiver bottleneck at phase transitions (breaths 1, 3, 5).
# New params: 16 latents × 1024 hidden (QR-init) + 1 zero-init gate scalar.
# Step 0 forward should be byte-identical to v112b (gate=0 → no effect).
#
# Success criteria:
#   1. Code + JIT compile cleanly
#   2. Perceiver gate starts near 0 and grows from gradient signal
#   3. 300 steps without crash/OOM/hang
#   4. hard cell_acc at step 300 vs v118-a (same envelope, no perceiver)
set -euo pipefail
...
EVAL_BATCH=8 \
CKPT_EVERY=300 \
CKPT_LABEL="${CKPT_LABEL:-v118_b_smoke}" \
PYTHIA_INIT=1 \
DEV='PCI+AMD' \
