"""v105.5 hidden-state linear-probe diagnostic.
...
Run:
  CKPT=.cache/fg_v105_5_ckpts/v105_5_prod_step3000.safetensors \
    DEV='PCI+AMD' .venv/bin/python -u scripts/diag_v105_5_linear_probe.py

Constraints respected:
  • No edits to mycelium/factor_graph_v105_5.py — the tapped forward below is a
    LOCAL reproduction of `fg_breathing_forward_v105_5`'s last-breath path using
    only the public helpers (`embed_factor_graph_v105_5`,
    `fg_layer_forward_v105_5`, `apply_hierarchical_ib_codebook`,
    `apply_projection_waist`) plus the model's attached v105.5 parameters.
...
def cast_layers_fp32(model):
    """Cast model layer weights to fp32 (matches diag_v105_5_per_position_acc.py)."""
    def _cast(obj, attr):
        t = getattr(obj, attr)
...
# ---------------------------------------------------------------------------
# Tapped forward: reproduces fg_breathing_forward_v105_5 but returns the
...
def fg_forward_tapped_v105_5(
    ...
    # NEW v105.6 — per-position L0 W_in/b_in
    ...
    for k in range(K):
        ...
        # v105.5 (2026-06-02) — PPFFN now BEFORE L0 (matches trained model)
        ...
        if k == K - 1:
            ...
            # v105_5_prod was trained with V105_5_AR_DIGITS=1 LSD-first.
...
def main():
    print("=== v105.5 hidden-state linear-probe diagnostic ===", flush=True)
    ...
    for batch in val_loader.iter_eval(batch_size=BATCH):
        var_pre, var_with_mag, digit_logits = fg_forward_tapped_v105_5(
