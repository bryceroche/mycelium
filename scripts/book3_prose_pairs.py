"""book3_prose_pairs.py — THE FIRST REAL-PROSE INGEST, v0 (gen-10,
...
entries = [json.loads(l) for l in open(".cache/book3.jsonl")]
...
for e in entries:
    ...
    rows.append({"n_vars": 24, "m": 300, "text": e["raw"],
                 ...
                 "gen": {"shape": "prose-v0", "generation": 14,
                         "src_idx": e["lane_idx"]}})
with open(".cache/book3_prose_pairs.jsonl", "w") as f:
    for r in rows:
        f.write(json.dumps(r) + "\n")
print(f"[prose-v0] {len(rows)} raw-prose training pairs "
      f"({skipped} skipped on verification) -> .cache/book3_prose_pairs.jsonl")
