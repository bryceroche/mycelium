"""v110-photon factor graph training driver — v109pi with cos² waist gate."""
import gc
import os
import sys
import time
...
from mycelium.factor_graph_v110_photon import (
    V110_PHOTON_K_MAX, V110_PHOTON_N_MAX, V110_PHOTON_F_MAX, V110_PHOTON_N_HEADS,
    V110_PHOTON_N_DIGITS,
    V110_PHOTON_WAIST_DIM, V110_PHOTON_ALTERNATION, V110_PHOTON_HARD_BREATH_LEVEL,
    V110_PHOTON_VAR_LOSS_WEIGHT, V110_PHOTON_CALIB_WEIGHT,
    V110_PHOTON_FACTOR_AUX_WEIGHT,
    V110_PHOTON_CODEBOOK_N, V110_PHOTON_IB_CENTROIDS, V110_PHOTON_PHASE_SCALE,
    V110_PHOTON_GATE_PROFILE, V110_PHOTON_ALPHA,
    attach_fg_params_v110_photon, fg_v110_photon_parameters, fg_v110_photon_state_dict,
    _compile_jit_fg_step_v110_photon, _compile_jit_fg_eval_v110_photon,
)
...
from scripts.v109_factor_graph_train import (
    ...
    load_ckpt_v109, evaluate_v109,
)

DIFFICULTIES = ["easy", "medium", "hard"]

# Inherit infrastructure from v109 (v110-photon adds no new params).
collect_fg_params_v110_photon = collect_fg_params_v109
model_state_dict_v110_photon = model_state_dict_v109
load_ckpt_v110_photon = load_ckpt_v109
evaluate_v110_photon = evaluate_v109


def main():
    V110_PHOTON_TASK_LOCAL = int(getenv("V110_PHOTON_TASK", 0)) > 0
    assert V110_PHOTON_TASK_LOCAL, "V110_PHOTON_TASK=1 must be set"

    K          = int(getenv("V110_PHOTON_K_MAX",    str(V110_PHOTON_K_MAX)))
    ...
    CKPT_LABEL = getenv("CKPT_LABEL",               "v110_photon_smoke")
    RESUME_FROM = getenv("RESUME_FROM",             "")
    PYTHIA_INIT = int(getenv("PYTHIA_INIT",         "1")) > 0
    SEED       = int(getenv("SEED",                 "42"))
    N_CODE     = int(getenv("V110_PHOTON_CODEBOOK_N", str(V110_PHOTON_CODEBOOK_N)))
    IB_PATH    = getenv("V110_PHOTON_IB_CENTROIDS",   V110_PHOTON_IB_CENTROIDS)
    N_DIGITS   = int(getenv("V110_PHOTON_N_DIGITS",   str(V110_PHOTON_N_DIGITS)))
    WAIST_DIM  = int(getenv("V110_PHOTON_WAIST_DIM",  str(V110_PHOTON_WAIST_DIM)))
    ALTERNATION = int(getenv("V110_PHOTON_ALTERNATION", "1")) > 0
    HARD_LEVEL = int(getenv("V110_PHOTON_HARD_BREATH_LEVEL", "0")) > 0
    PHASE_SCALE = float(getenv("V110_PHOTON_PHASE_SCALE", str(V110_PHOTON_PHASE_SCALE)))
    GATE_PROFILE = getenv("V110_PHOTON_GATE_PROFILE", V110_PHOTON_GATE_PROFILE)
    PHOTON_ALPHA = float(getenv("V110_PHOTON_ALPHA", str(V110_PHOTON_ALPHA)))
    VW         = float(getenv("V110_PHOTON_VAR_LOSS_WEIGHT",  str(V110_PHOTON_VAR_LOSS_WEIGHT)))
    FW         = float(getenv("V110_PHOTON_FACTOR_AUX_WEIGHT", str(V110_PHOTON_FACTOR_AUX_WEIGHT)))
    AW         = float(getenv("V110_PHOTON_CALIB_WEIGHT",      str(V110_PHOTON_CALIB_WEIGHT)))
    N_MAX      = int(getenv("V110_PHOTON_N_MAX",    str(V110_PHOTON_N_MAX)))
    F_MAX      = int(getenv("V110_PHOTON_F_MAX",    str(V110_PHOTON_F_MAX)))

    DIFFICULTY_FILTER = os.environ.get("V110_PHOTON_DIFFICULTY_FILTER", "").strip() or None
    TRAIN_PATH   = getenv("V110_PHOTON_TRAIN",       ".cache/factor_graph_train.jsonl")
    VAL_PATH     = getenv("V110_PHOTON_VAL",         ".cache/factor_graph_test.jsonl")
    GSM8K_PATH   = getenv("V110_PHOTON_GSM8K_TRAIN", ".cache/gsm8k_factor_graphs_train.jsonl")
    GSM8K_RATIO  = float(getenv("V110_PHOTON_GSM8K_RATIO", "0.5"))
    EVAL_BATCHES = int(getenv("EVAL_BATCHES", "10"))
    EVAL_BATCH   = int(getenv("EVAL_BATCH",   str(BATCH)))

    print(f"=== v110-photon factor graph training (v109pi + cos² waist gate) ===")
    ...
    params   = collect_fg_params_v110_photon(model)
    ...
    if RESUME_FROM:
        ...
        load_ckpt_v110_photon(model, RESUME_FROM)
    ...
    synth_loader = FactorGraphLoaderV107(
        ...
        n_max=N_MAX, f_max=F_MAX, k_max=K, n_heads=V110_PHOTON_N_HEADS, seed=SEED,
    )
    ...
    dual_loader = DualDataLoaderV107(
        ...
        n_max=N_MAX, f_max=F_MAX, k_max=K, n_heads=V110_PHOTON_N_HEADS, seed=SEED + 1,
    )
    val_loader = FactorGraphLoaderV107(
        VAL_PATH, batch_size=EVAL_BATCH,
        difficulty_filter=None, curriculum=False,
        n_max=N_MAX, f_max=F_MAX, k_max=K, n_heads=V110_PHOTON_N_HEADS, seed=SEED + 2,
    )

    ckpt_dir = ".cache/fg_v110_photon_ckpts"
    os.makedirs(ckpt_dir, exist_ok=True)

    Tensor.training = True
    step_fn = _compile_jit_fg_step_v110_photon(
    ...
    eval_fn = _compile_jit_fg_eval_v110_photon(
    ...
    for step in range(1, STEPS + 1):
        ...
        if step % CKPT_EVERY == 0:
            ...
            safe_save(model_state_dict_v110_photon(model), ckpt_path)
