"""form_assemble16.py — G51 THE MIXTURE (2026-08-21, word given): deeds
...
print(f"[assemble16] anchor-law corpus: {_before} -> {len(_byid)} "
...
print(f"[assemble16] G55 wild-val split: diet {len(_clean)} / wild-val {len(_wildval)} (seed 555, banked)", flush=True)
diet=to_train(_clean,"b12diet")
REPS=int(os.environ.get("DIET_REPS","1"))
ND, NU = 0, len(diet)
mix=[r for _ in range(REPS) for r in diet]
print(f"[assemble16] PURE-WILD top-up mix: {NU} uniques x{REPS} (no base, no deeds)", flush=True)
with open('.cache/form_mix16.jsonl','w') as f:
    for r in mix: f.write(json.dumps(r)+"\n")
n=len(mix)
out=np.lib.format.open_memmap('.cache/phase1_alg_states_form16_states.npy', mode='w+',
                              dtype=np.float16, shape=(n,T_ALG,2048))
samples, ids2, mask, offsets = PH.tokenize('.cache/form_mix16.jsonl')
...
np.savez('.cache/phase1_alg_states_form16.npz', tokmask=mask.astype(np.uint8),
         sent=sent.astype(np.int8), mix_sha=mix_sha16('.cache/form_mix16.jsonl'),
         **{f"g_{k}":v for k,v in gold.items()})
print(f"[assemble16] STITCHED base+deeds+diet + gold + sha ({n} rows)", flush=True)
