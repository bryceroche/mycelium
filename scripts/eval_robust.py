"""
Robust evaluation of strategy_v3 checkpoint.

5 random seeds for state init, 500 eval problems per seed.
Reports per-seed accuracy + mean/std across seeds.
This tells us the TRUE accuracy, not a lucky single-seed number.
"""
...
class TwoStepArithmeticDataset:
...
def build_model(checkpoint_path, device):
    ...
    probe_head = nn.Linear(64, 1).to(device)
    ...
    return transformer, tokenizer, compressor, lora, probe_head


def thinking_pass(transformer, tokenizer, compressor, lora, input_ids, attention_mask, state, strategy, pass_num):
    ...
    new_state = F.normalize(new_state, dim=-1) * state_radius

    return new_state, new_strategy


def evaluate_seed(transformer, tokenizer, compressor, lora, eval_data, device, init_seed):
    """Evaluate with a specific random seed for state initialization."""
    ...
    for i in range(0, len(eval_data.samples), batch_size_eval):
        batch_samples = eval_data.samples[i:i + batch_size_eval]
        ...
        # Random state init from this seed
        state = torch.randn(bs, state_size, device=device)
        state = F.normalize(state, dim=-1) * state_radius
        strategy = torch.zeros(bs, strategy_size, device=device)

        # 2 thinking passes
        for pass_num in range(2):
            state, strategy = thinking_pass(
                transformer, tokenizer, compressor, lora,
...
def main():
    print("=" * 60)
    print("ROBUST EVALUATION: strategy_v3_85pct checkpoint")
    print("5 seeds x 500 problems = 2500 total evaluations")
    print("=" * 60)

    device = torch.device('cuda')
    checkpoint_path = '/home/ubuntu/mycelium/checkpoints/strategy_v3_85pct.pt'

    transformer, tokenizer, compressor, lora, probe_head = build_model(checkpoint_path, device)

    # Fixed eval dataset (same problems for all seeds)
    eval_data = TwoStepArithmeticDataset(num_samples=500, seed=999)
    ...
    print(f"  Effective per-step: {mean_acc**0.5:.1f}% (sqrt of two-step)")
    print(f"{'=' * 60}")

    if std_acc > 15:
        print("WARNING: High variance across seeds — model is init-sensitive")
    elif std_acc > 5:
        print("NOTE: Moderate variance — consider learned init or zero init")
    else:
        print("GOOD: Low variance — model is robust to init")

    if mean_acc > 70:
        print(f"VERDICT: Ready for three-step (ceiling at {(mean_acc/100)**1.5 * 100:.1f}%)")
    elif mean_acc > 50:
        print("VERDICT: Promising but may need more training before three-step")
    else:
        print("VERDICT: Robustness problem — fix before adding complexity")
