"""form_assemble17.py — G51 THE MIXTURE (2026-08-21, word given): deeds
...
print(f"[assemble17] anchor-law corpus: {_before} -> {len(_byid)} "
      f"(superseded {len(_re12)}, dropped {len(_skips)} skips, 100% anchored)", flush=True)
_clean=sorted(_byid.values(), key=lambda r: r["src_idx"])
import numpy as _np
_rng=_np.random.RandomState(555)
_vidx=set(_rng.choice(len(_clean), 20, replace=False))
_wildval=[r for i,r in enumerate(_clean) if i in _vidx]
with open('.cache/g55_wildval.jsonl','w') as _f:
    for r in _wildval: _f.write(json.dumps(r)+"\n")
_clean=[r for i,r in enumerate(_clean) if i not in _vidx]
print(f"[assemble17] G55 wild-val split: diet {len(_clean)} / wild-val {len(_wildval)} (seed 555, banked)", flush=True)
diet=to_train(_clean,"b12diet")
REPS=int(os.environ.get("DIET_REPS","1"))
ND, NU = 0, len(diet)
mix=[r for _ in range(REPS) for r in diet]
print(f"[assemble17] PURE-WILD top-up mix: {NU} uniques x{REPS} (no base, no deeds)", flush=True)
with open('.cache/form_mix17.jsonl','w') as f:
    for r in mix: f.write(json.dumps(r)+"\n")
n=len(mix)
out=np.lib.format.open_memmap('.cache/phase1_alg_states_form17_states.npy', mode='w+',
                              dtype=np.float16, shape=(n,T_ALG,2048))
samples, ids2, mask, offsets = PH.tokenize('.cache/form_mix17.jsonl')
...
from mycelium.clause_grains import math_clause_indices, is_single_sentence
from phase1_algebra_head import SENT_MAX
sent=np.stack([math_clause_indices(s["text"],o,mask[i],sent_indices,T_ALG,SENT_MAX) for i,(s,o) in enumerate(zip(samples,offsets))])
_ng=sum(1 for s2 in samples if is_single_sentence(s2["text"]))
assert _ng>0, "POSITIVE PRESENCE: math grains never engaged"
print(f"[assemble17] math-grain rows: {_ng}/{len(samples)} (organ-gated)", flush=True)
np.savez('.cache/phase1_alg_states_form17.npz', tokmask=mask.astype(np.uint8),
         sent=sent.astype(np.int8), mix_sha=mix_sha16('.cache/form_mix17.jsonl'),
         **{f"g_{k}":v for k,v in gold.items()})
print(f"[assemble17] STITCHED base+deeds+diet + gold + sha ({n} rows)", flush=True)
