...
rows=[json.loads(l) for l in open('.cache/form_mix5.jsonl')]
base=np.load('.cache/phase1_alg_states_form4_states.npy', mmap_mode='r')
n=len(rows); nb=base.shape[0]
out=np.lib.format.open_memmap('.cache/phase1_alg_states_form5_states.npy', mode='w+', dtype=np.float16, shape=(n,T_ALG,2048))
CH=4096
...
out.flush(); del out
samples, ids2, mask, offsets = PH.tokenize('.cache/form_mix5.jsonl')
...
np.savez('.cache/phase1_alg_states_form5.npz', tokmask=mask.astype(np.uint8),
         sent=sent.astype(np.int8), mix_sha=mix_sha16('.cache/form_mix5.jsonl'),
