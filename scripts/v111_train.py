"""v111 coherent-photon training driver."""
import gc
import os
import sys
import time
...
from mycelium.factor_graph_v110_step3 import (
    ...
    V110_STEP3_PHOTON_ALPHA,
)
from mycelium.factor_graph_v111 import (
    V111_DIM_DELTA_INIT,
    attach_fg_params_v111,
    fg_v111_parameters, fg_v111_state_dict,
    _compile_jit_fg_step_v111,
    compile_jit_eval_v111,
)
...
def load_ckpt_v111(model, path: str) -> None:
    from tinygrad.nn.state import safe_load
    load_ckpt_v110_acc(model, path)
    sd = safe_load(path)
    if "fg_v113_dim_delta" in sd:
        dst = model.fg_v113_dim_delta
        src = sd["fg_v113_dim_delta"].to(dst.device).realize()
        dst.assign(src).realize()
        print(f"  loaded v111 param: fg_v113_dim_delta", flush=True)
...
def main():
    ...
    CKPT_LABEL = getenv("CKPT_LABEL",              "v111_smoke")
    ...
    DIM_DELTA_INIT = float(getenv("V111_DIM_DELTA_INIT", str(V111_DIM_DELTA_INIT)))
    ...
    print(f"=== v111 coherent-photon training ===")
    print(f"  dim_delta_init={DIM_DELTA_INIT}  SBP_noise={SBP_NOISE}")
    print(f"  alternation={ALTERNATION}  phase_scale={PHASE_SCALE}  photon_alpha={PHOTON_ALPHA}")
    ...
    params   = fg_v111_parameters(model)
    ...
    if RESUME_FROM:
        ...
        load_ckpt_v111(model, RESUME_FROM)
    ...
    step_fn = _compile_jit_fg_step_v111(
        ...
        phase_scale=PHASE_SCALE, photon_alpha=PHOTON_ALPHA,
        n_max=N_MAX, f_max=F_MAX, n_digits=N_DIGITS, grad_clip=1.0,
    )
    eval_fn = compile_jit_eval_v111(
    ...
    for step in range(1, STEPS + 1):
        ...
        if step % LOG_EVERY == 0:
            dt = time.time() - t0
            dim_delta_norm = float(np.linalg.norm(model.fg_v113_dim_delta.numpy()))
            dim_delta_max  = float(np.abs(model.fg_v113_dim_delta.numpy()).max())
            print(
                ...
                f"dd_norm={dim_delta_norm:.3f} dd_max={dim_delta_max:.3f}  "
        ...
        if step % CKPT_EVERY == 0:
            ...
            safe_save(fg_v111_state_dict(model), ckpt_path)
