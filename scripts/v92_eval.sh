#!/bin/bash
# v92 eval — eval_v85_dag.py on a v92 ckpt.
set -e
cd "$(dirname "$0")/.."

V92_CKPT="${V92_CKPT:-.cache/gsm8k_steps_ckpts/v92_smoke_step100.safetensors}"
NUM_EVAL="${NUM_EVAL:-60}"

# Same env needed for model construction (matches v92_smoke_train.sh).
...
# v92: NO reinit on eval — inherit values from the ckpt. Threshold tunable
# via V92_ACTIVE_EVAL_THRESHOLD (default 0.5).
export V92_REINIT_ARG_POS_EMB=0
export V92_RESET_ACTIVE_HEAD_NEUTRAL=0
export V92_ACTIVE_EVAL_THRESHOLD="${V92_ACTIVE_EVAL_THRESHOLD:-0.5}"
...
CKPT_BASENAME=$(basename "$V92_CKPT" .safetensors)
/home/bryce/mycelium/.venv/bin/python -u scripts/eval_v85_dag.py 2>&1 | tee ".cache/${CKPT_BASENAME}_eval.log"
