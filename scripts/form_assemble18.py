"""form_assemble18.py — G51 THE MIXTURE (2026-08-21, word given): deeds
...
print(f"[assemble18] anchor-law corpus: {_before} -> {len(_byid)} "
...
print(f"[assemble18] PURE-WILD top-up mix: {NU} uniques x{REPS} (no base, no deeds)", flush=True)
with open('.cache/form_mix18.jsonl','w') as f:
    for r in mix: f.write(json.dumps(r)+"\n")
n=len(mix)
out=np.lib.format.open_memmap('.cache/phase1_alg_states_form18_states.npy', mode='w+',
                              dtype=np.float16, shape=(n,T_ALG,2048))
samples, ids2, mask, offsets = PH.tokenize('.cache/form_mix18.jsonl')
...
np.savez('.cache/phase1_alg_states_form18.npz', tokmask=mask.astype(np.uint8),
         sent=sent.astype(np.int8), mix_sha=mix_sha16('.cache/form_mix18.jsonl'),
         **{f"g_{k}":v for k,v in gold.items()})
print(f"[assemble18] STITCHED base+deeds+diet + gold + sha ({n} rows)", flush=True)
