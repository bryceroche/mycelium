...
rows=[json.loads(l) for l in open('.cache/aim_mix.jsonl')]
base=np.load('.cache/phase1_alg_states_gen23_states.npy', mmap_mode='r')
n=len(rows); nb=base.shape[0]
out=np.lib.format.open_memmap('.cache/phase1_alg_states_aim_states.npy', mode='w+', dtype=np.float16, shape=(n,T_ALG,2048))
CH=4096
...
out.flush(); del out
samples, ids2, mask, offsets = PH.tokenize('.cache/aim_mix.jsonl')
gold=build_gold(samples, offsets)
sent=np.stack([sent_indices(s["text"],o,mask[i]) for i,(s,o) in enumerate(zip(samples,offsets))])
np.savez('.cache/phase1_alg_states_aim.npz', tokmask=mask.astype(np.uint8),
