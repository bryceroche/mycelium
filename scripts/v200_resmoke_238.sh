#!/usr/bin/env bash
# v200 #238 — WRITE operator (shared K-slot notebook, §2 WRITE spec).
#
...
#
# Run:
#   cd /home/bryce/mycelium && bash scripts/v200_resmoke_238.sh
#
set -euo pipefail
...
echo "=== v200 #238 — WRITE OPERATOR (shared K-slot notebook) ==="
...
echo "  ADVISORY: per-op family = mod-4 proxy (see line 1 of train_238.log)"
echo ""
...
CKPT_LABEL="${CKPT_LABEL:-v200_perceiver_238_write8}" \
SEED=42 \
B_REF="${B_REF:-4}" \
SMOKE_DIR="$SMOKE_DIR" \
"$PYTHON" -u scripts/v200_resmoke_238.py 2>&1 | tee "$SMOKE_DIR/train_238_console.log"

STEPS_DONE="${STEPS:-2000}"
EVAL_JSON="$SMOKE_DIR/step${STEPS_DONE}_eval_238.json"

echo ""
echo "=== #238 WRITE run complete ==="
echo "Final log line:"
tail -1 "$SMOKE_DIR/train_238.log"
echo ""
...
echo ""
echo "Instrumentation timeseries: $SMOKE_DIR/instrumentation_238.json"
