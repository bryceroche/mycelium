...
env ALG_FTYPES=8 RESUME=1 ALG_TRAIN=.cache/form_mix10.jsonl ALG_TRAIN_NAME=form10 \
    ...
    ALG_CKPT=.cache/g48_bridge.safetensors ALG_FREEZE_DUP=1 STEPS=4000 LR=2.5e-5 BATCH=8 SEED=138 SNAP_EVERY=0 \
    $PY scripts/phase1_algebra_head.py --train | tail -1
echo "== RING (bigtest straight) =="
env ALG_FTYPES=8 ALG_CKPT=.cache/g48_bridge.safetensors ALG_TEST=.cache/algebra_nl_bigtest.jsonl ALG_TEST_NAME=bigtest $PY scripts/phase1_algebra_head.py --eval | grep TOTAL
echo "== G42 COMPLETE =="
