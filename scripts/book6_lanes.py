"""book6_lanes.py — BOOK 6, stage 1 (2026-07-24): the first lane pass under
GEN-20 (spans-fed, ration-trained, fully attributable). Fresh L4/L5 stock beyond every consumed
candidate; the re-pricing question pinned: does the crowned gate beat
book-4-era 71% L3? Draws L4/L5 harvest candidates outside
...
N_CAND = int(os.environ.get("B6_CAND", "200"))
...
# also every candidate the first lane pass consumed (banked or refused)
for e in json.load(open(".cache/book4_lanes.json")):
    used.add(e["src_idx"])
for e in json.load(open(".cache/book5_lanes.json")):
    used.add(e["src_idx"])
...
cands = pool[:N_CAND]
print(f"[book6] harder-strata pool: {len(pool)} bankable "
...
for ci, x in enumerate(cands):
    texts = [x["problem"]] + [permuted_view(x["problem"], 600000 + 10 * ci + k)
                              for k in range(1, 5)]
    ...
    if cnt >= 3 and top == x["answer"]:
        ...
        lane1.append(dict(raw=x["problem"], answer=x["answer"],
                          ...
                          gate="lattice-vote+answer-key", generation="16", book=5))
    elif len(votes) >= 2:
        lane = "L2"
    else:
        lane = "L3"
...
c = Counter(l["lane"] for l in lanes)
print(f"\n[book6-lanes] {dict(c)} of {len(cands)}  "
      f"(re-pricing watch: book-4-era baseline 71% L3)")
json.dump(lanes, open(".cache/book6_lanes.json", "w"))
with open(".cache/book6_lane1.jsonl", "w") as f:
    for b in lane1:
        f.write(json.dumps(b) + "\n")
print(f"[book6] lane-1 free entries: {len(lane1)} -> .cache/book6_lane1.jsonl")
print("[book6] repair bench + surgery lists -> .cache/book6_lanes.json")
