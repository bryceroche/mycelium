...
def _eager_grad_norm_step(
    ...
    if _iw > 0.0:
        ...
        for _k in range(1, K):
            _imp = _imp + (_CE_LIST[_k] - _CE_LIST[_k - 1] + _m).relu()
...
def main() -> None:
    ...
    for _an in dir(model):
        ...
        if _t is not None and hasattr(_t, "shape") and hasattr(_t, "grad"):
            _name_of[id(_t)] = _an
    ...
    for step in range(1, STEPS + 1):
        ...
        if step % PB_EVERY == 0:
            ...
            if step >= _ka and _exc < 0.02 and abs(_spread) < 0.01:
                ...
                safe_save(fg_v200_state_dict(model), os.path.join(CKPT_DIR, f"{CKPT_LABEL}_KILLED_step{step}.safetensors"))
                raise SystemExit(3)
            if K > 1:
