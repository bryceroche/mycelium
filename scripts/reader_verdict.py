"""reader_verdict.py — the manifest-writing verdict (2026-07-13; the law's
...
Usage: reader_verdict.py <chain-log-path>
"""
...
# restructure: >=7 AND any single miss must be [45] (certificate on file)
probes = re.findall(r"\[\s*(\d+)\] gold\s+\d+ \| votes .*-> (BANKS|refuses)", log)
misses = [p for p, v in probes[-8:] if v == "refuses"]
acc_ok = 1 if (acc == 8 or (acc == 7 and misses == ["45"])) else 0
...
BARS = [
    ...
    ("acceptance(only-[45]-may-miss)", acc_ok, 1),
    ("sq", perkind("sq"), 0.70),
...
print("=== READER_V1 VERDICT (mechanical) ===")
for n, v, b in BARS:
...
m.update({
    "gen_id": "12", "date": "2026-07-13",
    "parser_ckpt": ".cache/phase1_reader_v1.safetensors",
    "corpora": {"train": ".cache/algebra_mixed12_train.jsonl",
...
m["hashes"]["parser"] = fh(".cache/phase1_reader_v1.safetensors")
m["hashes"]["train"] = fh(".cache/algebra_mixed12_train.jsonl")
