"""v110-step3 stochastic-BP consistency training driver.

Warm-starts from v110_step3_cont8_step1000.safetensors. Adds a dual-forward
KL consistency loss to widen the inference-time MC-BP noise tolerance window.

Env overrides (in addition to standard v110-step3 envs):
  V110_STEP3_SBP_NOISE_SCALE=0.02     training noise injected on x at start of breath
  V110_STEP3_SBP_CONS_WEIGHT=0.1      KL(det, noisy) consistency weight
"""
import gc
import os
import sys
...
from mycelium.factor_graph_v110_step3 import (
    ...
    _compile_jit_fg_eval_v110_step3,
)
from mycelium.factor_graph_v110_step3_sbp import (
    V110_STEP3_SBP_NOISE_SCALE, V110_STEP3_SBP_CONS_WEIGHT,
    _compile_jit_fg_step_v110_step3_sbp_simple,
)
...
def main():
    ...
    CKPT_LABEL = getenv("CKPT_LABEL",              "v110_step3_sbp_smoke")
    ...
    SBP_NOISE  = float(getenv("V110_STEP3_SBP_NOISE_SCALE", str(V110_STEP3_SBP_NOISE_SCALE)))
    SBP_CONS_W = float(getenv("V110_STEP3_SBP_CONS_WEIGHT",  str(V110_STEP3_SBP_CONS_WEIGHT)))
    ...
    print(f"=== v110-step3 SBP training (stochastic-BP consistency) ===")
    print(f"  noise_scale={SBP_NOISE}  cons_weight={SBP_CONS_W}")
    ...
    ckpt_dir = ".cache/fg_v110_step3_sbp_ckpts"
    os.makedirs(ckpt_dir, exist_ok=True)

    # ------------------------------------------------------------------
    # Noise tensors: noise sampled fresh per step. noise_scale is a Tensor
    # JIT input — alternate σ vs 0 per step (50/50) so the model sees BOTH
    # noisy and det forwards during training, generalizing to MC-BP at
    # inference (averaging N noisy forwards + det baseline both in distribution).
    # ------------------------------------------------------------------
    ns_zero_np  = np.array([0.0],       dtype=np.float16)
    ns_noisy_np = np.array([SBP_NOISE], dtype=np.float16)

    Tensor.training = True
    step_fn = _compile_jit_fg_step_v110_step3_sbp_simple(
    ...
    eval_fn = _compile_jit_fg_eval_v110_step3(
    ...
    print(f"\ntraining (simple SBP: 50% noisy@σ={SBP_NOISE}, 50% det)...\n")
    t0 = time.time()
    ...
    for step in range(1, STEPS + 1):
        ...
        # Fresh stochastic noise + 50/50 noisy/det per step
        is_noisy = (step % 2 == 1)
        if is_noisy:
        ...
        if step % PER_BREATH_EVERY == 0:
            ...
            uncert = [max(1.0 - c, UNCERTAINTY_MIN) for c in calibs]
            ratios = [step_mags[k] / uncert[k] for k in range(K)]
            ...
            print(f"  calib[B0..B{K-1}]:            {cal_str}  "
                  f"(model's confidence per breath)", flush=True)
            print(f"  step/(1-calib) [B0..B{K-1}]:  {r_str}  "
