"""v110-step3 + Gumbel-ST tree-feedback training driver.

Three modes via env:
  V110_STEP3_SBP_NOISE_SCALE=0     pure fb-gumbel (Path D)
  V110_STEP3_SBP_NOISE_SCALE=0.02  fb-gumbel + SBP (Path E)

Gumbel noise is freshly sampled per training step. At inference, pass
zeros (deterministic ST-argmax).

V110_STEP3_FB_GUMBEL_TAU controls the softmax temperature. τ=1 gives
near-categorical sampling; τ→0 gives near-argmax.
"""
import gc
import os
import sys
...
from mycelium.factor_graph_v110_step3_fb import V110_STEP3_FB_GATE_INIT
from mycelium.factor_graph_v110_step3_fb_gumbel import (
    V110_STEP3_FB_GUMBEL_TAU,
    attach_fg_params_v110_step3_fb_gumbel,
    fg_v110_step3_fb_gumbel_parameters, fg_v110_step3_fb_gumbel_state_dict,
    _compile_jit_fg_step_v110_step3_fb_gumbel,
    compile_jit_eval_v110_step3_fb_gumbel,
)
...
from scripts.v110_acc_factor_graph_train import evaluate_v110_acc
...
def main():
    ...
    CKPT_LABEL = getenv("CKPT_LABEL",              "v110_step3_fb_gumbel_smoke")
    ...
    FB_GATE_INIT = float(getenv("V110_STEP3_FB_GATE_INIT", str(V110_STEP3_FB_GATE_INIT)))
    GUMBEL_TAU   = float(getenv("V110_STEP3_FB_GUMBEL_TAU", str(V110_STEP3_FB_GUMBEL_TAU)))
    ...
    sbp_mode = "OFF (fb-gumbel only)" if SBP_NOISE == 0.0 else f"ON σ={SBP_NOISE} (fb-gumbel + SBP)"
    print(f"=== v110-step3 FB-GUMBEL training (Gumbel-ST tree-feedback) ===")
    print(f"  feedback_gate_init={FB_GATE_INIT}  gumbel_tau={GUMBEL_TAU}  SBP={sbp_mode}")
    ...
    params   = fg_v110_step3_fb_gumbel_parameters(model)
    ...
    if RESUME_FROM:
        ...
        load_ckpt_v110_step3_fb(model, RESUME_FROM)
    ...
    ckpt_dir = ".cache/fg_v110_step3_fb_gumbel_ckpts"
    ...
    step_fn = _compile_jit_fg_step_v110_step3_fb_gumbel(
        ...
        n_max=N_MAX, f_max=F_MAX, n_digits=N_DIGITS, tau=GUMBEL_TAU, grad_clip=1.0,
    )
    eval_fn = compile_jit_eval_v110_step3_fb_gumbel(
        model, K=K, B=EVAL_BATCH, tau=GUMBEL_TAU,
    ...
    print(f"\ntraining (SBP={sbp_mode}, gumbel tau={GUMBEL_TAU})...\n")
    t0 = time.time()
    ...
    rng_gumbel = np.random.RandomState(SEED + 13)
    ...
    for step in range(1, STEPS + 1):
        ...
        # SBP noise (50/50 alternating if SBP_NOISE > 0)
        ...
        # Gumbel noise: g = -log(-log(uniform(eps, 1-eps)))
        # Restrict u to [1e-4, 1-1e-4] → gumbel ∈ approx [-2.7, +9.2]
        # Bounded range avoids fp16 softmax overflow + extreme tail blowups.
        u = rng_gumbel.uniform(low=1e-4, high=1.0 - 1e-4,
                               size=(K, BATCH, N_MAX, N_DIGITS, 10)).astype(np.float32)
        gumbel_np = (-np.log(-np.log(u))).clip(-10.0, 10.0).astype(np.float16)
        gumbel_t  = Tensor(gumbel_np).cast(dtypes.half).contiguous().realize()

        outs = step_fn(
            domain_init, node_kinds, staging_mask, head_op_mask,
            gold_digits_t, gold_bins, obs_mask, fgb_t, fv_t,
            noise_t, ns_t, gumbel_t,
        )
        ...
        if step % LOG_EVERY == 0:
            ...
            fb_gate_val = float(model.fg_v111_feedback_gate.numpy()[0])
            print(
        ...
        if step % CKPT_EVERY == 0:
            ...
            safe_save(fg_v110_step3_fb_gumbel_state_dict(model), ckpt_path)
