...
"""v200 Stage 1C #235 smoke — §1A.E.4 READ-dominance fix (α·RMSNorm(read_ctx)).
...
Runs in sequence:
  ...
  4. Log 4-checkpoint within-breath per-element scale trajectory (§1A.E.9).
  5. Report alpha_read value + grad_norm at step 200 for §10 row 5 strain detection.

Architecture changes vs #234:
  +norm_read_ctx  RMSNorm on read_ctx before residual add (ones-init gain)
  +alpha_read     learnable scalar on α·RMSNorm(read_ctx), init=1.0 (NOT zero-init)
  arch_version suffix: prenorm4_gate-2_readnorm

Predicted per-element scale trajectory (§1A.E.9):
  post-norm_breath ~1.0   (RMSNorm output, gain init=1)
  post-READ-add   ~2.0   (bounded read_ctx ~1 + z ~1)
  post-THINK      ~3.5   (4 Llama layers, each pre-normed residual addition)
  post-blend      ~1.6   (z_pre≈1, gate=0.119, blend=1+0.119*(4-1)=1.36)
...
Output artifacts:
  ...
  .cache/v200_smoke/train_200_step_235.log   — smoke log (first line ADVISORY: prefix if any)
  .cache/v200_smoke/step200_eval_235.json    — eval + §1A.E.4 grid cell + §1A.E.9 trajectory
  .cache/v200_smoke/grad_norms_235.npz       — per-group grad norms (incl. alpha_read)
  .cache/v200_smoke/persistence/step200_z_235.npz  — sampled z traces

Usage:
  cd /home/bryce/mycelium
  V200_TASK=1 V200_STAGE2A_WAIST=1 .venv/bin/python scripts/v200_resmoke_235.py
"""
...
def _get_arch_version(config_sig: str = "K8_L32_prenorm4_gate-2_readnorm") -> str:
...
def _param_group_grad_norms(model: _Obj, stage2a_waist: bool) -> dict[str, float]:
    ...
    # alpha_read: named separately (§1A.C requires it; §10 row 5 strain check uses it)
...
# ---------------------------------------------------------------------------
# §1A.E.9 Four-checkpoint within-breath per-element scale trajectory
# ---------------------------------------------------------------------------

def _per_elem_scale(z: "np.ndarray") -> float:
    """Per-element scale: ‖z‖ / sqrt(numel), averaged over batch + latent positions."""
    numel = z.shape[-1]  # H = hidden_dim
    norms = np.linalg.norm(z, axis=-1)  # (B, L)
...
def measure_within_breath_trajectory(
    ...
    """Capture per-element scale at 4 within-breath checkpoints for §1A.E.9.

    Checkpoints per breath k:
      1. post-norm_breath  — just after breath-start RMSNorm fires
      2. post-READ-add     — just after z = z + α·RMSNorm(read_ctx)
      3. post-THINK        — just after Llama L0-L3 forward returns
      4. post-blend        — just after z = z_pre + gate_k*(z - z_pre)

    Returns dict with:
      trajectory_per_breath : list of K lists of 4 floats
      trajectory_avg         : [avg1, avg2, avg3, avg4] averaged over breaths
    """
    from mycelium.factor_graph_v200 import (
    ...
    for k in range(K):
        ...
        # CHECKPOINT 4: post-blend
        z4_np = latents.cast(dtypes.float).realize().numpy()
        s4 = _per_elem_scale(z4_np)

        traj_per_breath.append([s1, s2, s3, s4])

    Tensor.training = was_training

    # Average over breaths
    arr = np.array(traj_per_breath)  # (K, 4)
...
def _check_trajectory_match(
    ...
    predicted defaults to [1.0, 2.0, 3.5, 1.6] per §1A.E.9 table.
    A miss localizes which checkpoint broke (for routing per §1A.E.7).
    """
    if predicted is None:
        predicted = [1.0, 2.0, 3.5, 1.6]
...
def main() -> None:
    ...
    CKPT_LABEL = getenv("CKPT_LABEL",  "v200_perceiver_235_readnorm")
    SMOKE_DIR  = getenv("SMOKE_DIR",   ".cache/v200_smoke")
    REF_DIR    = os.path.join(SMOKE_DIR, "reference_curves")

    SMOKE_LOG_PATH  = os.path.join(SMOKE_DIR, "train_200_step_235.log")
    EVAL_JSON_PATH  = os.path.join(SMOKE_DIR, "step200_eval_235.json")
    GRAD_NORMS_PATH = os.path.join(SMOKE_DIR, "grad_norms_235.npz")
    PERSIST_DIR     = os.path.join(SMOKE_DIR, "persistence")
    PERSIST_Z_PATH  = os.path.join(PERSIST_DIR, "step200_z_235.npz")
    ...
    # #235: prenorm4 (4th breath-boundary RMSNorm) + gate-2 (delta_gate init=-2.0)
    #       + readnorm (α·RMSNorm(read_ctx) at READ residual-add site, §1A.E.4)
    config_sig  = f"K{K}_L{N_LATENTS}_prenorm4_gate-2_readnorm"
    ...
    if advisory_prefix:
        log(advisory_prefix)
    log("=" * 72)
    log("v200 Stage 1C #235 SMOKE (§1A.E.4 READ-dominance fix: α·RMSNorm(read_ctx))")
    ...
    log(f"  Five RMSNorms: norm_breath + norm_read + norm_read_ctx(NEW) + norm_commit + norm_readout")
    ...
    log(f"  Predicted trajectory (§1A.E.9): post_norm=1.0  post_read=2.0  post_think=3.5  post_blend=1.6")
    log("=" * 72)
    ...
    grad_norm_keys = [
        ...
        "waist_down_proj", "waist_up_proj",
        "tree_readout", "calib_head", "delta_gate", "breath_embed",
        "alpha_read",  # #235: named separately for §10 row 5 strain detection (§1A.C)
    ]
    ...
    # ====================================================================
    # §1A.E.9 QUANTITATIVE TRAJECTORY MATCH (4-checkpoint within-breath)
    # ====================================================================
    log("\n§1A.E.9 — Four-checkpoint within-breath per-element scale trajectory:")
    log("  Predicted: post_norm=1.0  post_read=2.0  post_think=3.5  post_blend=1.6  (±30% tolerance)")
    Tensor.training = False
    ...
    log(f"  Measured avg over K breaths: post_norm={traj_avg[0]:.3f}  post_read={traj_avg[1]:.3f}  "
        f"post_think={traj_avg[2]:.3f}  post_blend={traj_avg[3]:.3f}")
    log(f"  Per-breath trajectory:")
    for k_idx, row in enumerate(traj_result["trajectory_per_breath"]):
        log(f"    k={k_idx}: norm={row[0]:.3f}  read={row[1]:.3f}  think={row[2]:.3f}  blend={row[3]:.3f}")
    traj_check = _check_trajectory_match(traj_avg)
    log(f"  Trajectory match: {traj_check['trajectory_match']}")
    ...
    for ckpt, ok in traj_check["per_checkpoint_match"].items():
        log(f"    {ckpt}: {'PASS' if ok else 'FAIL'}")

    # ====================================================================
    # §1A.C §10 row 5 strain detection: alpha_read value + grad norm traj
    # ====================================================================
    log("\n§10 row 5 strain detection — alpha_read scalar:")
    ...
    if alpha_val > 5.0:
        ...
        # Check for alternating sign or wild variation (breath-conflicted gradient)
    ...
    if n_pass == 6:
        final_line = (
            ...
            f"run=235  steps={STEPS}  loss={end_loss:.4f}  "
    ...
    eval_json = {
        ...
        "e4_grid": e4_result,
        # §1A.E.9 quantitative trajectory match (per-element scale at 4 checkpoints)
        ...
        "cont_control": {
            ...
            "note": "Stage 1C #235 smoke; Gate A waived; 200 steps vs full chain",
        },
    ...
    print(f"§1A.E.9 trajectory match: {traj_check['trajectory_match']}  avg={[f'{v:.2f}' for v in traj_avg]}")
