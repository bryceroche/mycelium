"""v87 diagnostic — verify slot_pos reinit breaks attention symmetry.

Loads the v86 step300 ckpt, optionally REINITIALIZES slot_pos_embed and
v86_args_slot_pos at scale V87_SLOT_POS_INIT_SCALE, then computes per-slot
args cross-attn over 3 example problems.

Compare:
  - V87_SLOT_POS_INIT_SCALE=0 (or unset): baseline (v86), expect JSD ~0.001
  - V87_SLOT_POS_INIT_SCALE=0.5 with reinit: expect JSD > 0.1

Reuses the v86 diagnostic's encoder + cross-attn invocation; only adds the
optional reinit between ckpt load and forward.
"""
import os
import sys
...
def main():
    print("=== v87 slot_pos reinit diagnostic ===")
    ...
    # v87 reinit applied AFTER load_state_dict.
    V87_SCALE = float(os.environ.get("V87_SLOT_POS_INIT_SCALE", "0.0"))
    if V87_SCALE > 0.0:
        dec = model.v85_slot_decoder
        K_max_v, H_v = dec.slot_pos_embed.shape
        new_pos = Tensor.uniform(
            K_max_v, H_v, low=-V87_SCALE, high=V87_SCALE,
        ...
        print(f"[v87] reinitialized slot_pos at scale {V87_SCALE}")
    else:
        print("[v87] no reinit (V87_SLOT_POS_INIT_SCALE=0)")
    ...
    examples = examples[:3]
    print(f"\n=== running v86/v87 args attn on {len(examples)} samples ===")
    ...
    for ei, ex in enumerate(examples):
        ...
        # breathe_with_lookup may return tuple or dict — handle both.
        if isinstance(out, tuple):
            # Older shape: (final, match_weights, per_breath_x, waist_per_breath)
            if len(out) >= 4:
                waist_per_breath = out[3]
            else:
                # Try: (final, match_weights, ..., waist_per_breath) last position
                waist_per_breath = out[-1]
        ...
        if waist_per_breath is None:
            print("ERROR: waist_per_breath is None — need return_waist_compressed=True path")
            return
        ...
        if "args_attn" not in decoder_out:
            print(f"  [problem {ei}] V86_ARGS_CROSS_ATTN not active — no args_attn returned.")
            return
    ...
    print(f"\n=== SUMMARY ===")
    print(f"V87_SLOT_POS_INIT_SCALE={V87_SCALE}")
    print(f"mean pairwise JSD per problem: {pjsds_all}")
    print(f"overall mean pairwise JSD: {np.mean(pjsds_all):.4f}")
    print(f"target: > 0.1; v86 baseline ~0.001; uniform ceiling ln(2)={np.log(2):.4f}")
