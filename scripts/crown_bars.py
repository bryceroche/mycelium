"""crown_bars.py — THE CROWN FIRE'S BARS (2026-07-21, pinned at charter).
...
p = build_params(0)
sd = safe_load(".cache/crown_reader.safetensors")
...
# (1) acceptance on held-out greedy mints
trained_knots = {json.loads(l)["knot"] for l in open(".cache/crown_corpus_blind.jsonl")}
held = [json.loads(l) for l in open(".cache/crown_corpus_greedy.jsonl")]
held = [r for r in held if r["knot"] not in trained_knots][:60]
acc = sum(vote5(r["macro"]["text"], r["answer"], 500000 + 10 * i)[0]
          for i, r in enumerate(held))
rate = acc / max(len(held), 1)
print(f"[BAR 1] FRAC_OF acceptance (held-out, n={len(held)}): {acc} = {rate:.0%} "
      f"({'PASS' if rate >= 0.70 else 'FAIL'} vs 70%)")
