"""v112 dual-notebook training driver.

Cold-start from Pythia-410M init. Two accumulate-style notebooks:
  COMMIT notebook  — written on EVEN (collapse) breaths, read every breath
  PROPAGATION nb   — written on ODD (expand) breaths, read every breath

Both use causal cross-attention. Both have zero-init W_o → inert at step 0;
model learns to use them from scratch through task gradient.

Env vars use the V110_STEP3_* prefix since v112 is built on the v110-step3
base. Add CKPT_LABEL/STEPS/BATCH/LR/etc. as usual.
"""
import gc
import os
import sys
...
from mycelium.factor_graph_v112 import (
    attach_fg_params_v112,
    fg_v112_parameters, fg_v112_state_dict,
    _compile_jit_fg_step_v112,
    compile_jit_eval_v112,
)
...
def load_ckpt_v112(model, path: str) -> None:
    """Load v110-acc ckpt + v112 commit/prop notebook params if present.

    Existing v110-step3 ckpts (without v112 params) load via v110-acc loader.
    v112 params stay at fresh init if not in ckpt.
    """
    from tinygrad.nn.state import safe_load
    load_ckpt_v110_acc(model, path)
    sd = safe_load(path)
    v112_keys = [
        "fg_v114_commit_W_q", "fg_v114_commit_W_k", "fg_v114_commit_W_v",
        "fg_v114_commit_W_o", "fg_v114_commit_b_o",
        "fg_v114_commit_W_write", "fg_v114_commit_b_write",
        "fg_v114_prop_W_q", "fg_v114_prop_W_k", "fg_v114_prop_W_v",
        "fg_v114_prop_W_o", "fg_v114_prop_b_o",
        "fg_v114_prop_W_write", "fg_v114_prop_b_write",
    ]
    loaded = 0
    for key in v112_keys:
        if key in sd:
            ...
            loaded += 1
    if loaded > 0:
        print(f"  loaded {loaded} v112 dual-notebook params", flush=True)
    else:
        print(f"  v112 dual-notebook params: fresh init "
              f"(W_o zero → contribution=0 at step 0)", flush=True)
...
def main():
    ...
    CKPT_LABEL = getenv("CKPT_LABEL",              "v112_smoke")
    ...
    print(f"=== v112 dual-notebook training (cold-start) ===")
    print(f"  COMMIT notebook: writes EVEN breaths (collapse phase, waist ON)")
    print(f"  PROP   notebook: writes ODD breaths (expand phase, waist OFF)")
    print(f"  both READ every breath, both zero-init W_o (inert at step 0)")
    print(f"  gate_profile={GATE_PROFILE}  photon_alpha={PHOTON_ALPHA}  SBP_noise={SBP_NOISE}")
    ...
    print(f"  PYTHIA_INIT={PYTHIA_INIT}  RESUME_FROM='{RESUME_FROM}'")
    ...
    params   = fg_v112_parameters(model)
    ...
    if RESUME_FROM:
        ...
        load_ckpt_v112(model, RESUME_FROM)
    ...
    step_fn = _compile_jit_fg_step_v112(
    ...
    for step in range(1, STEPS + 1):
        ...
        # SBP noise: alternate noisy/det when SBP_NOISE > 0; else always zeros
        ...
        if step % LOG_EVERY == 0:
            dt = time.time() - t0
            # Notebook dim norms (track that the model is learning to use them)
            c_Wo_norm = float(np.linalg.norm(model.fg_v114_commit_W_o.numpy()))
            c_Wq_norm = float(np.linalg.norm(model.fg_v114_commit_W_q.numpy()))
            p_Wo_norm = float(np.linalg.norm(model.fg_v114_prop_W_o.numpy()))
            p_Wq_norm = float(np.linalg.norm(model.fg_v114_prop_W_q.numpy()))
            print(
                ...
                f"|c_Wo|={c_Wo_norm:.3f} |c_Wq|={c_Wq_norm:.2f} "
                f"|p_Wo|={p_Wo_norm:.3f} |p_Wq|={p_Wq_norm:.2f}  "
        ...
        if step % CKPT_EVERY == 0:
            ...
            safe_save(fg_v112_state_dict(model), ckpt_path)
