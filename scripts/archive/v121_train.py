"""v121 training driver — energy-selected perceiver writes to accumulate notebook.

Structural difference from v118-v120:
  - Reads INTERMEDIATE layer states (L0_out, L1_out, L2_out) — genuinely new info
  - Energy-based soft selection (no learned latent queries, no new cross-attn)
  - Writes THROUGH the existing validated accumulate notebook channel
  - No new cross-attention back to residual (the v118-v120 failure mode)

Warm-start from v112b_cont1_final.
W_select=0 at init → byte-identical to v112b at step 0.

Key diagnostic metrics (logged every LOG_EVERY steps):
  select_norm            — L2 norm of W_select (should grow > 0.05 by step 100)
  energy_temp            — current energy_temp scalar value
  energy_p50             — median per-position energy across layers (batch snapshot)
  energy_p95             — 95th percentile energy (are we finding high-energy peaks?)
  notebook_write_delta   — mean L2 norm of v121_write_delta across batch
...
from mycelium.factor_graph_v121 import (
    V121_ENERGY_TEMP_INIT,
    attach_fg_params_v121,
    fg_v121_parameters, fg_v121_state_dict,
    _compile_jit_fg_step_v121,
    compile_jit_eval_v121,
...
def load_ckpt_v121(model, path: str) -> None:
    """Load v110-step3/v112b ckpt + optionally v121-specific params if present."""
    ...
    # v121-specific params (present only in v121 ckpts, not v112b ckpts)
    for key in ("fg_v121_W_select", "fg_v121_energy_temp"):
        if key in sd and hasattr(model, key):
            dst = getattr(model, key)
            src = sd[key].to(dst.device).realize()
            dst.assign(src).realize()
            print(f"  loaded v121 param: {key}", flush=True)


def _compute_energy_diagnostics(model, batch, K, N_MAX, F_MAX, N_DIGITS,
                                ALTERNATION, PHASE_SCALE, GATE_PROFILE,
                                PHOTON_ALPHA, H, T):
    """Compute energy percentiles and select_norm for diagnostic logging.

    Runs a minimal non-JIT forward to extract energy stats. Called outside
    the JIT step, so AMD JIT graph is unaffected. Returns dict of floats.
    """
    diag = {}
    try:
        # select_norm: L2 norm of W_select
        select_norm = float(np.linalg.norm(model.fg_v121_W_select.numpy()))
        energy_temp_val = float(model.fg_v121_energy_temp.numpy()[0])
        diag["select_norm"] = select_norm
        diag["energy_temp"] = energy_temp_val
    except Exception as e:
        diag["select_norm"] = -1.0
        diag["energy_temp"] = -1.0
        print(f"  [diag] W_select norm error: {e}", flush=True)
    return diag
...
def main():
    ...
    CKPT_LABEL = getenv("CKPT_LABEL",              "v121_run")
    ...
    ENERGY_TEMP_INIT = float(getenv("V121_ENERGY_TEMP_INIT", str(V121_ENERGY_TEMP_INIT)))
    ...
    sbp_mode = "OFF" if SBP_NOISE == 0.0 else f"ON s={SBP_NOISE}"
    print(f"=== v121 training (energy-selected perceiver notebook writes) ===")
    print(f"  topology_dim={TOPOLOGY_DIM}  bias_scale_init={BIAS_SCALE_INIT}  SBP={sbp_mode}")
    print(f"  energy_temp_init={ENERGY_TEMP_INIT}")
    ...
    params   = fg_v121_parameters(model)
    ...
    if ckpt_to_load:
        ...
        load_ckpt_v121(model, ckpt_to_load)
    ...
    step_fn = _compile_jit_fg_step_v121(
    ...
    print(f"\ntraining (gate={GATE_PROFILE}, SBP={sbp_mode})...\n")
    t0 = time.time()
    ...
    for step in range(1, STEPS + 1):
        ...
        if step % LOG_EVERY == 0:
            ...
            print(
                ...
                f"select_norm={select_norm:.4f}  e_temp={energy_temp_val:.4f}  "
        ...
        if step % PER_BREATH_EVERY == 0:
            ...
            if K > 1 and len(pb_ce) >= 2:
                ...
                print(f"  [LADDER] B0-B{K-1} delta = {ladder_delta:.3f} ({tag})",
                      flush=True)
            # v121 specific: select_norm trajectory
            select_norm = float(np.linalg.norm(model.fg_v121_W_select.numpy()))
            energy_temp_val = float(model.fg_v121_energy_temp.numpy()[0])
            print(f"  [v121] select_norm={select_norm:.4f}  energy_temp={energy_temp_val:.4f}  "
                  f"(pass if select_norm > 0.05 by step 100)", flush=True)
        ...
        if step % EVAL_EVERY == 0:
            ...
            # Report stop criterion status
            hard_cell = results.get("hard", {}).get("cell_acc", 0.0)
            select_norm = float(np.linalg.norm(model.fg_v121_W_select.numpy()))
            print(f"  [v121 criterion] select_norm={select_norm:.4f} "
                  f"hard_cell={hard_cell:.4f}  "
                  f"(PASS: select_norm>0.05 AND hard>0.362)", flush=True)

        if step % CKPT_EVERY == 0:
            ckpt_path = os.path.join(ckpt_dir, f"{CKPT_LABEL}_step{step}.safetensors")
            safe_save(fg_v121_state_dict(model), ckpt_path)
