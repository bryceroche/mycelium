...
Single experimental variable vs #236: the (L=32, T=24) latent_topology_mask
...
Output artifacts:
  ...
  .cache/v200_smoke/train_237.log               — first line ADVISORY: ...
  .cache/v200_smoke/step2000_eval_237.json      — final eval + all reads
  .cache/v200_smoke/instrumentation_237.json    — per-checkpoint metric timeseries
  .cache/v200_smoke/grad_norms_237.npz
  .cache/v200_smoke/persistence/step{N}_z_237.npz       — per eval checkpoint
  .cache/v200_perceiver_ckpts/v200_perceiver_237_mask1a_step{N}.safetensors

Usage:
  cd /home/bryce/mycelium
  bash scripts/v200_resmoke_237.sh
"""
...
def main() -> None:
    ...
    CKPT_LABEL = getenv("CKPT_LABEL",  "v200_perceiver_237_mask1a")
    SMOKE_DIR  = getenv("SMOKE_DIR",   ".cache/v200_smoke")
    REF_DIR    = os.path.join(SMOKE_DIR, "reference_curves")

    SMOKE_LOG_PATH  = os.path.join(SMOKE_DIR, "train_237.log")
    EVAL_JSON_PATH  = os.path.join(SMOKE_DIR, f"step{STEPS}_eval_237.json")
    INSTR_JSON_PATH = os.path.join(SMOKE_DIR, "instrumentation_237.json")
    GRAD_NORMS_PATH = os.path.join(SMOKE_DIR, "grad_norms_237.npz")
    ...
    config_sig  = f"K{K}_L{N_LATENTS}_prenorm5_seamthree_gate-2_mask1a"
    ...
    if advisory_prefix:
        log(advisory_prefix)
    log("=" * 72)
    log("v200 #237 — STRUCTURAL READ MASKS (§2 per-latent topology mask, partition 1a)")
    log(f"  Single variable vs #236: latent_topology_mask (24 per-token + 4 per-op + 4 global)")
    ...
    def _persist_z(probe: dict, step_num: int) -> None:
        ...
        eff_gate = 1.0 / (1.0 + np.exp(-np.asarray(
            ...
            dtype=np.float32)[:K]))
        z_path = os.path.join(PERSIST_DIR, f"step{step_num}_z_237.npz")
        np.savez(
            z_path,
    ...
    # trajectory lives in instrumentation_237.json)
    # ====================================================================
    ...
    # _persist_z (persistence/step{N}_z_237.npz + provenance), incl. step 0
    ...
    if STAGE2A:
        ...
        log(f"  note: first deployment of the recalibrated metric — no prior-run reference;"
            f" values persisted per checkpoint in instrumentation_237.json")
    else:
        crit5 = True
    ...
    if n_pass == 6:
        final_line = (
            f"{adv_line_prefix}#237 MASKS RUN PASSED  run=237  " + common_tail + "  all6=YES"
        )
    else:
        failed = [f"C{i+1}" for i, c in enumerate(criteria_passed) if not c]
        final_line = (
            f"{adv_line_prefix}#237 MASKS RUN FAILED  run=237  failed={','.join(failed)}  "
            + common_tail
        )
    ...
    print(f"Latent z bundles: {PERSIST_DIR}/step{{N}}_z_237.npz")
