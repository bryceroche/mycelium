"""v101 factor graph training driver — v100 + per-breath waist compression.

Architectural change over v100:
  Per-breath waist projection (1024→512→1024, GELU) inserted after the 4
  transformer layers and before the delta_gate residual update.  This is the
  JPEG codec Quantize step — the lossy compression that forces commitment.
...
Env vars (set in launchers):
  ...
  PYTHIA_INIT=1
  V101_CURRICULUM=1
  V101_CURRICULUM_ANNEAL=1000
  V101_DIFFICULTY_FILTER=easy

Also accepts all V100_* env vars for data loader compatibility (DIFFICULTY_FILTER
reads V101_ first, falls back to V100_ for convenience).
"""
import gc
import os
import sys
...
from mycelium.factor_graph_v101 import (
    V101_K_MAX, V101_FACTOR_AUX_WEIGHT, V101_CALIB_WEIGHT,
    V101_N_MAX, V101_F_MAX, V101_N_HEADS, V101_KL_DIAG, V101_WAIST,
    attach_fg_params_v101, fg_v101_waist_parameters, fg_v101_state_dict,
    fg_breathing_forward_v101,
    _compile_jit_fg_step_v101, _compile_jit_fg_eval_v101,
)
...
def collect_fg_params_v101(model) -> list[Tensor]:
    """Trainable parameters: shared L0-L3 attn/FFN + v100 fg params + v101 waist params."""
    params: list[Tensor] = []
    sw = model.block.shared
    ...
    params += fg_v101_waist_parameters(model)
    return params


def model_state_dict_v101(model) -> dict:
    """State dict for v101 factor-graph training (v100 keys + v101 waist keys)."""
    sd = {
        "ln_f.g": model.ln_f_g,
        "ln_f.b": model.ln_f_b,
    }
    ...
    sd.update(fg_v101_state_dict(model))
    return sd


def load_ckpt_v101(model, path: str):
    """Load a v100 or v101 checkpoint.

    v100 keys are loaded exactly (all 45 expected keys present).
    v101 waist keys (fg_v101.*) are loaded if present; if missing (e.g. warm-starting
    from a v100 ckpt), the near-identity init from attach_fg_params_v101 is kept.
    """
    sd = safe_load(path)
    targets = model_state_dict_v101(model)
    missing = []
    loaded  = []
    ...
    v101_waist_keys = [k for k in targets if k.startswith("fg_v101.")]
    loaded_waist    = [k for k in loaded  if k.startswith("fg_v101.")]
    if not loaded_waist:
        print(f"  no v101 waist keys in ckpt — keeping near-identity init (warm-start from v100)")
    else:
        print(f"  loaded {len(loaded_waist)}/{len(v101_waist_keys)} v101 waist keys")
    if missing:
        non_waist_missing = [k for k in missing if not k.startswith("fg_v101.")]
        if non_waist_missing:
            print(f"  ckpt missing {len(non_waist_missing)} non-waist keys (kept init): "
                  f"{non_waist_missing[:5]}")


def evaluate_v101(model, loader: FactorGraphLoaderV100, K: int,
                  max_batches: int = 20, eval_fn=None,
                  n_max: int = V101_N_MAX, f_max: int = V101_F_MAX) -> dict:
...
def main():
    V101_TASK_LOCAL = int(getenv("V101_TASK", 0)) > 0
    assert V101_TASK_LOCAL, "V101_TASK=1 must be set"

    K      = int(getenv("V101_K_MAX",    str(V101_K_MAX)))
    ...
    CKPT_LABEL        = getenv("CKPT_LABEL",            "v101_smoke")
    RESUME_FROM       = getenv("RESUME_FROM",           "")
    PYTHIA_INIT       = int(getenv("PYTHIA_INIT",       "1")) > 0
    SEED              = int(getenv("SEED",              "42"))
    WAIST             = int(getenv("V101_WAIST",        str(V101_WAIST)))

    N_MAX  = int(getenv("V101_N_MAX",  str(V101_N_MAX)))
    F_MAX  = int(getenv("V101_F_MAX",  str(V101_F_MAX)))
    T_MAX  = N_MAX + F_MAX

    DIFFICULTY_FILTER = (
        os.environ.get("V101_DIFFICULTY_FILTER", "").strip()
        or os.environ.get("V100_DIFFICULTY_FILTER", "").strip()
        or None
    )
    CURRICULUM        = int(getenv("V101_CURRICULUM",       "0")) > 0
    CURRICULUM_ANNEAL = int(getenv("V101_CURRICULUM_ANNEAL", "1000"))

    TRAIN_PATH   = getenv("V101_TRAIN", getenv("V100_TRAIN", ".cache/factor_graph_train.jsonl"))
    VAL_PATH     = getenv("V101_VAL",   getenv("V100_VAL",   ".cache/factor_graph_test.jsonl"))
    EVAL_BATCHES = int(getenv("EVAL_BATCHES", "20"))
    EVAL_BATCH   = int(getenv("EVAL_BATCH",   str(BATCH)))

    FACTOR_AUX_WEIGHT = float(getenv("V101_FACTOR_AUX_WEIGHT", str(V101_FACTOR_AUX_WEIGHT)))
    CALIB_WEIGHT      = float(getenv("V101_CALIB_WEIGHT",       str(V101_CALIB_WEIGHT)))

    KL_DIAG_EVERY   = int(getenv("KL_DIAG_EVERY", "100"))
    KL_DIAG_ENABLED = V101_KL_DIAG or (int(getenv("V101_KL_DIAG", "0")) > 0)

    print(f"=== v101 factor graph training (v100 + per-breath waist compression) ===")
    print(f"device={Device.DEFAULT}  B={BATCH}  K={K}  steps={STEPS}  lr={LR}")
    print(f"N_MAX={N_MAX}  F_MAX={F_MAX}  T_MAX={T_MAX}  waist={WAIST}")
    ...
    # Attach v100 params first, then v101 waist params
    attach_fg_params_v100(model, hidden=cfg.hidden, n_heads=cfg.n_heads,
                          k_max=K, n_max=N_MAX, f_max=F_MAX)
    attach_fg_params_v101(model, hidden=cfg.hidden, waist=WAIST)
    Device[Device.DEFAULT].synchronize()

    params = collect_fg_params_v101(model)
    n_params = sum(int(np.prod(t.shape)) for t in params)
    print(f"  trainable params: {n_params/1e6:.1f}M")
    # Report waist param count separately
    waist_params = fg_v101_waist_parameters(model)
    n_waist = sum(int(np.prod(t.shape)) for t in waist_params)
    print(f"  waist params: {n_waist/1e6:.3f}M  "
          f"(W_compress={cfg.hidden}×{WAIST}={cfg.hidden*WAIST}, "
          f"W_expand={WAIST}×{cfg.hidden}={WAIST*cfg.hidden})")

    if RESUME_FROM:
        print(f"resuming from ckpt: {RESUME_FROM}")
        load_ckpt_v101(model, RESUME_FROM)
        print("  loaded.")
    ...
    train_loader = FactorGraphLoaderV100(
        ...
        n_max=N_MAX, f_max=F_MAX, k_max=K, n_heads=V101_N_HEADS,
        seed=SEED,
    )
    ...
    ckpt_dir = ".cache/fg_v101_ckpts"
    os.makedirs(ckpt_dir, exist_ok=True)

    Tensor.training = True
    step_fn = _compile_jit_fg_step_v101(
    ...
    eval_fn = _compile_jit_fg_eval_v101(
    ...
    for step in range(1, STEPS + 1):
        ...
        if KL_DIAG_ENABLED and step % KL_DIAG_EVERY == 0:
            Tensor.training = False
            var_lh_diag, _, _ = fg_breathing_forward_v101(
                model, domain_init, node_kinds, staging_mask, head_op_mask,
                K=K, n_max=N_MAX, f_max=F_MAX,
            )
        ...
        if step % CKPT_EVERY == 0:
            ...
            safe_save(model_state_dict_v101(model), ckpt_path)
