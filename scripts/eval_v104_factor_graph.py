"""v104 factor graph evaluation: per-difficulty accuracy + K-sweep + depth diagnostics.

Architecture: IB-anchored 1024d residual codebook matching (v102 forward path,
semantic centroid initialization).

Usage:
  python scripts/eval_v104_factor_graph.py CKPT_PATH [--test PATH] [--K N] [--batch N]
...
Codebook diagnostic: prints temperature, codebook pairwise cosine statistics, and
         delta_gate_quant values (proxy for how much the IB codebook path contributes).
         Low |cosine|   → orthogonal codebook entries (good separation).
         Large gate      → codebook path active.
"""
import argparse
import os
import sys
...
from mycelium.factor_graph_v104 import (
    V104_K_MAX, V104_N_MAX, V104_F_MAX, V104_CODEBOOK_N, V104_IB_CENTROIDS,
    attach_fg_params_v104, fg_v104_state_dict,
...
def load_ckpt_v104(model, path: str):
    ...
    loaded    = [k for k in v104_keys if k in sd]
    if not loaded:
        print(f"  INFO: no v104 codebook keys in ckpt — using IB-anchored init")
    else:
        print(f"  loaded {len(loaded)}/{len(v104_keys)} v104 codebook keys from ckpt")
...
def run_eval_at_K(model, loader: FactorGraphLoaderV100, K: int,
                  ...
                  f_max: int = V104_F_MAX) -> tuple[dict, dict]:
...
def print_codebook_diagnostic(model):
    """Print learned codebook state: cosine stats + gate activation signal."""
    cb  = model.fg_v104_codebook.realize().numpy()           # (N_CODE, H)
    gq  = model.fg_v104_delta_gate_quant.realize().numpy()   # (K_max,)
    tmp = float(model.fg_v104_temperature.realize().numpy().flat[0])

    n_code, hidden = cb.shape
    ...
    print(f"  Codebook (IB-anchored): n_code={n_code}  hidden={hidden}  temperature={tmp:.4f}")
    ...
    print(f"    delta_gate_quant: {np.array2string(gq[:5], precision=4)}...  "
          f"({gq_nz}/{len(gq)} nonzero > 1e-6)")
    print(f"    gate max/mean: {np.abs(gq).max():.4f} / {np.abs(gq).mean():.4f}")


def main():
    ap = argparse.ArgumentParser()
    ap.add_argument("ckpt", help="Path to a .safetensors v104 factor-graph ckpt.")
    ap.add_argument("--test",    default=".cache/factor_graph_test.jsonl")
    ap.add_argument("--K",       type=int, default=None,
                    help="Single K to eval at (default = V104_K_MAX env var or ckpt K).")
    ...
    ap.add_argument("--n_max",   type=int, default=V104_N_MAX)
    ap.add_argument("--f_max",   type=int, default=V104_F_MAX)
    ap.add_argument("--n_code",  type=int, default=V104_CODEBOOK_N)
    ap.add_argument("--ib",      type=str, default=V104_IB_CENTROIDS,
                    help="Path to IB centroids npz (for attach_fg_params_v104).")
    args = ap.parse_args()
    ...
    attach_fg_params_v104(model, hidden=cfg.hidden, n_code=args.n_code,
                          ib_centroids_path=args.ib)
    Device[Device.DEFAULT].synchronize()

    print(f"loading ckpt {args.ckpt}...")
    load_ckpt_v104(model, args.ckpt)
