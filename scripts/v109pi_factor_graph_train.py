"""v109pi factor graph training driver — v109 + per-breath Q rotation."""
import gc
import os
import sys
import time
...
from mycelium.factor_graph_v109pi import (
    ...
    V109PI_WAIST_DIM, V109PI_ALTERNATION, V109PI_HARD_BREATH_LEVEL,
    V109PI_VAR_LOSS_WEIGHT, V109PI_CALIB_WEIGHT, V109PI_FACTOR_AUX_WEIGHT,
    V109PI_CODEBOOK_N, V109PI_IB_CENTROIDS, V109PI_PHASE_SCALE,
    attach_fg_params_v109pi, fg_v109pi_parameters, fg_v109pi_state_dict,
    fg_breathing_forward_v109pi,
    _compile_jit_fg_step_v109pi, _compile_jit_fg_eval_v109pi,
)
...
# Inherit infrastructure from v109; only the forward changes.
collect_fg_params_v109pi = collect_fg_params_v109
model_state_dict_v109pi = model_state_dict_v109
load_ckpt_v109pi = load_ckpt_v109
evaluate_v109pi = evaluate_v109


def main():
    V109PI_TASK_LOCAL = int(getenv("V109PI_TASK", 0)) > 0
    assert V109PI_TASK_LOCAL, "V109PI_TASK=1 must be set"

    K          = int(getenv("V109PI_K_MAX",     str(V109PI_K_MAX)))
    ...
    CKPT_LABEL = getenv("CKPT_LABEL",           "v109pi_smoke")
    RESUME_FROM = getenv("RESUME_FROM",         "")
    PYTHIA_INIT = int(getenv("PYTHIA_INIT",     "1")) > 0
    SEED       = int(getenv("SEED",             "42"))
    N_CODE     = int(getenv("V109PI_CODEBOOK_N", str(V109PI_CODEBOOK_N)))
    IB_PATH    = getenv("V109PI_IB_CENTROIDS",   V109PI_IB_CENTROIDS)
    N_DIGITS   = int(getenv("V109PI_N_DIGITS",   str(V109PI_N_DIGITS)))
    WAIST_DIM  = int(getenv("V109PI_WAIST_DIM",  str(V109PI_WAIST_DIM)))
    ALTERNATION = int(getenv("V109PI_ALTERNATION", "1")) > 0
    HARD_LEVEL = int(getenv("V109PI_HARD_BREATH_LEVEL", "0")) > 0
    PHASE_SCALE = float(getenv("V109PI_PHASE_SCALE", str(V109PI_PHASE_SCALE)))
    VW         = float(getenv("V109PI_VAR_LOSS_WEIGHT",  str(V109PI_VAR_LOSS_WEIGHT)))
    FW         = float(getenv("V109PI_FACTOR_AUX_WEIGHT", str(V109PI_FACTOR_AUX_WEIGHT)))
    AW         = float(getenv("V109PI_CALIB_WEIGHT",      str(V109PI_CALIB_WEIGHT)))
    N_MAX      = int(getenv("V109PI_N_MAX",     str(V109PI_N_MAX)))
    F_MAX      = int(getenv("V109PI_F_MAX",     str(V109PI_F_MAX)))

    DIFFICULTY_FILTER = os.environ.get("V109PI_DIFFICULTY_FILTER", "").strip() or None
    TRAIN_PATH   = getenv("V109PI_TRAIN",       ".cache/factor_graph_train.jsonl")
    VAL_PATH     = getenv("V109PI_VAL",         ".cache/factor_graph_test.jsonl")
    GSM8K_PATH   = getenv("V109PI_GSM8K_TRAIN", ".cache/gsm8k_factor_graphs_train.jsonl")
    GSM8K_RATIO  = float(getenv("V109PI_GSM8K_RATIO", "0.5"))
    EVAL_BATCHES = int(getenv("EVAL_BATCHES", "10"))
    EVAL_BATCH   = int(getenv("EVAL_BATCH",   str(BATCH)))

    print(f"=== v109pi factor graph training (v109 + π-cycled Q rotation) ===")
    ...
    params   = collect_fg_params_v109pi(model)
    ...
    if RESUME_FROM:
        ...
        load_ckpt_v109pi(model, RESUME_FROM)
    ...
    synth_loader = FactorGraphLoaderV107(
        ...
        n_max=N_MAX, f_max=F_MAX, k_max=K, n_heads=V109PI_N_HEADS, seed=SEED,
    )
    ...
    dual_loader = DualDataLoaderV107(
        ...
        n_max=N_MAX, f_max=F_MAX, k_max=K, n_heads=V109PI_N_HEADS, seed=SEED + 1,
    )
    val_loader = FactorGraphLoaderV107(
        VAL_PATH, batch_size=EVAL_BATCH,
        difficulty_filter=None, curriculum=False,
        n_max=N_MAX, f_max=F_MAX, k_max=K, n_heads=V109PI_N_HEADS, seed=SEED + 2,
    )

    ckpt_dir = ".cache/fg_v109pi_ckpts"
    os.makedirs(ckpt_dir, exist_ok=True)

    Tensor.training = True
    step_fn = _compile_jit_fg_step_v109pi(
    ...
    for step in range(1, STEPS + 1):
        ...
        if step % CKPT_EVERY == 0:
            ...
            safe_save(model_state_dict_v109pi(model), ckpt_path)
