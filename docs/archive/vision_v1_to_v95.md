# Mycelium v4: The Original Vision (Archived)

This describes the v1-v95 conceptual arc that motivated the project.
It does NOT describe the current implementation. See `README.md` for
the current state.

Archived 2026-06-01 after the v98 Sudoku pivot.

The legacy machinery this document describes (`Controller`, `Notebook`,
`LookupTable`, sine-modulated within-breath temperature, controller-emitted
gate/step_mult, π-cycled per-breath RoPE) lives in `mycelium/breathing.py`
but is not called by any current training path. The current architecture
(v98 Sudoku / v100-v107 factor graphs / v105 family digit-level) is
described in the current `README.md` and `CLAUDE.md`.
...
|---|---|---|---|
| 1 | Rotation (π-cycled RoPE, per-head phase offsets) | implemented | `breathing.py: RoPE` |
| 2 | Integration (gated running integral across breaths) | implemented | `breathing.py: BreathingBlock.breathe` |
| 3 | Notebook (512d pages, persisting across cycles) | implemented | `controller.py: Notebook` |
| 4 | Lookup table (16×1024 cosine matcher, joint-trained) | implemented | `lookup_table.py: LookupTable` |
| 5 | Controller (state reader + decision heads + notebook attn) | implemented | `controller.py: Controller` |
| 6 | Temperature modulation (controller scalar × sine baseline) | implemented | `breathing.py: BreathingLayer (temp_mult)` |
| 7 | Step size / rotation rate (controller emits step_mult) | implemented | `breathing.py: breathe_controlled` |
...
## 13. Summary — "The Shape of Thought" (original framing)
